...
def train(
    ...
    # Light xavier init for all linear layers.
    ...
    for epoch in range(epochs):
        for _ in range(steps_per_epoch):
            ...
            if step < warmup_steps:
                # Linear ramp from 0.0 to lambda_ep
                current_lambda_ep = lambda_ep * (step / warmup_steps)
            else:
                current_lambda_ep = lambda_ep

            loss_g = loss_gan + current_lambda_ep * ep_z
        ...
        torch.save(
            {
                ...
                "opt_prior_state_dict": opt_prior.state_dict(),
                "opt_D_state_dict": opt_D.state_dict(),
            },
            out_path / "model_latest.pt",
        )
