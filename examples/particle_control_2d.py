...
(examples/yue2_particle_2d.py, experiments/train_gym_particle_finetune.py).
This file is the earlier pure RpGAN + sample-point b_cap game. A passing run
prints GATE COLLAPSE: the baseline fails after imitation.

The parent is imitation. The baseline then drops that paired loss and trains
with Rp logistic GAN, sample-point b_cap, a shared action head, and a particle
cloud at 100x the head learning rate.
...
Knob map (toy -> collapsed particle game, not the YuE2 default):

| Toy | Collapsed particle finetune |
| --- | --- |
...
| Rp logistic + sample ``b_cap`` | ``require_classic_particle_gan`` (this FAIL baseline only) |
