"""Frozen Anima transformer blocks transplanted into a CIFAR U-Net.
...
class DonorAttention(nn.Module):
    ...
    def forward(self, x, context, rope=None):
        ...
        q = self.q_norm(heads(self.q_proj(x)))
        k = self.k_norm(heads(self.k_proj(context)))
...
class FrozenAnima(nn.Module):
    ...
    @torch.no_grad()
    def prepare(self, alpha_bar, dtype):
        ...
        raw = torch.cat([phase.cos(), phase.sin()], -1)
        shared = self.t_embedder[1](raw)
        e = self.t_embedding_norm(raw)
        tables = [torch.stack([getattr(block, 'adaln_modulation_' + stage)(e) + shared
                               for stage in block.stages], 1) for block in self.blocks.values()]
        self.to(dtype=dtype)
        self.register_buffer('modulation_table', torch.stack(tables), persistent=False)
    ...
    def forward(self, x, context, t):
        ...
        with torch.autocast(x.device.type, dtype=torch.bfloat16, enabled=dtype == torch.bfloat16):
            for i, block in enumerate(self.blocks.values()):
                x = block(x, context, self.modulation_table[i, t], (self.rope_cos, self.rope_sin))
        return x


class AnimaTransplantGenerator(ImageGenerator):
    """Existing U-Net plus a zero-start, frozen donor branch at its 8x8 grid."""
    def __init__(self, cfg):
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.donor = FrozenAnima(cfg['anima_blocks'], width, self.context_width, heads, rank)
            ...
            self.pretrained_metadata = {**bundle['metadata'], 'bundle_sha256': digest,
                                       'initialization': cfg['anima_init'], 'frozen': True,
            ...
            self.donor.prepare(cfg['alpha_bar'], dtype)
