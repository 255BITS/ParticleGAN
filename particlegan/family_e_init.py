...
def _sign_columns(q: torch.Tensor, key: int) -> torch.Tensor:
    signs = torch.tensor([_thue(j + key * 17) for j in range(q.shape[1])], dtype=torch.float64, device="cpu")
    return q * signs
...
def _square_givens(n: int, key: int, alpha: float) -> torch.Tensor:
    q = torch.eye(n, dtype=torch.float64, device="cpu")
    step = 0
    for i in range(n):
...
def _real_fourier(n: int) -> torch.Tensor:
    ...
    cols = [torch.full((n,), 1.0 / math.sqrt(n), dtype=torch.float64, device="cpu")]
...
def _circulant(n: int, key: int) -> torch.Tensor:
    spec = torch.empty(n, dtype=torch.float64, device="cpu")
    spec[0] = _thue(key)
...
def _skew_dense(n: int, key: int) -> torch.Tensor:
    a = torch.zeros((n, n), dtype=torch.float64, device="cpu")
...
def _expm_skew(n: int, key: int) -> torch.Tensor:
    ...
    b = a / (2.0 ** steps)
    term = torch.eye(n, dtype=torch.float64, device="cpu")
    out = term.clone()
    for k in range(1, 18):
        term = term @ (b / k)
        out = out + term
...
def _sobol(n: int, d: int, key: int) -> torch.Tensor:
    ...
    unit = torch.tensor(rows, dtype=torch.float64, device="cpu").clamp(1e-12, 1.0 - 1e-12)
...
def _lattice(n: int, d: int, key: int) -> torch.Tensor:
    ...
    while len(primes) < d:
        if all(p % q != 0 for q in primes):
            primes.append(p)
        p += 1 if p == 2 else 2
    alpha = torch.tensor([math.sqrt(p) for p in primes], dtype=torch.float64, device="cpu")
...
def _qr_square(block: torch.Tensor) -> torch.Tensor:
    ...
    sign = torch.where(torch.diagonal(r) < 0, -torch.ones(r.shape[0], dtype=torch.float64, device="cpu"),
                       torch.ones(r.shape[0], dtype=torch.float64, device="cpu"))
    return q * sign
...
def basis(family: str, rows: int, cols: int, key: int = 0) -> torch.Tensor:
    ...
    if rows < 1 or cols < 1:
        return torch.zeros((rows, cols), dtype=torch.float64, device="cpu")
    size = max(rows, cols)
    if family == "giv":
...
def _r2(n: int, d: int) -> torch.Tensor:
    ...
    for _ in range(64):
        phi = (1.0 + phi) ** (1.0 / (d + 1))
    alpha = torch.tensor([phi ** (-(j + 1)) for j in range(d)], dtype=torch.float64, device="cpu")
    index = torch.arange(1, n + 1, dtype=torch.float64, device="cpu")[:, None]
...
def _pattern(n: int, dist: tuple, key: int) -> torch.Tensor:
    ...
    unit = torch.tensor([_frac((i + 1) * _PHI + key * _PHI) for i in range(n)], dtype=torch.float64, device="cpu")
    unit = unit * 2.0 - 1.0
    if n > 1:
...
def _weyl_quarter(n: int, dist: tuple, key: int) -> torch.Tensor:
    ...
    if n == 0:
        return torch.zeros(0, dtype=torch.float64, device="cpu")
    index = torch.arange(1, n + 1, dtype=torch.float64, device="cpu")
...
def _rewrite(param: nn.Parameter, dist: tuple, key: int, oname: str, pname: str) -> tuple[torch.Tensor, str]:
    ...
    if param.ndim == 1:
        bias = _STATE["bias"]
        if bias == "bz":
            return torch.zeros(param.shape, dtype=torch.float64, device="cpu"), "bias_zero"
        if bias == "wq":
