...
validation (20/20 down to 0/20). Collect now rolls a safe teacher and a
speed-biased fast teacher on the same seed, keeps a pair only when both land,
and aligns by progress. `safe_fast_weight` stays 0. The note is
