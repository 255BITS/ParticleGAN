...
**Not the default.** This note is the PR #16 autopsy. The default particle
gym recipe is YuE2 paired-error RpGAN at `adv_weight=1`, which scored
validation **20/20**, test **50/50**, mean return **287.7** on the shared
control protocol
([PR #18](https://github.com/255BITS/ParticleGAN/pull/18)). Do not treat the
teacher-forced MSE gate below as a landing result, and do not change the
YuE2 default because this pad fails the #16 recipe.
...
2. **The #16 gym trainer ran the fixed arm, not the collapsed one.** The
   current default trainer is YuE2 paired-error RpGAN (`controller_objective`),
   not this live pair. On the #16 run, trainable
...
This autopsy explains the #16 miss. It is not a change to the gym trainer.

The default stays YuE2 paired-error RpGAN at adversarial weight 1. That recipe
is the one that landed on the shared protocol (validation 20/20, test 50/50,
mean return 287.7, [PR #18](https://github.com/255BITS/ParticleGAN/pull/18)).
Do not set `adv_weight` to 0. Do not read EMA action MSE ≤ 0.18, or a training
`diag_action_mse` in 0.07–0.18, as a landing score. Those are the old gate.
On this pad the #16 fixed arm passes that gate and fails the honest one
(landing rate 0.326). The collapsed run and the #16 run already showed that
proxy moving while test landings stayed at 2/50 and 4/50.

The honest pad (learner previous command, landing rate ≥ 0.80) is the CPU
check for a future live `(record, z)` change. It is not a gate on the YuE2
default, which was scored in the simulator. Scoring only the action slice
inside the same teacher-forced RpGAN scores 0.574 and is not a Lunar recipe.

The training change this note left untested is a batch whose previous command
is the controller's own action along a short rollout of this plant, with the
loss still the live RpGAN pair. That idea is not the shipped default.
