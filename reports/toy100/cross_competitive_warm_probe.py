...
def variants():
    def factory(method):
        ...
        def activate(state,prefix):
            recorder,_=prefix;recorder.enabled=method=='cross_only'
            ...
            receipt=dict(method=method,shared_gate_eligible=False,
                scratch_optimizer_policy='same_sample_cross_only_competitive_response' if recorder.enabled else 'control')
        ...
        return activate
    return {name:factory(name) for name in ('identity','constant','cross_only')}
