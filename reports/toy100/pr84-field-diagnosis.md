...
The [proxy script](pr84_coverage_proxy.py) uses the audit's cold seven-mode and passing warm eight-mode **single noisy draw per particle** (`support_scope` in the host receipt). It draws one 128-example real minibatch, matching the locked mode-hold host's actual `BATCH=128`, then applies 20 exact nearest-real centroid (Lloyd) updates in **output coordinates only**. The transfer recipe resolves a different batch size, but this legacy host does not receive that recipe. Each real point is assigned to its nearest generated support point; empty cells stay fixed. The mode centers are used solely to score the resulting clouds.

| Cloud | Initial | After 1 centroid update | After 2 | After 20 |
| --- | --- | --- | --- | --- |
| Cold | 7 modes, HQ 1.0, coverage loss .45835 | 7, .9167, .02916 | 8, 1.0, .00686 | 8, 1.0, .00657 |
| Warm | 8 modes, HQ 1.0, coverage loss .00931 | 8, 1.0, .00626 | 8, 1.0, .00607 | 8, 1.0, .00603 |

At the cold cloud the centroid directions for two particles project `+1.708` and `+0.147` toward the missing mode; the critic-guided proposal above projects away from it. This supports a one-sided coverage correction as a **candidate**, not a result in parameter space. The first cold centroid step temporarily drops one particle outside the HQ radius. The [full proxy receipt](continuous-evidence/pr84-field/coverage-proxy.json) fixes the single sampled minibatch and all 21 measurements; no seeds, widths, or gains were searched. The proxy starts from a noisy observation and does not account for generator Jacobian coupling, further noise, Adam moments, or interaction with the GAN update.

## Exact clean final-state pullback filter

The [one-state probe](pr84_pullback_final_probe.py) replays the same 1,200-step frozen PR84 run and checks the complete untimed host result and core per-update optimizer invariants against the independent cold audit. It captures the actual final phase's D real batch from the host sampler, then measures one [prior-only Adam-metric centroid pullback](coverage_pullback.py) with the clean, unwrapped generator. The correction is undone afterward; it does not participate in the audited run. It accepts a full nonlinear step (`alpha=1`) and reduces its own one-sided minibatch coverage loss from `.64466` to `.01128`. Clean particle support changes from seven modes/HQ `1.0` to eight modes/HQ `.9167` at that single state. The nearest particle moves `+2.330` toward the missing mode, and all 12 local output Jacobians have rank two with linearized residuals below `2.4e-8`. The prior Adam moments and global RNG remain unchanged. The [exact receipt](continuous-evidence/pr84-field/pullback-final.json) records the same-batch hash, correction, output displacements, and source hashes. This validates one nonlinear step only; sustained warm and cold training remain separate gates.
