...
Faster twin of probe.py: same audit, same receipts, same numbers. It caches
the per-overload tag lookup in the random-op dispatch hook, clones parameters
and gradients only for steps whose mobility row is recorded (and the step
before, which the gradient cosine needs), and stops recomputing the invariant
optimizer device/dtype receipts on every step. Eight gates reproduce the
committed results exactly under replay.py's comparison; see probe-fast-check.json.
"""
import argparse
...
class RandomAudit(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        # Every aten op reaches this hook; cache the per-overload tag lookup so
        # the non-random fast path stays cheap. Same ops audited as before.
