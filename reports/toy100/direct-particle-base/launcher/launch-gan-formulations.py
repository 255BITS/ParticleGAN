#!/usr/bin/env python3
"""Start three bounded, detached formulation attempts and point the live monitor at them."""
import argparse
...
def main():
    ...
    gate_order = research['first_gates'] + research['remaining_regression_gates']
    common = COMMON
    ...
    if args.dry_run:
        print(json.dumps(dict(base=base, batch=str(batch), minutes=args.minutes,
                              gpu=GPU, lanes=list(LANES), proposals_per_lane=args.proposals), indent=2))
        return
    batch.mkdir()
    ...
    for lane, focus in LANES.items():
        ...
        command = [str(ROOT / 'try-gan.sh'), '--repo', str(REPO), '--base', base,
                   '--gpu', GPU, '--minutes', str(args.minutes), '--candidates', str(args.proposals),
                   '--workers', '1', '--runs-dir', str(directory), '--prompt-file', str(brief),
                   '--focus', lane]
        ...
        records.append(dict(lane=lane, pid=process.pid, directory=str(directory), command=command,
                            ...
                            gpu=GPU, workers=1, gate_order=gate_order))
