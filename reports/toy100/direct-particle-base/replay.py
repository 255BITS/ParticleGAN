"""Replay a selected direct-particle GAN gate from retained source and fixtures."""
import argparse
import gzip
import hashlib
import json
...
p.add_argument('--task', required=True, choices=sorted(x.name.removesuffix('.json.gz')
               for x in (bundle / 'results').glob('*.json.gz')))
...
assert sha(bundle / 'prepared-sources.json') == declaration['source_manifest_sha256']
...
for key in ('status', 'config', 'spec', 'result', 'verdict', 'mobility',
            'regularizer_receipt', 'randomness', 'response_receipt'):
    assert without_timing(actual[key]) == without_timing(reference[key]), key
...
assert list(actual['proof']['optimizers'].values()) == list(reference['proof']['optimizers'].values())
assert actual['worker_sha256'] == declaration['file_hashes']['probe.py']
...
receipt = dict(status='PASS', replay_verdict=actual['status'], task=a.task,
               ...
               random_draws_equal=True, response_and_mobility_equal=True,
               cuda_updates_verified=True, file_hashes=declaration['file_hashes'])
