...
def main(argv=None):
    ...
    (root / "model_options.json").write_text(json.dumps(options, indent=2) + "\n")
    ...
    torch.manual_seed(config["seed"])
    summary = runner.train(config, root / "grid100")
    gate = evaluate_suite(root, problem="grid100")
    print(json.dumps({"variant": args.variant, "summary_status": summary["status"],
                      "gate_status": gate["status"],
                      "final_live": gate["problems"]["grid100"].get("final_metrics")}), flush=True)
