"""One-variable batch-1024 follow-up of the saved sigma05 input-noise probe.
...
def main(argv=None):
    ...
    base_config = ROOT / "artifacts/toy100/instance-noise-cpu/sigma05/staggered100/config.json"
    config = json.loads(base_config.read_text())
    config.update(name=f"output026_input_{args.variant}_batch1024", device=args.device,
                  problem=args.problem, batch_size=1024)
    original_provenance = runner._source_provenance

    def provenance():
        value = original_provenance()
        value["source_sha256"]["artifacts/toy100/instance_noise_batch1024_probe.py"] = options["source_sha256"]
        original = ROOT / "artifacts/toy100/instance_noise_probe.py"
        value["source_sha256"]["artifacts/toy100/instance_noise_probe.py"] = hashlib.sha256(original.read_bytes()).hexdigest()
        value["trainer_factory"] = "instance_noise_batch1024_probe.make_factory"
