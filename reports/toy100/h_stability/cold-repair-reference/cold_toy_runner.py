"""H cold failure screens using the frozen runner, with complete host snapshots.
...
from cold_policy import cold_policy
...
critic_signal.signal_policy = cold_policy
...
def sources():
    ...
    for path in (Path(__file__), Path(__file__).with_name("cold_policy.py")):
