    ...
    def gradients(self, player, completed, draws=1):
        ...
        for _ in range(draws):
            if player=='d':
                ...
                latent,_=self.prior.sample(mode_hold.BATCH,generator=self.stream)
                fake=self.g(latent)
                if self.gan.mode in ('rp', 'ra'):
                    real=self.sample_real()
                    loss=self.gan.g_loss(self.d(fake),self.d(real))
                else:
                    loss=self.gan.g_loss(self.d(fake))
            if draws==1:
                optimizer.zero_grad()
...
def run(variant, output, steps=200, full_window=False, *, convergence_gate=None):
    verify_sources()
    ...
    if convergence_gate is not None:
        assert convergence_gate.start_step == state['step'] == 1200
        assert steps == convergence_gate.settling_budget + convergence_gate.hold_budget
        declaration.update(convergence_protocol=convergence_gate.declaration(),
                           stop_on_first_failure=False,
                           scope='convergence-aware diagnostic; original toy verdicts unchanged',
                           stop_rule='allow settling failures; first confirmed convergence arms a disjoint hold')
        from convergence_gate import __file__ as gate_source
        declaration['convergence_gate_sha256'] = sha(Path(gate_source))
        (output/'convergence_gate.py').write_bytes(Path(gate_source).read_bytes())
    ...
    with signal_policy(options) as receipt:
        ...
        def save_state(path, step, samples):
            torch.save(dict(step=step,task='mode_hold',torch_rng=torch.get_rng_state(),
                            ...
                            ema_g=ema_g,ema_z=ema_z,samples=samples),path)
        ...
        with (output/'metrics.jsonl').open('w',buffering=1) as metrics_log:
            for completed in range(1200,1200+steps):
                ...
                if (completed+1)%25==0 or (not passing and convergence_gate is None):
                ...
                if convergence_gate is not None:
                    done = convergence_gate.observe(point)
                    if convergence_gate.converged_step == point['step']:
                        save_state(output/'converged-state.pt',point['step'],draw)
                        print_json(dict(event='CONVERGED', **convergence_gate.summary()))
                    if done:
                        break
                elif not passing and not full_window:
                    break
        final_step=points[-1]['step']
        save_state(output/'final-state.pt',final_step,draw)
    ...
    if convergence_gate is not None:
        result.update(status=convergence_gate.status, convergence=convergence_gate.summary(),
                      gate='post_convergence_diagnostic',
                      hold_budget_complete=convergence_gate.summary()['hold_budget_complete'],
                      frozen_toy_verdicts_unchanged=True)
