...
def main():
    ...
    candidate = (args.candidate or root / selection['cold_candidate']).resolve()
    ...
    hashes = sources()
    for path in (Path(__file__), Path(runner.__file__), Path(__file__).with_name('convergence_gate.py')):
