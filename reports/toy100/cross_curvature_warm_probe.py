...
def main():
    ...
    declaration=dict(methods=['identity','constant','cross_curvature'],prefix_steps=1000,total_steps=1200,
        ...
        solver=dict(krylov_dim=8,linear_tolerance=.1,nonlinear_tolerance=None,curvature_bound=args.curvature_bound,advantage_gate=args.advantage_gate,amplification_bound=args.amplification_bound,explicit=args.explicit,fd_relative=1e-4,
    ...
    result=run_warm_variants(config,variants(),output_dir=args.output,
        prefix_context=lambda:cross_curvature(start_step=1000,amplification_bound=args.amplification_bound,explicit=args.explicit,curvature_bound=args.curvature_bound,advantage_gate=args.advantage_gate))
