"""Fail-fast gates for PR84 plus one reverse-KL/chi-squared prior pullback."""
...
from reports.toy100.chi_mixture_candidate import METHOD, chi_mixture_candidate


def _declaration(output, phase):
    names = (
        "reports/toy100/chi_mixture_candidate.py",
        "reports/toy100/chi_mixture_pullback.py",
        "reports/toy100/chi_mixture_probe.py",
    ...
    row = dict(phase=phase, seed=0, shared_gate_eligible=False,
               ...
               operator="Frozen PR84 GAN update then prior-only reverse-KL plus chi-squared KDE pullback",
               objective_addition="KL(p_kde||q_kde) + chi^2(q_kde||p_kde), weights 1, 1",
               paper="https://arxiv.org/abs/2603.10592",
               bandwidth="max real-to-nearest-particle distance, clamped to [0.07, 3]",
               step_cap=0.1, latent_cap=1.0,
               conditional_policy="unchanged PR84 path",
               native_real_batch=128,
               nominal_rates=dict(g=0.00425, d=0.00425, prior=0.0085),
               update_order="D then bounded G, followed by a mixture prior projection before EMA",
               source=source)
...
def warm(output):
    ...
    def variant(method, state, prefix):
        ...
        recorder, _ = prefix
        recorder.enabled = method in ("original", "chi_mixture")
        recorder.correction = method == "chi_mixture"
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "original", "chi_mixture")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: chi_mixture_candidate(start_step=1000))
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="chi_mixture_candidate", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with chi_mixture_candidate(task=task) as (recorder, source):
...
def main():
    ...
    if args.phase == "cold":
        ...
        if (not previous["identity_cold_parity"] or not previous.get("original_control_exact_parity")
                or previous["variants"]["chi_mixture"]["status"] != "PASS"
                or not previous["variants"]["chi_mixture"]["local_stability"]["pass_all"]
                or previous["variants"]["chi_mixture"]["local_stability"]["checks"] != 200):
