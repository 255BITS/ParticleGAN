...
from reports.toy100.chamfer_smoothed_candidate import chamfer_smoothed_candidate, METHOD


def _declaration(output, phase):
    names = (
        "reports/toy100/chamfer_smoothed_candidate.py",
        "reports/toy100/coverage_smoothed_candidate.py",
        "reports/toy100/chamfer_projection_probe.py",
...
def warm(output):
    ...
    def variant(method, state, prefix):
        ...
        recorder, _ = prefix
        recorder.enabled = method in ("original", "chamfer")
        recorder.correction = method == "chamfer"
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "original", "chamfer")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: chamfer_smoothed_candidate(start_step=1000))
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="chamfer_smoothed_candidate", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with chamfer_smoothed_candidate(task=task) as (recorder, source):
...
def main():
    ...
    if args.phase == "cold":
        ...
        if (not previous["identity_cold_parity"] or not previous.get("original_control_exact_parity")
                or previous["variants"]["chamfer"]["status"] != "PASS"
                or not previous["variants"]["chamfer"]["local_stability"]["pass_all"]
                or previous["variants"]["chamfer"]["local_stability"]["checks"] != 200):
