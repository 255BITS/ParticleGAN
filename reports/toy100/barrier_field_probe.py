"""Fail-fast gates for the PR84 barrier-field candidate, plus a PR84 control."""
...
from reports.toy100.pr84_barrier_field_candidate import METHOD, pr84_barrier_field_candidate
from reports.toy100.pr84_smoothed_candidate import pr84_smoothed_candidate


FACTORIES = {
    "baseline": pr84_smoothed_candidate,
    "barrier": pr84_barrier_field_candidate,
}


def _declaration(output, phase, method):
    names = (
        "reports/toy100/pr84_barrier_field_candidate.py",
        "reports/toy100/barrier_field_probe.py",
    ...
    row = dict(phase=phase, method_name=method, seed=0, shared_gate_eligible=False,
               scratch_optimizer_policy=METHOD if method == "barrier" else "pr84_smoothed",
               hypothesis=("G follows the sharp critic two or four stencil widths "
                           "opposite its local gradient when that probe beats the "
                           "five-point stencil; otherwise the score is PR84."),
               purity="no coverage, Chamfer, mode-count, assignment, or clip ladder",
               host="neural", probe_scales=[2.0, 4.0],
...
def cold(output, method, tasks):
    ...
    factory = FACTORIES[method]
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name=f"{method}_barrier_field_screen", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in tasks:
        ...
        try:
            ...
            if hasattr(recorder, "records"):
                uses = [r.get("barrier_uses", 0) for r in recorder.records]
                row["barrier_uses_total"] = int(sum(uses))
                row["barrier_steps_fired"] = int(sum(v > 0 for v in uses))
            stages.append(row)
