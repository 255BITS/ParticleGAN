...
OUT = W / 'out' / VARIANT; (OUT / 'logs').mkdir(parents=True, exist_ok=True)
...
T0 = time.time()
def run(x):
    v, off, kind, t, init_only = x
    o = OUT / v / f'{kind}-{t}'
    done = o / ('ortho-init.json' if init_only else 'result.json')
    if done.exists(): return
    if time.time() > DEADLINE - 60:
        print(json.dumps(dict(skip=f'{v}/{kind}-{t}')), flush=True); return
    subprocess.run(['rm', '-rf', str(o)]); o.parent.mkdir(parents=True, exist_ok=True); s = time.time()
    e = dict(env, K3P_SEED_OFFSET=str(off), K3P_INIT_ONLY='1' if init_only else '0')
    ...
    print(json.dumps(dict(done=f'{v}/{kind}-{t}', rc=rc, seconds=round(time.time() - s, 1), t=round(time.time() - T0))), flush=True)
with ThreadPoolExecutor(JOBS) as ex: list(ex.map(run, tasks))
print(json.dumps(dict(event='PLAN_DONE', plan=PLAN, variant=VARIANT, wall=round(time.time() - T0))), flush=True)
