...
def main():
    ...
    parser.add_argument('--plain-curvature',action='store_true');args=parser.parse_args()
    ...
    for task in declaration['task_order']:
        ...
        try:
            with alternating_curvature(task=task,curvature_bound=args.curvature_bound,**({'bound_d':True,'d_curvature_bound':args.d_curvature_bound,'ratio_reference':args.ratio_reference,'per_group':args.per_group,'per_particle':args.per_particle,'critic_average':args.critic_average,'smooth_critic':args.smooth_critic,'stencil_critic':args.stencil_critic,'plain_curvature':args.plain_curvature} if args.bound_d else {'advantage_gate':args.advantage_gate})) as (recorder,source):
