"""Cold acquisition gate for the one declared implicit response."""
import argparse
import hashlib
import json
...
def main():
    ...
    config.update(name='implicit_response',lr_floor=1.,lr_anneal_start=0.)
    ...
    declaration=dict(seed=0,task_order=['trajectory','mode_hold'],budgets={'mode_hold':1200,'trajectory':400},
        stop_at_first_failed_host=True,shared_gate_eligible=False,scratch_optimizer_policy='same_sample_linearized_implicit_response',
        ...
        source={name:hashlib.sha256((ROOT/'reports/toy100'/name).read_bytes()).hexdigest() for name in
                ('implicit_extra_scratch.py','fixed_metric_extra_scratch.py','extra_adam_scratch.py','implicit_extra_probe.py')})
    ...
    for task in declaration['task_order']:
        ...
        try:
            with implicit_extra(task=task) as (recorder,source):
            ...
            data=dict(result=result,applied=context['applied'],noise=context['noise_receipt'],
                ...
                scratch_optimizer_policy='same_sample_linearized_implicit_response')
