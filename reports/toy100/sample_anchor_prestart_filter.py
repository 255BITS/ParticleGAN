...
from reports.toy100.sample_anchor_prestart_candidate import METHOD, sample_anchor_prestart_candidate

SOURCES=('reports/toy100/sample_anchor_prestart_filter.py','reports/toy100/sample_anchor_prestart_candidate.py',
...
def main():
    ...
    declaration=dict(method=METHOD,scope='saved-state rejection filter only',
        ...
        update='unchanged native PR84 followed by sample-group distinct anchors, joint G/prior fitting and whole-map anchor-objective acceptance',
        grouping='native real128, largest additive MST gap; no configured group count',
    ...
    def selected(*,task,prediction):
        with sample_anchor_prestart_candidate(task=task,correction=prediction) as value:
            yield value
