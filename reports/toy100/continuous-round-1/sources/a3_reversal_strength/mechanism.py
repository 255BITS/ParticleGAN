...
import stationary_policy  # Explicit horizon-blind adapters; same constant values as A1.
...
receipt = {'calls': 0, 'dimensions': {}, 'extra_critic_forwards': 0, 'rule': 'k3', 'floor': FLOOR,
           ...
           'critic_parameters': None, 'lr_max': None, 'lr_last': None, 's_trace': [], 'rule': 'a3_reversal_strength', 'anchor_decay': ANCHOR_DECAY,
...
def handover_weight():
    return 0.0  # Stationary caps plus active EMA anchor, independent of rates.
...
def anchored_gradient_gap(D, x_real, g, dimension):
    ...
    try:
        ...
        for p, v in zip(params, saved):
            p.data = v
    gap = (g - gb).pow(2).flatten(1).sum(dim=1).mean() / dimension
    if receipt['calls'] % 50 == 0:
        receipt.setdefault('anchor_trace', []).append(dict(call=receipt['calls'], gap=float(gap.detach()), real_gradient_rms=float(g.detach().square().mean().sqrt()), anchor_gradient_rms=float(gb.square().mean().sqrt()), gradient_anchor_cosine=float(F.cosine_similarity(g.detach().flatten(),gb.flatten(),dim=0))))
    return gap * _state.get('restoring_strength', 1.0)
...
def _guard(opt, args, kwargs):
    ...
    if _state['depth'] != 1:
        return
    # Raw critic-gradient reversals set the next call's anchor strength in [1,2].
    # Keep the original .999 critic EMA memory, rates and all optimizer rules.
    with torch.no_grad():
        current = torch.cat([p.grad.detach().flatten() for group in opt.param_groups for p in group['params'] if p.grad is not None])
        previous = _state.get('gradient_previous')
        cosine = current.new_zeros(()) if previous is None else F.cosine_similarity(current, previous, dim=0, eps=1e-30)
        _state['restoring_strength'] = 1.0 + (-cosine).relu().clamp_max(1.0)
        _state['gradient_previous'] = current.clone()
        if receipt['calls'] % 50 == 0:
            receipt.setdefault('memory_trace', []).append(dict(call=receipt['calls'],coherence=float(cosine),ema_alpha=1.0-ANCHOR_DECAY,restoring_strength=float(_state['restoring_strength'])))
    flags = []
