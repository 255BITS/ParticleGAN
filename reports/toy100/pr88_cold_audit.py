"""Exact-head PR88 cold acquisition and same-process PR84 controls.

The submitted adapter is not edited. Conditional trajectory is the native
PR84 path. No hold, assignment-round, Gauss-Newton, coefficient or seed sweep.
"""
import argparse
...
HEAD = '7d7676c37ebd390836aa957ecb085367f1e88a35'
...
def main():
    ...
    if subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=code, text=True).strip() != HEAD:
        raise RuntimeError('wrong PR88 checkout')
    ...
    config.update(name='pr88_exact_cold_independent_audit', lr_floor=1., lr_anneal_start=0.)
    ...
    declaration = dict(method=METHOD, head=HEAD, shared_gate_eligible=False,
        ...
        submitter_environment='reported torch2.13.0+cu126; author noted different warm hashes despite same version string',
        tasks=['trajectory','mode_hold'], budgets={task:specs[task]['steps'] for task in ('trajectory','mode_hold')},
        variants=['pr84','pr88_disabled','pr88'],
        scope='cold acquisition only; PR88 source/constants unchanged; no warm or hold rerun',
        setup_failure='preserve errors separately from quality verdicts; no compatibility repair',
    ...
    (args.output/'source/pr88_cold_audit.py').write_bytes(Path(__file__).read_bytes())
        ...
        for variant in declaration['variants']:
            ...
            try:
                context = (pr84_smoothed_candidate(task=task) if variant=='pr84'
                           else refreshed_chamfer_candidate(task=task))
                with context as (recorder, generated):
                    if variant != 'pr84':
                        recorder.correction = variant == 'pr88'
                ...
                value = dict(task=task, variant=variant, result=result, verdict=verdict,
                    ...
                    additional_prior_projection='PR88 prior-output Gauss-Newton Jacobians; additional to inherited3-field receipt',
                ...
                stages.append(dict(task=task, variant=variant, verdict=verdict, live=result['live'],
                    elapsed_seconds=value['elapsed_seconds'], corrections=value['dynamics'].get('chamfer_accepted',0),
                    state_sha256=value['final_state_sha256']))
                if variant=='pr88_disabled':
                    ...
                    if not all(checks.values()):
                        raise AssertionError(f'disabled PR88 differs from PR84: {checks}')
    ...
    for name in ('reports/toy100/chamfer_refreshed_candidate.py','configs/toy100/constraints_simple_regularization.json'):
