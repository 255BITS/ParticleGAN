"""Pin PR84, then one mild anti-restoring stay rule. Fail-fast. No merge.
...
def run_warm(output):
    ...
    def activate(method, state, prefix):
        ...
        _tail("VARIANT_START", variant=method, completed=completed, mild=recorder.mild)
        if method == "identity":
            yield dict(method=method, shared_gate_eligible=False)
        else:
            with constant_rate_context(state) as rates:
                receipt = dict(method=method, shared_gate_eligible=False, mild=recorder.mild)
    ...
    factories = {name: (lambda state, prefix, name=name: activate(name, state, prefix))
                 for name in ("identity", "pr84", "stay")}
    run_warm_variants(
        config, factories, output_dir=output / "warm",
        steps=STAY_UNTIL, prefix_context=lambda: pr84_antirestoring_stay(start_step=1000, mild=False))
    summary = json.loads((output / "warm" / "summary.json").read_text())
    rows = {}
    for name in ("pr84", "stay"):
        ...
        rows[name] = dict(warm=_eight(warm), stay=_eight(stay),
                          ...
                          mild_updates=evidence.get("dynamics_receipt", {}).get("mild_updates"))
...
def run_cold(output):
    ...
    torch.set_num_threads(1)
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="antirestoring_stay_cold", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        for variant, factory in (
            ...
            ("stay", lambda **kw: pr84_antirestoring_stay(mild=True, **kw)),
        ):
            ...
            row = dict(task=task, variant=variant, verdict=verdict.get("passed"),
                       ...
                       mild_updates=recorder.receipt().get("mild_updates"))
            rows[f"{task}:{variant}"] = row
            _tail("COLD_DONE", **row)
            if variant == "stay" and task == "trajectory" and rows.get("trajectory:pr84", {}).get("verdict") and not row["verdict"]:
...
def main():
    ...
    if warm is not None:
        worse = warm["stay"]["warm"]["ge8"] < warm["pr84"]["warm"]["ge8"]
        decision["warm"] = warm
