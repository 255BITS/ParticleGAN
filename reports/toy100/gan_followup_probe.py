...
FACTORIES = {
    ...
    "reachstall_rollback": ("reports.toy100.pr84_reach_candidate", "pr84_reach_candidate",
                            dict(ramp="stall", trust_rollback=True)),
}
SOURCES = (
...
def warm(output, method):
    ...
    def variant(name, state, prefix):
        ...
        recorder.accounting = lambda calls, outer: state["declare_optimizer_accounting"](
            ...
            moment_lag=len(getattr(recorder, "rollback_steps", ())))
...
def stay(output, method, steps):
    ...
    records = [dict(step=r["outer_step"], sharp=r.get("critic_sharpness"), width=r.get("critic_width"),
                    ...
                    rollback=bool(r["g"].get("rolled_back")),
                    trust_pre=r["g"].get("trust_pre"), trust_post=r["g"].get("trust_post"))
