...
Scoring refuses unless ``scripts/toy100_env.sh`` is active. That check is
``benchmarks.toy100.canonical_env.require_canonical_env`` and runs before any
...
def _stamp(row):
    from benchmarks.toy100.canonical_env import harness_receipt
    row["harness"] = harness_receipt()
    return row
...
def stay(output, method, steps):
    ...
    payload = dict(summary=row, diagnostic=diag, records=records)
    _stamp(payload)
    (output / "stay.json").write_text(json.dumps(payload, default=float) + "\n")
    emit(**row)
...
def main():
    ...
    from benchmarks.toy100.canonical_env import require_canonical_env
