"""Cold acquisition gate for the one declared cross-only competitive response."""
import argparse
import hashlib
import json
...
def main():
    ...
    declaration=dict(seed=0,task_order=['trajectory','mode_hold'],budgets={'mode_hold':1200,'trajectory':400},
        stop_at_first_failed_host=True,shared_gate_eligible=False,scratch_optimizer_policy='same_sample_cross_only_competitive_response',
        solver=dict(krylov_dim=8,linear_tolerance=.1,nonlinear_tolerance=.5,fd_relative=1e-4,correction_limit=2.,max_backtracks=8),
        source={name:hashlib.sha256((ROOT/'reports/toy100'/name).read_bytes()).hexdigest() for name in
                ('cross_competitive_scratch.py','implicit_extra_scratch.py','fixed_metric_extra_scratch.py','extra_adam_scratch.py','cross_competitive_probe.py')})
    ...
    for task in declaration['task_order']:
        ...
        try:
            with cross_competitive(task=task,matched_output_rms_limit=args.matched_output_rms,
                                    error_relative_gain=args.error_relative_gain) as (recorder,source):
