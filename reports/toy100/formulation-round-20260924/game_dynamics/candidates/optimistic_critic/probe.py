...
from mechanism import Correction
...
def step(opt, *args, **kwargs):
    ...
    item['calls'] += 1
    proof['adam_calls'] += 1
    result = correction.step(opt, original_step, *args, **kwargs)
    if item["calls"] % 250 == 0:
        print(json.dumps(dict(event="progress", optimizer=list(proof["optimizers"]).index(str(id(opt))), updates=item["calls"])), flush=True)
    if item['calls'] == 1:
...
try:
    ...
    with correction.context(), audit, patch.object(torch.optim.Adam, '__init__', init), patch.object(torch.optim.Adam, 'step', step):
...
proof["game_correction"] = correction.receipt()
if record.get('status') not in ('ERROR', 'INITIALIZATION_CAPTURED'):
    correction.snapshot(a.output / 'final-state.pt', audit)
    record['final_state_sha256'] = hashlib.sha256((a.output / 'final-state.pt').read_bytes()).hexdigest()
record.update(seconds=time.perf_counter()-started, proof=proof,
