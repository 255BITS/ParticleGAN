...
RUNS=[sorted(Path(row['directory']).glob('20*'))[-1] for row in RECORDS if row['lane']=='ra_r1_fake_cap']
...
def worker(run):
    ...
    try:
        ...
        for task in tasks:
            ...
            expected_spec=dict(baseline['spec'])
        ...
        (run/'regression-audit.json').write_text(json.dumps(dict(status='PASS',gates=4,source_files=1614,code=True,frozen_verdict=True,initialization=True,randomness=True,cuda_updates=True),indent=2)+'\n')
...
(ROOT/'regression-summary.json').write_text(json.dumps(rows,indent=2)+'\n')
