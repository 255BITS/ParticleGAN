...
RUNS=[sorted(Path(row['directory']).glob('20*'))[-1] for row in RECORDS if row['lane']=='rp_r1_fake_cap']
...
(ROOT/'rp-regression-summary.json').write_text(json.dumps(rows,indent=2)+'\n')
