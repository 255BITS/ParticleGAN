"""Particle-only Adam with a full gradient covariance shared over table rows."""
import json
import torch
    ...
    def step(self, optimizer, original_step, *args, **kwargs):
        ...
        for p, gradient, group in pending:
            ...
            if not state:
                ...
                state['exp_avg_sq'] = torch.zeros((p.shape[1], p.shape[1]), device=p.device, dtype=p.dtype)
            beta1, beta2 = group['betas']
            state['step'].add_(1)
            n = self.counts.get(id(p), 0) + 1
            self.counts[id(p)] = n
            state['exp_avg'].lerp_(gradient, 1 - beta1)
            pooled = gradient.T @ gradient / len(p)
            state['exp_avg_sq'].lerp_(pooled, 1 - beta2)
            covariance = state['exp_avg_sq'] / (1 - beta2 ** n)
            isotropic = torch.trace(covariance) / p.shape[1]
            metric = (covariance + isotropic * torch.eye(p.shape[1], device=p.device)) * 0.5
            eigenvalues, eigenvectors = torch.linalg.eigh(metric)
            inverse_scale = eigenvalues.clamp_min(0).sqrt().add_(group['eps']).reciprocal()
            inverse_root = (eigenvectors * inverse_scale[None, :]) @ eigenvectors.T
            displacement = -group['lr'] / (1 - beta1 ** n) * (state['exp_avg'] @ inverse_root)
            p.add_(displacement)
            if n == 1 or n % 200 == 0:
