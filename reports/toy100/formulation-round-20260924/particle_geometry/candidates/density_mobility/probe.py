...
def prior_sample(self, *args, **kwargs):
    latent, indices = original_prior_sample(self, *args, **kwargs)
    update.sampled(self, latent, indices)
    return latent, indices
ParticlePrior.sample = prior_sample
original_prior_forward = ParticlePrior.forward
def prior_forward(self, indices):
    latent = original_prior_forward(self, indices)
    update.sampled(self, latent, indices.to(self.z.device))
    return latent
ParticlePrior.forward = prior_forward
...
def step(opt, *args, **kwargs):
    ...
    item['calls'] += 1
    proof['adam_calls'] += 1
    result = update.step(opt, original_step, *args, **kwargs)
    if item['calls'] == 1:
...
try:
    ...
    # Capture own state only; measurement and training hosts remain untouched.
    from benchmarks.locked_shared import mode_hold as ring_host
    original_checkpoint = ring_host.checkpoint
    def capture_checkpoint(completed, measure):
        original_checkpoint(completed, measure)
        if a.task == 'mode_hold' and completed == spec['steps']:
            import inspect
            frame = inspect.currentframe().f_back
            while frame and frame.f_code.co_name != 'train_mode_hold':
                frame = frame.f_back
            values = frame.f_locals
            policy = values['noise_policy']
            torch.save(dict(completed_steps=completed,
                models={k: values[k].state_dict() for k in ('generator', 'critic', 'prior')},
                optimizers={k: values[k].state_dict() for k in ('opt_g', 'opt_d')},
                ema_g=values['ema_g'], ema_z=values['ema_z'],
                cpu_rng=torch.get_rng_state(), cuda_rng=torch.cuda.get_rng_state(),
                stream_rng=values['stream'].get_state(), noise_input_rng=policy.input_stream.get_state(),
                base_lrs=values['base_lrs'], particle_update_counts=update.receipt()['updates']),
                a.output / 'own-final-state.pt')
    ring_host.checkpoint = capture_checkpoint
    with audit, patch.object(torch.optim.Adam, '__init__', init), patch.object(torch.optim.Adam, 'step', step):
        if spec['runner'] == 'vector':
            result, context = run_vector(spec, card, recipe, noise, model_policy=declared_model_policy(config))
        elif spec['runner'] == 'image':
            result, context = run_image(spec, recipe, noise, model_policy=declared_model_policy(config))
        else:
            result, _ = run_legacy(spec, recipe, noise, model_policy=declared_model_policy(config))
    if spec['runner'] in ('vector', 'image'):
        torch.save(dict(trainer=context['trainer'].state_dict(),
                        data_rng=context['data_rng'].get_state(),
                        particle_update_counts=update.receipt()['updates']), a.output / 'own-final-state.pt')
    if result.get('error'):
...
record.update(particle_update=update.receipt(), gpu=torch.cuda.get_device_name(0), deterministic=torch.are_deterministic_algorithms_enabled(), tf32=torch.backends.cuda.matmul.allow_tf32, seconds=time.perf_counter()-started, proof=proof,
