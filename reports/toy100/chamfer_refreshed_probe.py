"""Fail-fast gates for PR84 plus a unit-mean bidirectional Chamfer prior projection."""
...
from reports.toy100.chamfer_refreshed_candidate import refreshed_chamfer_candidate, METHOD


def _declaration(output, phase):
    names = (
        "reports/toy100/chamfer_refreshed_candidate.py",
        "reports/toy100/coverage_smoothed_candidate.py",
        "reports/toy100/chamfer_refreshed_probe.py",
        ...
        "reports/toy100/chamfer_refreshed_pullback.py",
    ...
    row = dict(phase=phase, seed=0, shared_gate_eligible=False,
               ...
               operator="Frozen PR84 GAN update then prior-only symmetric Chamfer surrogate target pullback",
               objective_addition="mean_real min_particle squared distance + mean_particle min_real squared distance",
...
def warm(output):
    ...
    def variant(method, state, prefix):
        ...
        recorder, _ = prefix
        recorder.enabled = method in ("original", "refreshed")
        recorder.correction = method == "refreshed"
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "original", "refreshed")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: refreshed_chamfer_candidate(start_step=1000))
    ...
    for key in ("warm_state_sha256", "final_state_sha256", "observations", "diagnostic"):
        archive_parity[key] = untimed(observed[key]) == untimed(expected[key])
    compact = dict(method=METHOD,
                   source=json.loads((output / "declaration.json").read_text())["source"],
                   original_control_exact_parity=all(archive_parity.values()),
                   archived_pr84_warm_parity=archive_parity,
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="refreshed_chamfer_candidate", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with refreshed_chamfer_candidate(task=task) as (recorder, source):
...
def continuation(output):
    """Same warm state, uninterrupted checks through update 2400. No cold host."""
    from benchmarks.toy100.warm_equilibrium_probe import constant_rate_context, run_warm_variants
    ...
    def variant(method, state, prefix):
        print(json.dumps(dict(event="VARIANT_START", variant=method,
                              completed_steps=state["completed_steps"])), flush=True)
        recorder, _ = prefix
        recorder.enabled = method == "refreshed"
        recorder.correction = method == "refreshed"
        completed, target = state["completed_steps"], state["target_steps"]
        recorder.accounting = lambda calls, outer: state["declare_optimizer_accounting"](
            calls=completed + calls + (target - completed - outer),
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "refreshed")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    ...
    (output / "summary.json").write_text(json.dumps(dict(
        method=METHOD, steps=2400, identity_cold_parity=result["identity_cold_parity"],
        variants={k: dict(status=v["status"], local_stability=v["local_stability"],
                          long_hold=v["long_hold"], final=v["final"])
                  for k, v in result["variants"].items()}), indent=2) + "\n")
    print(json.dumps(dict(event="CONTINUATION_DONE", **{
        k: result["variants"][k]["status"] for k in result["variants"]})), flush=True)
...
def hold(output):
    ...
    torch.set_num_threads(1)
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    with refreshed_chamfer_candidate(task="mode_hold") as (recorder, _source):
        def hook(state):
            recorder.accounting = lambda calls, outer: state["declare_optimizer_accounting"](
    ...
    compact = dict(status=result["status"], final=result["final"],
                   stationary=result["stationary"], continued_hold=result["continued_hold"],
                   seconds=result["seconds"])
    (output / "hold.json").write_text(json.dumps(result, allow_nan=False) + "\n")
    (output / "summary.json").write_text(json.dumps(compact, indent=2) + "\n")
...
def main():
    ...
    parser.add_argument("--phase", choices=("warm", "continuation", "cold", "hold"), required=True)
    ...
    if args.phase == "cold":
        ...
        if (not previous["identity_cold_parity"]
                or previous["variants"]["refreshed"]["status"] != "PASS"
                or not previous["variants"]["refreshed"]["local_stability"]["pass_all"]
                or previous["variants"]["refreshed"]["local_stability"]["checks"] != 200):
            raise RuntimeError("warm gate did not pass; cold acquisition forbidden")
    if args.phase == "hold":
        if args.previous is None:
            raise ValueError("hold requires a passing cold status")
        previous = json.loads(args.previous.read_text())
        stages = previous.get("stages", [])
        if len(stages) != 2 or any(not row.get("verdict", {}).get("passed") for row in stages):
            raise RuntimeError("both cold hosts must pass before the extended hold")
