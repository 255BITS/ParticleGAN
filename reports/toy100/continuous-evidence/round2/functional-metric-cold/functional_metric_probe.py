"""Reproducible functional-metric warm filter and fail-fast cold acquisition."""
import argparse
...
def warm(state, prefix, **options):
    with constant_rate_context(state, lr=.00425) as rate, functional_metric(state=state, **options) as receipt:
...
def main():
    ...
    sources = {}
    for name in ("reports/toy100/functional_metric_scratch.py", "reports/toy100/functional_metric_probe.py",
    ...
    if args.cold_from is None:
        ...
        for row in rows[:1]:
            ...
            for task in ("trajectory", "mode_hold"):
                ...
                with functional_metric(output_step=row["output_step"]) as receipt:
                ...
                data = dict(evidence=evidence, context=context, verdict=verdict, receipt=receipt,
