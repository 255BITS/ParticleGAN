...
def archive(output,phase):
    ...
    files=[Path(__file__),ROOT/'reports/toy100/alternating_linesearch_scratch.py',
        ...
        ROOT/'benchmarks/toy100/continuous_probe.py',ROOT/'configs/toy100/constraints_simple_regularization.json']
    ...
    declaration=dict(method=METHOD,phase=phase,options=OPTIONS,seed=0,shared_gate_eligible=False,
        ...
        candidate_count=1,change='Preserve PR82 G curvature rule; replace only D bound with verified Armijo descent',
...
def variants():
    ...
    def factory(method):
        ...
        def activate(state,prefix):
            recorder,_=prefix;recorder.enabled=method=='d_linesearch'
        ...
        return activate
    return {name:factory(name) for name in ('identity','constant','d_linesearch')}
...
def main():
    ...
    if args.phase=='cold':
        ...
        if not prior['identity_cold_parity'] or prior['variants']['d_linesearch']['status']!='PASS':
    ...
    for task in ('trajectory','mode_hold'):
        ...
        try:
            ...
            raw=dict(error=repr(error),traceback=traceback.format_exc(),receipt=recorder.receipt(),
                partial_step=recorder.row,shared_gate_eligible=False)
