...
def main():
    ...
    parser.add_argument('--amplification-bound',type=float);args=parser.parse_args()
    ...
    declaration=dict(seed=0,task_order=['trajectory','mode_hold'],budgets={'mode_hold':1200,'trajectory':400},
        ...
        solver=dict(krylov_dim=8,linear_tolerance=.1,nonlinear_tolerance=None,curvature_bound=1.,amplification_bound=args.amplification_bound,fd_relative=1e-4,correction_limit=2.,max_backtracks=8),
    ...
    for task in declaration['task_order']:
        ...
        try:
            with cross_curvature(task=task,amplification_bound=args.amplification_bound) as (recorder,source):
