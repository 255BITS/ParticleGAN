...
class ProjectedSkewRecorder(ImplicitExtraRecorder):
    ...
    def _cross_product(self,direction,label,q0):
        ...
        diagnostics=[];image=torch.zeros_like(direction)
        for role,section,other in (('d',slice(None,n_d),slice(n_d,None)),
                                   ('g',slice(n_d,None),slice(None,n_d))):
            physical=self.root_metric[section]*direction[section]
            if not torch.count_nonzero(physical):continue
            # Each cross block gets a representable perturbation. A common
            # epsilon can erase the smaller player's movement in float32.
            desired=self.fd_relative*(1+float(torch.linalg.vector_norm(self.base_flat[section])))
            ...
            for sign in (1.,-1.):
                ...
                values.append(field[other])
            image[other]=(values[0]-values[1])/(2*epsilon)
    ...
    def receipt(self):
        ...
        value.update(method='rank_two_projected_implicit_skew_response',
            ...
            fd_relative=self.fd_relative,fd_scaling='independent epsilon for each perturbed player block',
            correction_enabled=self.correction,skew_steps=self.skew_steps,queries=self.queries,
