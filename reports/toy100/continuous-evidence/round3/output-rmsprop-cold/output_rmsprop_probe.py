"""Reproducible output-space RMSProp warm filter and fail-fast cold acquisition."""
import argparse
...
from reports.toy100.output_rmsprop_scratch import output_rmsprop


@contextmanager
def warm(state, prefix, **options):
    with constant_rate_context(state, lr=.00425) as rate, output_rmsprop(state=state, **options) as receipt:
...
def main():
    ...
    rows = [dict(name="metric", output_step=.00425), dict(name="observed", observe_only=True)]
    from benchmarks.transfer_suite import suite
    archive = suite.snapshot(args.output)
    sources = {}
    for name in ("reports/toy100/functional_metric_scratch.py", "reports/toy100/joint_functional_metric_scratch.py", "reports/toy100/output_rmsprop_scratch.py", "reports/toy100/output_rmsprop_probe.py",
    ...
    if args.cold_from is None:
        ...
        for row in rows[:1]:
            ...
            for task in ("trajectory", "mode_hold"):
                ...
                with output_rmsprop(output_step=row["output_step"]) as receipt:
