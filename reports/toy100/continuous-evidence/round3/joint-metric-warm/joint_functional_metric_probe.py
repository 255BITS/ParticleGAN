"""Reproducible joint-functional-metric warm filter and fail-fast cold acquisition."""
import argparse
...
from reports.toy100.joint_functional_metric_scratch import joint_functional_metric


@contextmanager
def warm(state, prefix, **options):
    with constant_rate_context(state, lr=.00425) as rate, joint_functional_metric(state=state, **options) as receipt:
...
def main():
    ...
    rows = [dict(name="metric", output_step=.029), dict(name="observed", observe_only=True)]
    from benchmarks.transfer_suite import suite
    archive = suite.snapshot(args.output)
    sources = {}
    for name in ("reports/toy100/functional_metric_scratch.py", "reports/toy100/joint_functional_metric_scratch.py", "reports/toy100/joint_functional_metric_probe.py",
    ...
    if args.cold_from is None:
        ...
        for row in rows[:1]:
            ...
            for task in ("trajectory", "mode_hold"):
                ...
                with joint_functional_metric(output_step=row["output_step"]) as receipt:
