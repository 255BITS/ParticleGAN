...
def main():
    ...
    banks = {name:first_native_bank(state) for name,state in
             (('cold1',cold),('warm1324',warm))}
    width, nearest = local_width(banks['cold1'][0])
    ...
    declaration = dict(scope='one native D real bank, two saved clean output clouds; no training',
        ...
        native_real_bank_sha256={name:sha(value[0].contiguous().numpy().tobytes())
                                 for name,value in banks.items()},
        data_rng_after_bank_sha256={name:value[1] for name,value in banks.items()},
    ...
    result = dict(status='INCOMPLETE', declaration=declaration,
                  real_banks={name:value[0].tolist() for name,value in banks.items()},
                  nearest_neighbor_distances=nearest.tolist(),
                  cases={}, global_training_rng_unchanged=None)
    before_rng = torch.random.get_rng_state().clone()
    try:
        for name,state in (('cold1',cold),('warm1324',warm)):
            final, row = optimize(banks[name][0], initial_support(state), width,
