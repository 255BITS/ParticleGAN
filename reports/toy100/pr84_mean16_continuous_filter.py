...
from reports.toy100.pr84_mean16_replay_candidate import METHOD, pr84_mean16_replay_candidate

SOURCES=('reports/toy100/pr84_mean16_continuous_filter.py','reports/toy100/pr84_mean16_replay_candidate.py',
...
def main():
    ...
    declaration=dict(method=METHOD,scope='saved-state rejection filter only',
        ...
        compute_budget='32 useful averaged-field gradient calls per role plus retained original callbacks',
    ...
    def selected(*,task,prediction):
        with pr84_mean16_replay_candidate(task=task,correction=prediction) as value:
            yield value
