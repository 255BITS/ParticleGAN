"""Fail-fast warm and cold gates for alternating positive G own-secant update."""
...
from reports.toy100.alternating_positive_secant_scratch import alternating_positive_secant, METHOD


def _declaration(output, phase):
    names = (
        "reports/toy100/alternating_positive_secant_scratch.py",
        "reports/toy100/alternating_positive_secant_probe.py",
    ...
    row = dict(phase=phase, seed=0, shared_gate_eligible=False,
               ...
               d_curvature_bound=2.0, g_fallback_bound=0.25,
               positive_curvature_rule="rank_one_secant_resolvent",
               nominal_rates=dict(g=0.00425, d=0.00425, prior=0.0085),
               update_order="D bound, then G proposal and positive own-secant correction",
               source=source)
...
def warm(output):
    ...
    def variant(method, state, prefix):
        recorder, _ = prefix
        recorder.enabled = method == "secant"
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "secant")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: alternating_positive_secant(start_step=1000))
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="alternating_positive_secant", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with alternating_positive_secant(task=task) as (recorder, source):
...
def main():
    ...
    if args.phase == "cold":
        ...
        if not previous["identity_cold_parity"] or previous["variants"]["secant"]["status"] != "PASS":
