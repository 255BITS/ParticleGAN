"""Fail-fast gates for frozen D Armijo plus positive own-secant G response."""
...
from reports.toy100.alternating_armijo_secant_scratch import alternating_armijo_secant, METHOD

FROZEN_D_SHA = "8679fc5f00b78c17e20a93ed69404a69ea80c77e3657fa0c3c04f49008630be3"
OPTIONS = dict(curvature_bound=.25, d_armijo=.1, max_retries=12,
               reject_exhausted=True)


def _declaration(output, phase):
    names = (
        "reports/toy100/alternating_armijo_secant_scratch.py",
        "reports/toy100/alternating_armijo_secant_probe.py",
        "reports/toy100/alternating_positive_secant_scratch.py",
        "reports/toy100/alternating_linesearch_scratch.py",
        "reports/toy100/alternating_linesearch_diagnosis.py",
    ...
    if source["reports/toy100/alternating_linesearch_scratch.py"] != FROZEN_D_SHA:
        raise RuntimeError("frozen D Armijo adapter source changed")
    ...
    row = dict(phase=phase, seed=0, shared_gate_eligible=False,
               ...
               d_armijo=.1, max_d_retries=12, reject_d_exhausted=True,
               g_fallback_bound=.25,
               positive_curvature_rule="rank_one_secant_resolvent",
               nominal_rates=dict(g=0.00425, d=0.00425, prior=0.0085),
               update_order="D Armijo accepted point, then G positive own-secant correction",
               source=source)
...
def warm(output):
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "combo")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: alternating_armijo_secant(start_step=1000, **OPTIONS))
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="alternating_armijo_secant", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with alternating_armijo_secant(task=task, **OPTIONS) as (recorder, source):
...
def main():
    ...
    if args.phase == "cold":
        ...
        if not previous["identity_cold_parity"] or previous["variants"]["combo"]["status"] != "PASS":
