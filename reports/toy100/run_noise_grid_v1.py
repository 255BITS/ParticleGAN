"""Run the predeclared 12-row, eight-host shared-noise screen.

The manifest is frozen before training. One row reuses an exact completed
episode; the other eleven are run once with unchanged host seeds, budgets,
...
MANIFEST = ROOT / "reports/toy100/noise-grid-v1-manifest.json"
...
def _verify_frozen(manifest: dict) -> None:
    ...
    if len(manifest["rows"]) != 12 or sum(row["kind"] == "new" for row in manifest["rows"]) != 11:
        raise ValueError("declared table is not nine grid rows plus three controls")
    if len({row["id"] for row in manifest["rows"]}) != 12:
...
def run(*, verify_only: bool = False) -> dict:
    ...
    output = ROOT / "artifacts/toy100-accuracy/compatibility/noise-grid-v1"
    logs = output / "logs"
    logs.mkdir(exist_ok=True)
    results_path = output / "results.json"
    results = {"protocol": "toy100-shared-noise-grid-results-v1",
