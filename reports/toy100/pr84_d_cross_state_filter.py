"""Strict three-branch local gate for one frozen actual-D response candidate.
...
from reports.toy100.pr84_d_cross_response import METHOD, pr84_d_cross_response
...
def main():
    ...
    files = ('reports/toy100/pr84_d_cross_state_filter.py',
             'reports/toy100/pr84_d_cross_response.py',
    ...
    declaration = dict(method=METHOD, scope='saved_state_local_diagnostic_only',
        ...
        live_threshold=dict(modes=8, hq=.9), variants=['original', 'd_cross_response'],
        response_gain=1., response_clipping=False, seed=0, noise_horizon=1200,
        ...
        state_stage='accepted_states field is phase2 before post-G D correction; final_state includes it')
    ...
    def selected_context(*, task, prediction):
        ...
        # selects this response controller, never opponent extrapolation.
        with pr84_d_cross_response(task=task, correction=prediction) as value:
            yield value
    ...
    with patch.object(replay.prediction_module, 'pr84_opponent_prediction', selected_context):
        for start, end in BRANCHES:
            variants = {}
            for name, switch in (('original', 'current'), ('d_cross_response', 'predicted')):
                try:
                    ...
                    (args.output/f'{name}-{start}-{end}.error.json').write_text(json.dumps(
                        dict(status='ERROR', error=repr(error), response_clipping=False))+'\n')
                    raise
                value['opponent'] = name
                ...
                if name == 'original':
                    ...
                    if value['rng_final_sha256'] != variants['original']['rng_final_sha256']:
                        raise RuntimeError('D response advanced the training random streams')
                    if value['points'][0] != variants['original']['points'][0]:
                        raise RuntimeError('post-G D response changed its initial accepted G step')
    ...
    all_pass = all(row['variants']['d_cross_response']['local_gate']['pass_all'] for row in results)
