...
SOURCE = Path('/ml2/hypergan/gan-attempts/batch-20260924T153037Z/critic_signal/20260924T153037Z-1298555/repo')
...
def run(variant, output, full_window=False):
    verify_sources()
    ...
    declaration=dict(candidate=CANDIDATE.name,variant=variant,policy=POLICIES[variant],
                     ...
                     runner_sha256=sha(Path(__file__)),steps=1200,
    ...
    with signal_policy(options) as receipt:
        ...
        with (output/'metrics.jsonl').open('w',buffering=1) as metrics_log:
            for completed in range(1200,2400):
    ...
    passed=len(points)==1200 and window['pass_all']
    result=dict(candidate=CANDIDATE.name,variant=variant,gate='selected_h_warm_stability_probe',
                ...
                full_budget=len(points)==1200,shared_gate_eligible=False)
...
if __name__=='__main__':
    ...
    parser.add_argument('--diagnostic-full-window',action='store_true',
                        help='record all 1200 steps after failures; failure verdict is unchanged')
    args=parser.parse_args()
    run(args.variant,args.output,args.diagnostic_full_window)
