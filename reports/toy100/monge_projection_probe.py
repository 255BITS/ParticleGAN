"""Fail-fast warm and cold gates for alternating sampled-real prior Monge projection."""
...
from reports.toy100.monge_smoothed_candidate import monge_smoothed_candidate, METHOD


def _declaration(output, phase):
    names = (
        "reports/toy100/monge_smoothed_candidate.py",
        "reports/toy100/monge_projection_probe.py",
        ...
        "reports/toy100/monge_pullback.py",
    ...
    row = dict(phase=phase, seed=0, shared_gate_eligible=False,
               ...
               operator="Frozen PR84 GAN update then prior-only capped Monge prior step",
               objective_addition="capped squared distance to a min-cost real-representative bijection",
               conditional_policy="unchanged PR84 path",
               native_real_batch=128, pinv_rtol=1e-6, max_halves=8, output_cap=0.2,
...
def warm(output):
    ...
    def variant(method, state, prefix):
        recorder, _ = prefix
        recorder.enabled = method in ("original", "monge")
        recorder.correction = method == "monge"
    ...
    factories = {name: (lambda state, prefix, name=name: variant(name, state, prefix))
                 for name in ("identity", "constant", "original", "monge")}
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    result = run_warm_variants(config, factories, output_dir=output / "forks",
        prefix_context=lambda: monge_smoothed_candidate(start_step=1000))
...
def cold(output):
    ...
    config = json.loads((ROOT / "configs/toy100/constraints_simple_regularization.json").read_text())
    config.update(name="monge_smoothed_candidate", lr_floor=1., lr_anneal_start=0.)
    ...
    for task in ("trajectory", "mode_hold"):
        ...
        try:
            with monge_smoothed_candidate(task=task) as (recorder, source):
...
def main():
    ...
    if args.phase == "cold":
        ...
        if (not previous["identity_cold_parity"] or not previous.get("original_control_exact_parity")
                or previous["variants"]["monge"]["status"] != "PASS"
                or not previous["variants"]["monge"]["local_stability"]["pass_all"]
                or previous["variants"]["monge"]["local_stability"]["checks"] != 200):
