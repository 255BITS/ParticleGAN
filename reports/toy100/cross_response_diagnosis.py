...
def main():
    ...
    if reference.exists():expected=json.loads(reference.read_text())
    else:
        reference=ROOT/'reports/toy100/continuous-evidence/cross-implicit'/('warm' if args.task=='warm' else 'cold')/(relative+'.gz')
        expected=json.loads(gzip.decompress(reference.read_bytes()))
    observed=json.loads((args.output/relative).read_text())
    if args.task=='warm':
