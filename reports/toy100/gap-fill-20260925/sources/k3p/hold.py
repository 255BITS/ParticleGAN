...
dense, coarse, ema_points = [], [], []
state = dict(stop_after=None)


class GateDone(BaseException):
    pass


def ema_measure(step):
...
def log(event):
    if event.get('event') != 'checkpoint':
        return
    step = event['step']
    point = dict(step=step, modes=event['modes'], hq=event['hq'])
    if step <= gate.start_step:
        coarse.append(point)
        if step % 200 == 0:
            e = ema_measure(step); ema_points.append(e)
            print(json.dumps(dict(step=step, live_modes=point['modes'], live_hq=point['hq'], ema_modes=e['modes'], ema_hq=e['hq'])), flush=True)
        return
    dense.append(point)
    milestone = False
    if not gate.done:
        gate.observe(point)
        milestone = step == gate.converged_step or gate.done
        if gate.done:
            state['stop_after'] = step + a.post_window
    if step % 50 == 0 or milestone:
        e = ema_measure(step); ema_points.append(e)
        print(json.dumps(dict(step=step, live_modes=point['modes'], live_hq=point['hq'], ema_modes=e['modes'], ema_hq=e['hq'],
                              gate=gate.status, hold_checks=gate.hold_checks, streak=gate.streak)), flush=True)
...
try:
    ...
    spec = {**spec, 'steps': a.steps}
    with audit, patch.object(torch.optim.Adam, '__init__', init), patch.object(torch.optim.Adam, 'step', step):
        try:
            cp._run_extended(spec, recipe, noise, config, noise_horizon=1200, diagnostic_every=50,
                             dense_after=gate.start_step, dense_until=a.steps, shift_step=None, shift=(1.0, 0.0),
                             freeze_after_shift=False, log=log)
            stopped = 'budget'
        except GateDone:
            stopped = 'gate_done_plus_post_window'
    summary = gate.summary()
    good = summary['hold_checks'] - (1 if summary['status'] == 'POST_CONVERGENCE_FAIL' else 0)
    after = [p for p in dense if summary['first_hold_failure'] and p['step'] >= summary['first_hold_failure']]
    post = dict(checks=len(after), passing=sum(p['modes'] == 8 and p['hq'] >= .9 for p in after),
                min_modes=min((p['modes'] for p in after), default=None), min_hq=min((p['hq'] for p in after), default=None),
                final=after[-1] if after else None)
    held = [p for p in dense if summary['converged_step'] and summary['converged_step'] < p['step'] <= summary['converged_step'] + summary['hold_checks']]
    record.update(status=summary['status'], gate=summary, good_hold_updates=good, stopped=stopped,
                  hold_window=dict(min_hq=min((p['hq'] for p in held), default=None), min_modes=min((p['modes'] for p in held), default=None)),
                  post_failure_diagnostic=post, dense=dense, coarse=coarse, ema=ema_points,
                  last_live=dense[-1] if dense else None, last_ema=ema_points[-1] if ema_points else None)
except Exception:
    record.update(status='ERROR', error=traceback.format_exc())
record.update(latent_receipt=latent.receipt, response_receipt=response.receipt, mobility=mobility, regularizer_receipt=mechanism.receipt,
...
print(json.dumps({k: record.get(k) for k in ('status', 'good_hold_updates', 'gate', 'hold_window', 'post_failure_diagnostic', 'last_live', 'last_ema', 'seconds', 'error')}), flush=True)
