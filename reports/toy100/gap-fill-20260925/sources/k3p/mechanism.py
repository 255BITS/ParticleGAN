...
def scaled_penalty(self, D, x_real, x_fake, step=1, collect_stats=True, *, ema_critic=None, generator=None):
    # CriticPenalty always passes ema_critic. The K3P penalty has no arm; the
    # legacy a_r1r2 check used to TypeError before unequal mass could step.
    # At s == 1 (constant LR) the body below does not read ema_critic.
    del ema_critic, generator
    if getattr(self, 'arm', 'a_r1r2') != 'a_r1r2' or (self.lazy_k > 1 and step % self.lazy_k != 0):
        return _original_penalty(self, D, x_real, x_fake, step, collect_stats)
    _state['pending'] = True
