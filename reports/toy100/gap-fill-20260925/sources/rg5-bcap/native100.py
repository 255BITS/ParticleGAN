...
p.add_argument('--seed', type=int, default=None, help='declared seed override of config.json seed')
p.add_argument('--stop', type=int, default=0, help='screen: stop the 7000-step schedule after the eval at this step')
a = p.parse_args()
...
try:
    ...
    import benchmarks.toy100.train as _train_module
    _policy = _train_module.step_with_policy
    class ScreenStop(Exception):
        pass
    def _stoppable(trainer, *args, **kwargs):
        if a.stop and trainer.completed_steps >= a.stop:
            raise ScreenStop()
        return _policy(trainer, *args, **kwargs)
    _train_module.step_with_policy = _stoppable
    try:
        ...
        evals = [json.loads(l) for l in open(a.output / 'native' / a.task / 'events.jsonl') if '"eval"' in l]
        evals = [e for e in evals if e.get('event') == 'eval']
        record.update(status='SCREEN', steps=a.stop, screen={f"{e['model']}@{e['step']}": dict(modes=e['metrics']['modes'], hq=e['metrics']['hq']) for e in evals if e['step'] >= 250})
        raise
    ...
    if type(error).__name__ != 'ScreenStop':
        record.update(status='ERROR', error=traceback.format_exc())
if hasattr(latent, 'guard_receipt'):
    rows = latent.guard_receipt.pop('rows')
    with open(a.output / 'guard.jsonl', 'w') as f:
        for row in rows:
            f.write(json.dumps(row) + '\n')
    record['guard_receipt'] = latent.guard_receipt
