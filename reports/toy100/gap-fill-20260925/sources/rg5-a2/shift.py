...
if hasattr(latent, 'guard_receipt'):
    _rows = latent.guard_receipt.pop('rows')
    (a.output / 'guard.json').write_text(json.dumps(latent.guard_receipt, default=str) + '\n')
    with open(a.output / 'guard.jsonl', 'w') as _f:
        for _row in _rows:
