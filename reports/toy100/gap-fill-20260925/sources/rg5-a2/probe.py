...
record.update(latent_receipt=latent.receipt, response_receipt=response.receipt, mobility=mobility, regularizer_receipt=mechanism.receipt, seconds=time.perf_counter()-started, proof=proof,
...
if record['status'] in ('PASS','FAIL'):
    record['checkpoint']=checkpoint.save(a.output / 'final-state.pt',response)
