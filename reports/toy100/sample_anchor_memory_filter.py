"""Strict original 44-check gate for sample-group anchors plus joint fitting."""
...
SOURCES=('reports/toy100/sample_anchor_memory_filter.py','reports/toy100/sample_anchor_memory_candidate.py',
    'reports/toy100/sample_anchor_prestart_candidate.py', 'reports/toy100/sample_group_two_bank_memory.py',
...
def main():
    ...
    declaration=dict(method=METHOD,scope='saved-state rejection filter only; explicitly fresh learner at each window, not a memory resume',
        fit_start='pre-G parameters; same target and numerical budget',
        ...
        update='native PR84 then confirmed-memory anchors fit from pre-G; bootstrap rests G/prior, D and exactly-once Adam retained',
        grouping='two-bank corroboration then fixed reference partition; starts with empty memory at each declared saved-state window',
        output_budget='one active-quadratic distinct-anchor MM step; requires N>K',
        nonconverged_fit='restore pre-G/prior, no native fallback',
        ...
        caveat='additional sampled-data objective; original G curvature bound does not constrain the fit')
    ...
    def selected(*,task,prediction):
        with sample_anchor_memory_candidate(task=task,start_step=start-1,correction=prediction) as value:
            yield value
