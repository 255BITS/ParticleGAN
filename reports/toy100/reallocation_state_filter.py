"""Strict original 44-check gate for global data allocation plus joint fitting."""
...
from reports.toy100.reallocation_smoothed_candidate import METHOD, reallocation_smoothed_candidate

SOURCES=('reports/toy100/reallocation_state_filter.py',
...
def main():
    ...
    declaration=dict(method=METHOD,scope='saved-state rejection filter only',
        ...
        update='unchanged native PR84 followed by data-only global allocation, joint G/prior fitting and whole-map C+Q acceptance',
        discrete_budget='up to N improving donor/sample relocations then one C+Q quadratic target',
    ...
    def selected(*,task,prediction):
        with reallocation_smoothed_candidate(task=task,correction=prediction) as value:
            yield value
