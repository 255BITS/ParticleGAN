"""Source-bound allocation gates: saved44 -> warm200 -> dense1200 -> cold.
...
def require_previous(previous, *, phase, method, factory, sources, filter_sha):
    ...
    if phase == "cold" and (previous["first200_parity"] is not True
...
def compare_first200(old, new):
    ...
    if a["first200_full_state_sha256"] != b["first200_full_state_sha256"]:
        raise RuntimeError("long hold changed first200 models/moments/EMA/RNG/full noise history")
...
def run_continuation(output, declaration, factory, previous_path):
    ...
    def activate(name, state, prefix):
        ...
        def accounting(calls, outer):
            ...
            if outer == 200:
                receipt["first200_full_state_sha256"] = _sha(snapshot(recorder._local))
    ...
    if phase == "hold":
        ...
        compare_first200(old, candidate)
