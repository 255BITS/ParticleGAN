"""Memory-preserving extension of the qualified v2 gate.
...
def bound_sources(filter_sources):
    ...
    names = set(filter_sources) | set(REFERENCES.values()) | {
        ...
        "reports/toy100/allocation_continuous_probe_v2.py",
...
def compare_first200(old, new, *, old_directory, new_directory):
    from reports.toy100.learner_state_envelope import validate
    import torch
    def memory(path, receipt):
        name = receipt['first200_learner_envelope_file']
        raw = (path / name).read_bytes()
        if sha(raw) != receipt['first200_learner_envelope_file_sha256']:
            raise RuntimeError('first200 learner file changed')
        value = torch.load(path / name, weights_only=True)
        validate(value, method=old['dynamics_receipt']['method'], completed_step=1200)
        if value['envelope_sha256'] != receipt['first200_learner_envelope_sha256']:
            raise RuntimeError('first200 complete envelope changed')
        return value
    ma = memory(old_directory, old['dynamics_receipt'])
    mb = memory(new_directory, new['dynamics_receipt'])
    for key in ('learner_sha256', 'source_sha256', 'bank_count', 'last_observed_step'):
        if ma[key] != mb[key]:
            raise RuntimeError(f'first200 learner parity failed: {key}')
...
def save_memory(output, declaration, recorder, host, *, completed_step, label):
    """Save the complete learner separately while retaining existing host audits."""
    import torch
    from reports.toy100.learner_state_envelope import pack, source_fingerprint
    value = pack(host, recorder, method=declaration['method'],
        source_sha256=source_fingerprint(declaration['source']), completed_step=completed_step)
    path = output / f'{label}-learner-envelope.pt'
    torch.save(value, path)
    return dict(learner_envelope_file=path.name,
        learner_envelope_file_sha256=sha(path.read_bytes()),
        learner_envelope_sha256=value['envelope_sha256'],
        learner_state_sha256=value['learner_sha256'])
    ...
    def activate(name, state, prefix):
        ...
        def accounting(calls, outer):
            ...
            if outer == 200:
                ...
                if name == 'candidate':
                    memory = save_memory(output, declaration, recorder, value,
                        completed_step=1200, label=f'{name}-update1200-pre-ema')
                    receipt.update({'first200_' + key: val for key, val in memory.items()})
            if outer % 200 == 0:
        ...
        if name == "identity":
            ...
            if recorder.enabled:
                ...
                if name == 'candidate':
                    receipt.update(save_memory(output, declaration, recorder, value,
                        completed_step=target, label=f'{name}-final'))
...
def run_cold(output, declaration, factory):
    ...
    for task, steps in (("trajectory", 400), ("mode_hold", 1200)):
        ...
        if task == 'mode_hold':
            data.update(save_memory(output, declaration, recorder, value,
                completed_step=steps, label=f'{task}-final'))
...
def main():
    ...
    declaration = dict(method=method, factory=args.factory, phase=args.phase, source=sources,
        ...
        first200_state_comparison="exact learner state plus complete host after only native evaluation-counter reconciliation",
        learner_state_policy="full versioned host+memory envelopes; support IDs/statistics persist through resume",
