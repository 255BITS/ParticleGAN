# CIFAR AE-GAN: 16k duration and 32k count experiments running

Branch `feat/cifar-ae-gan-pretrained-encoder`. Latest user authorized continuing 16k and trying 32k to 40k. Both arms are now launched (2026-09-18): GPU0 16,384 particles from 40k to 80k, pipeline PID270329; GPU1 32,768 particles from original 1,024-particle 10k checkpoint to 40k, pipeline PID270732. Each automatically runs endpoint information/density/coverage diagnostics after training. No subsequent training promotion queued. Current plan and validation: `particle_next/PLAN.md`; process metadata: `particle_next/LAUNCH.json`. Tail `runs/cifar_particle_ae/particle_next/PIPELINE.log`. Historical completion/idle statements below refer to earlier experiments and are superseded by this paragraph. User prioritizes particle-count scaling toward FID50k<13; do not divert to Gaussian-prior comparisons.
