def train_mode_hold(recipe: ModeHoldRecipe | None = None, *, seed: int = 0,
    ...
    stream = torch.Generator(device=torch.get_default_device()).manual_seed(seed)
    means = ring_means()
    ...
    regularizer = None  # paired public penalty is built with opt_d below
    ...
    _matched_capture(generator, critic, prior, stream, noise_policy)
    ...
    def measure(step: int):
        ...
        with context:
            latent, _ = prior.sample(EVAL_N, generator=torch.Generator(device=torch.get_default_device()).manual_seed(seed + 9))
    ...
    for step in range(recipe.steps):
        ...
        _matched_audit_step(opt_d, "d", d_loss)
        ...
        _matched_audit_step(opt_g, "g", g_loss)
        with torch.no_grad():
