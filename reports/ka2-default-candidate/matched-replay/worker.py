...
DEFAULT_FIXTURE = ROOT / 'reports/toy100/direct-particle-base/initialization-fixtures/mode_hold/initial-values.pt'
...
p.add_argument('--initial-state', type=Path, default=DEFAULT_FIXTURE)
...
if not a.initial_state.is_file():
    p.error('--initial-state must name the shared, existing initialization fixture')
a.output.mkdir(parents=True, exist_ok=False)
worker_bytes = Path(__file__).read_bytes()
(a.output / 'worker.py').write_bytes(worker_bytes)
...
def controller_signature(opt, role):
    ...
    if a.arm == 'research':
        ...
        out = dict(lr_last=state['lr_last'], lr_max=state['lr_max'],
                   ...
                   observed_steps=mechanism.receipt['critic_steps'], anchor_started=anchor is not None,
                   live_buffers=stream_digest(host_critic_owner.named_buffers()),
                   anchor_buffers=None if anchor is None else stream_digest(host_critic_owner.named_buffers()))
        out.update({key: state[key] for key in ('w', 'alpha', 'last_sur', 'sur_base', 'last_ratio', 'low_streak') if key in state})
        if 'sur_hist' in state:
            out.update(controller_history_signature(state['sur_hist']))
            out.update({key: mechanism.receipt[key] for key in ('ema_updates', 'ema_skips', 'ema_reseeds')})
        return out
    record = opt.record
    ...
    out = dict(lr_last=record.lr_last, lr_max=record.lr_max, calls=record.calls, anchor=anchor,
               observed_steps=record.observed_steps, anchor_started=record.anchor_started,
               live_buffers=stream_digest(opt.critic.named_buffers()),
               anchor_buffers=None if not record.anchor_started else stream_digest(opt.ema_critic.named_buffers()))
    ...
    if hasattr(record, 'sur_hist'):
        out.update(controller_history_signature(record.sur_hist))
        out.update({key: getattr(record, key) for key in ('ema_updates', 'ema_skips', 'ema_reseeds')})
    return out


def controller_history_signature(history):
    encoded = json.dumps(history, separators=(',', ':'), allow_nan=False).encode()
    return dict(sur_hist_length=len(history), sur_hist_sha256=hashlib.sha256(encoded).hexdigest())
...
def capture_host(generator, critic, prior, stream, noise_policy):
    global host_critic_owner
    host_critic_owner = critic.model if hasattr(critic, 'policy') else critic
...
record = dict(arm=a.arm, variant=a.variant, config=config, steps=a.steps, noise_horizon=1200,
              ...
              initialization_fixture=str(a.initial_state.resolve()),
