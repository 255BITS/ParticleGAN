#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel play_pause2."""
...
def make_task():
    ...
    task.update(
        name="img_play_pause2",
        pattern="play_pause2",
        modes=2,
        family="image_play_pause_media_control",
        importance_reason=(
            "Novel application: media-control glyphs on 8x8 grayscale — "
            "right-pointing play triangle vs pause double vertical bars. "
            "Product-adjacent to players and transport UI. Distinct from "
            "traffic-stack order, folder-tab L/R, EQ boosts, and geometry GMM breaks."
        ),
...
def templates_pp(spec):
    if spec["pattern"] != "play_pause2":
        return _ORIG(spec)
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "play_pause2 play-triangle vs pause-bars: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_pp):
        centers = templates_pp(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["PLAY triangle", "PAUSE bars"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("play_pause2 templates (media control)")
            fig.tight_layout()
