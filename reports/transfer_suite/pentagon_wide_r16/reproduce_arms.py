"""Pentagon-wide: five islands on regular pentagon R=16; published batchfeat vs MLP."""
...
# Regular pentagon vertices; circumradius R=16. Chord (side) = 2*R*sin(pi/5) ≈ 18.8,
# slightly wider than hex side=16. Local sigma unscaled. Distinct from hex (6),
# three_island (3), four_corner (4), eight_ring (8 on r=8).
R = 16.0
LOCAL_WIDTH = 1.0
BASE_INIT = 0.5
REACH = R
...
def pentagon_means(r=R):
    # Point-up pentagon: start at top.
    out = []
    for k in range(5):
...
def make_job():
    ...
    means = pentagon_means()
    n = len(means)
    spec["means"] = means
    spec["covariances"] = [[[w2, 0.0], [0.0, w2]] for _ in means]
    spec["masses"] = [1.0 / n] * n
    spec["name"] = f"vector_pentagon_wide_r{int(R)}_local{LOCAL_WIDTH}"
    spec["family"] = "pentagon_wide"
    spec["identifiable"] = True
    spec["importance_reason"] = (
        "Five isotropic islands on a regular pentagon with circumradius 16 "
        "(chord ~18.8, wider than hex side=16) and local sigma unscaled: "
        "published absolute RBF lengths that fit native two_broad saturate "
        "across the odd-sided lattice while a lengthscale-free MLP with "
        "matched prior init remains solvable. Distinct from hex_wide_side16 "
        "(6), three_island_wide_d16 (3), four_corner_wide_g8 (4), and "
        "eight_ring_wide_r8."
    )
    spec["limitations"] = (
...
def run_winner(out):
    ...
    print(f"START {name} init={BASE_INIT} R={R} local={LOCAL_WIDTH}", flush=True)
    ...
    return finalize(name, payload, out, t0, dict(
        kind="batchfeat", init_std=BASE_INIT, R=R, local_width=LOCAL_WIDTH, reach=REACH))
...
def run_control(out):
    ...
    init = CONTROL_INIT
    torch.set_num_threads(1)
    print(f"START {name} init={init} R={R} local={LOCAL_WIDTH}", flush=True)
    t0 = time.perf_counter()
    applied = []
    ...
    return finalize(name, payload, out, t0, dict(
        kind="mlp", init_std=init, R=R, local_width=LOCAL_WIDTH, reach=REACH))
