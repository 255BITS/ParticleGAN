#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel smile_frown2."""
...
def make_task():
    ...
    task.update(
        name="img_smile_frown2",
        pattern="smile_frown2",
        modes=2,
        family="image_smile_frown",
        importance_reason=(
            "Novel application: smile vs frown soft curves on 8x8 grayscale — upward-opening "
            "bright band (smile, center lower) vs downward-opening bright band (frown, center "
            "higher). Product-adjacent to expression / curve-chirality discrimination. Distinct "
            "from T_junction2, radial_wedge2, vh_bars2, soft_ring2, intensity2, mask_inpaint2, "
            "sparse_obs2, diag_ramp2, colorize_lr2, yin_yang2, U_orient2, arrow2, zigzag2, "
            "pacman2, and digit/letter topology."
        ),
...
def templates_smile_frown(spec):
    if spec["pattern"] != "smile_frown2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg, fg = 0.05, 0.9
    m0 = torch.full((8, 8), bg)
    m1 = torch.full((8, 8), bg)
    # image y increases down: smile = high y in center; frown = low y in center
    for x in range(1, 7):
        y_smile = int(round(5 - 0.22 * (x - 3.5) ** 2))
        y_frown = int(round(2 + 0.22 * (x - 3.5) ** 2))
        if 0 <= y_smile < 8:
            m0[y_smile, x] = fg
            if y_smile + 1 < 8:
                m0[y_smile + 1, x] = fg
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "smile_frown2 smile-vs-frown curves: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_smile_frown):
        centers = templates_smile_frown(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["smile", "frown"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("smile_frown2 templates (curve chirality)")
            fig.tight_layout()
