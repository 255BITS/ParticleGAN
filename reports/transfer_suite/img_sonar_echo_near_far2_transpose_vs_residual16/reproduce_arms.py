#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel sonar_echo_near_far2."""
...
def make_task():
    ...
    task.update(
        name="img_sonar_echo_near_far2",
        pattern="sonar_echo_near_far2",
        modes=2,
        family="image_sonar_ascope",
        importance_reason=(
            "Novel application: active-sonar A-scope ranging on 8x8 grayscale — "
            "near (early) echo return vs far (late) echo return with shared TX ping. "
            "Product-adjacent to radar/sonar ranging UIs. Distinct from chirp polarity, "
            "diffraction-order LR, and kymograph space-time streaks."
        ),
...
def templates_sonar(spec):
    if spec["pattern"] != "sonar_echo_near_far2":
        return _ORIG(spec)
    ...
    # near: blob at x=1..2
    images[0, 0, 2:6, 1] = fg
    images[0, 0, 3:5, 2] = 0.7
    images[0, 0, 4, 0] = 0.4  # tx ping
    # far: blob at x=5..6
    images[1, 0, 2:6, 6] = fg
    images[1, 0, 3:5, 5] = 0.7
    images[1, 0, 4, 0] = 0.4
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "sonar_echo_near_far2 near vs far A-scope echo: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_sonar):
        centers = templates_sonar(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["near echo", "far echo"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("sonar_echo_near_far2 templates (A-scope)")
            fig.tight_layout()
