#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel colorize_lr2."""
...
if not (REPO / "particlegan").exists():
    REPO = Path("/home/ntc/sliders-outscore/ParticleGAN")
...
def make_task():
    ...
    task.update(
        name="img_colorize_lr2",
        pattern="colorize_lr2",
        modes=2,
        family="image_colorize_lr_shade",
        importance_reason=(
            "Novel application: colorization-adjacent unpaired shade transfer — identical 8x8 "
            "silhouette with left-bright/right-dim vs left-dim/right-bright photometric assignment. "
            "Product-adjacent to colorization / channel-emphasis demos; distinct from vh_bars2, "
            "mask_inpaint2, soft_ring2, intensity2, sparse_obs2, diag_ramp2, plus_vs_x2, edge_cond2."
        ),
...
def templates_colorize_lr(spec):
    if spec["pattern"] != "colorize_lr2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    # shared central blob silhouette; mode0 left-bright, mode1 right-bright (chrominance proxy)
    for m in range(2):
        images[m, 0, 2:6, 2:6] = 0.55
    images[0, 0, 2:6, 2:4] = 0.95
    images[0, 0, 2:6, 4:6] = 0.25
    images[1, 0, 2:6, 2:4] = 0.25
    images[1, 0, 2:6, 4:6] = 0.95
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "colorize_lr2 colorization proxy LR shade: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_colorize_lr):
        centers = templates_colorize_lr(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["left-bright", "right-bright"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("colorize_lr2 templates (colorization proxy LR shade)")
            fig.tight_layout()
