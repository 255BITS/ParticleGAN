...
out_png = root / "boomerang_wide_scatter.png"
means = np.array([[-16.0, 8.0], [0.0, 16.0], [16.0, -2.0]], dtype=np.float64)
...
for ax, (arm, title) in zip(axes, titles):
    ...
    ax.plot(means[:, 0], means[:, 1], "r--", alpha=0.4, lw=1)
...
fig.suptitle(f"boomerang_wide_r16 @ tip 510e005 — local sigma=1.0 ({note})")
fig.tight_layout()
