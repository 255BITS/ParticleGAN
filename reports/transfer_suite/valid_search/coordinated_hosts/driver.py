...
OUT=Path('/tmp/pr36-valid-recipe-host')
    ...
    def adam_init(self,params,*args,**kwargs):
        ...
        for group in params:
            ...
            for is_prior in (False,True):
                ...
                if selected: groups.append(dict(group,params=selected,_research_prior=is_prior))
        args=list(args)
...
def main():
    ...
    parity=[]
    checks=[('vector','vector_unequal_mass',CARD,Path('/tmp/pr36-valid-recipe/screen/episodes/b999_lr075_d2_p30__vector_unequal_mass.json.gz')),
            ('image','img_blobs4',dict(CARD,g_factor=1.,d_factor=1.,prior_factor=1.),Path('/tmp/pr36-valid-adam999/episodes/bcap3_adam999__img_blobs4.json.gz'))]
    ...
    jobs += [('image',dict(s,architecture='residual_upsample',width=16)) for s in image_tasks.TASKS if s['tier']=='ranking']
