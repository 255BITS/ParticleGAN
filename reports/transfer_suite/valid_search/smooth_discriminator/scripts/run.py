"""Serial research-architecture experiment; fixed original vector recipe."""
import argparse
...
tasks=(['vector_unequal_mass','vector_unequal_width','vector_overlap'] if args.phase=='screen' else ['vector_two_broad','vector_anisotropic','vector_spiral'])
originals={s['name']:s for s in suite.manifest()['tasks']}
output=ROOT/args.phase;output.mkdir(exist_ok=False);(output/'episodes').mkdir()
plan=dict(candidates=cards,tasks=tasks,formulation='original Rp logistic b_cap3/kappa1.25 prior_reg.05 no particle L2',recipe='original Adam(0,.99),G LR .001,D LR .0015,prior LR .01,cosine,1:1 updates',scope='research discriminator architecture only',seed=0)
...
for card in cards:
 for task in tasks:
  original=deepcopy(originals[task]);spec=original|{'research_discriminator':deepcopy(card['architecture'])}
