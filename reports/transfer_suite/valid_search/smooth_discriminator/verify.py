...
for item in inventory['files']:
 ...
 if item['path'].endswith('.gz'):
  ...
  assert sha(raw)==item['uncompressed_sha256'] and len(raw)==item['uncompressed_bytes'],item['path']
...
assert len(rows)==30 and {n:sum(v) for n,v in by.items() if len(v)==6}=={'axis_softplus5':3,'axis_tanh':3}
summary={'gzip_roundtrips':roundtrips,'verified':True,'episodes':len(rows),'live_observations':24*len(rows),'architecture_cards':len(by),'source_files':len(protocol['source_sha256']),
 ...
 'ema_final_passes_separate':ema_final,'only_research_discriminator_changed':True,'source_commit_base':'a11c5304cde01c7fdc96e8a49a5a576b8cb8ebff'}
