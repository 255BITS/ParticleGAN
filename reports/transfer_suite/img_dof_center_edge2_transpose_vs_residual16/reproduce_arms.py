#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel dof_center_edge2."""
...
def make_task():
    ...
    task.update(
        name="img_dof_center_edge2",
        pattern="dof_center_edge2",
        modes=2,
        family="image_dof_center_edge",
        importance_reason=(
            "Novel application: sharp-center disk vs sharp outer ring on 8x8 grayscale — "
            "center focus vs peripheral/edge focus. Product-adjacent to depth-of-field / "
            "focus-plane discrimination. Distinct from soft_ring2, intensity2, ramp_corner2, "
            "diag_ramp2, radial_wedge2, smile_frown2, mask_inpaint2, sparse_obs2, colorize_lr2, "
            "T_junction2, vh_bars2, and soft probes X_plus/L_chirality/tri_ud/shear/hourglass."
        ),
...
def templates_dof(spec):
    if spec["pattern"] != "dof_center_edge2":
        return _ORIG(spec)
    ...
    cy, cx = 3.5, 3.5
    dist = torch.sqrt((gy - cy) ** 2 + (gx - cx) ** 2)
    center = torch.exp(-((dist / 1.6) ** 2))
    images[0, 0] = center * 0.9 + 0.05
    ring = torch.exp(-(((dist - 2.6) / 0.85) ** 2))
    images[1, 0] = ring * 0.9 + 0.05
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "dof_center_edge2 sharp-center vs sharp-ring focus: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_dof):
        centers = templates_dof(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["center focus", "edge focus"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("dof_center_edge2 templates (focus plane)")
            fig.tight_layout()
