#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel ramp_corner2."""
...
def make_task():
    ...
    task.update(
        name="img_ramp_corner2",
        pattern="ramp_corner2",
        modes=2,
        family="image_ramp_corner",
        importance_reason=(
            "Novel application: NW vs SE intensity corner ramp on 8x8 grayscale — brightness "
            "falling off from the top-left corner vs from the bottom-right corner. Product-adjacent "
            "to lighting-direction / shading discrimination. Distinct from diag_ramp2, intensity2, "
            "soft_ring2, smile_frown2, mask_inpaint2, sparse_obs2, colorize_lr2, T_junction2, "
            "radial_wedge2, vh_bars2, and soft probes halfmoon/C_open/wave_phase/ring_gap/S_soft/blob_count."
        ),
...
def templates_ramp_corner(spec):
    if spec["pattern"] != "ramp_corner2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    yy = torch.arange(8).float()
    xx = torch.arange(8).float()
    gy, gx = torch.meshgrid(yy, xx, indexing="ij")
    dist_nw = torch.sqrt(gy ** 2 + gx ** 2)
    dist_se = torch.sqrt((gy - 7) ** 2 + (gx - 7) ** 2)
    images[0, 0] = (1.0 - (dist_nw / dist_nw.max()).clamp(0, 1)) * 0.85 + 0.05
    images[1, 0] = (1.0 - (dist_se / dist_se.max()).clamp(0, 1)) * 0.85 + 0.05
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "ramp_corner2 NW-vs-SE corner lighting ramp: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_ramp_corner):
        centers = templates_ramp_corner(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["NW ramp", "SE ramp"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("ramp_corner2 templates (lighting direction)")
            fig.tight_layout()
