#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel C_vs_O2."""
...
def make_task():
    ...
    task.update(
        ...
        pattern="C_vs_O2",
        modes=2,
        family="image_open_closed_topology",
        importance_reason=(
            "Novel application: thick open C vs closed O on 8x8 grayscale — same outer "
            "support, topological open/closed distinction. Product-adjacent to "
            "topology-aware generation / hole detection. Distinct from soft_ring2, "
            "letter chirality farms, and spokes_rings soft-miss."
        ),
...
def templates_sr(spec):
    if spec["pattern"] != "C_vs_O2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg, fg = 0.05, 0.9
    m0 = torch.full((8, 8), bg)
    m1 = torch.full((8, 8), bg)
    # Closed O: thick ring (outer box minus inner hole)
    m1[1:7, 1:3] = fg  # left
    m1[1:7, 5:7] = fg  # right
    m1[1:3, 1:7] = fg  # top
    m1[5:7, 1:7] = fg  # bottom
    # Open C: same ring but open on the right (gap at cols 5-6 mid rows)
    m0[1:7, 1:3] = fg  # left
    m0[1:3, 1:7] = fg  # top
    m0[5:7, 1:7] = fg  # bottom
    # no right wall — open C
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "C_vs_O2 open vs closed curve topology: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_sr):
        centers = templates_sr(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["open C", "closed O"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("C_vs_O2 templates (radial vs angular)")
            fig.tight_layout()
