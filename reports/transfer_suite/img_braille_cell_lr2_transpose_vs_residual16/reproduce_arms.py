#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel braille_cell_lr2."""
...
def make_task():
    ...
    task.update(
        name="img_braille_cell_lr2",
        pattern="braille_cell_lr2",
        modes=2,
        family="image_braille_cell_lr",
        importance_reason=(
            "Novel application: Braille-style tactile cell mass LEFT-heavy "
            "(cols 1-2 raised dots) vs RIGHT-heavy (cols 5-6), with a shared "
            "faint middle vertical guide on 8x8 grayscale. Product-adjacent to "
            "tactile/UI glyph asymmetry. Distinct from barcode quiet-zone LR, "
            "finder diagonal, diffraction-order LR, and sparse-obs tasks."
        ),
...
def templates_braille(spec):
    if spec["pattern"] != "braille_cell_lr2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    images[:] = bg
    # shared middle vertical guide dots
    images[0, 0, 2, 3] = 0.35
    images[0, 0, 5, 3] = 0.35
    images[1, 0, 2, 3] = 0.35
    images[1, 0, 5, 3] = 0.35
    # left-heavy Braille mass cols 1-2
    for r, c in [(1, 1), (1, 2), (3, 1), (4, 2), (6, 1)]:
        images[0, 0, r, c] = fg
    # right-heavy cols 5-6
    for r, c in [(1, 5), (1, 6), (3, 6), (4, 5), (6, 6)]:
        images[1, 0, r, c] = fg
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "braille_cell_lr2 left-heavy vs right-heavy mass: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_braille):
        centers = templates_braille(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["LEFT-heavy", "RIGHT-heavy"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("braille_cell_lr2 templates")
            fig.tight_layout()
