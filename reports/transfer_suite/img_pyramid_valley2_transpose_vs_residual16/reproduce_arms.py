#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel pyramid_valley2."""
...
def make_task():
    ...
    task.update(
        name="img_pyramid_valley2",
        pattern="pyramid_valley2",
        modes=2,
        family="image_radial_intensity_polarity",
        importance_reason=(
            "Novel application: radial intensity polarity on 8x8 grayscale — bright-center "
            "pyramid vs dark-center valley on shared disk support. Product-adjacent to "
            "photometric / shading polarity and depth-from-shading. Distinct from glyph "
            "chirality farms, shade_lr2 soft, and geometry GMM breaks."
        ),
...
def templates_pv(spec):
    if spec["pattern"] != "pyramid_valley2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg = 0.05
    yy, xx = torch.meshgrid(torch.arange(8.0), torch.arange(8.0), indexing="ij")
    r = torch.sqrt((xx - 3.5) ** 2 + (yy - 3.5) ** 2)
    m0 = (0.95 - 0.18 * r).clamp(bg, 0.95)
    m1 = (0.15 + 0.18 * r).clamp(0.05, 0.9)
    mask = r <= 3.6
    m0 = torch.where(mask, m0, torch.full_like(m0, bg))
    m1 = torch.where(mask, m1, torch.full_like(m1, bg))
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "pyramid_valley2 radial intensity polarity: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_pv):
        centers = templates_pv(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["pyramid (bright center)", "valley (dark center)"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("pyramid_valley2 templates (radial intensity polarity)")
            fig.tight_layout()
