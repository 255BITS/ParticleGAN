"""tall_spike_wide_h16: Tall isosceles spike: base tips at (±6,0), apex at (0,16). Base NN=12, legs≈17.1; batchfeat vs MLP."""
...
def geom_means():
    return [[-6.0, 0.0], [6.0, 0.0], [0.0, 16.0]]
...
def make_job():
    ...
    spec["masses"] = [1.0 / n] * n
    spec["name"] = f"vector_tall_spike_wide_h16_local{LOCAL_WIDTH}"
    spec["family"] = "tall_spike_wide"
    spec["identifiable"] = True
    spec["importance_reason"] = (
        "Tall isosceles spike: base tips at (±6,0), apex at (0,16). Base NN=12, legs≈17.1. Narrower base than chevron_wide_d16 (base 16) and not equilateral three_island. Absolute RBF lengths fit native two_broad but struggle with the far apex; MLP matched init should pass."
    )
    spec["limitations"] = (
