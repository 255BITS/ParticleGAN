#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel moire_beat2."""
...
def make_task():
    ...
    task.update(
        name="img_moire_beat2",
        pattern="moire_beat2",
        modes=2,
        family="image_moire_beat_frequency",
        importance_reason=(
            "Novel application: dual-frequency moiré beat on 8x8 grayscale — "
            "period-2 vs period-3 vertical stripes. Product-adjacent to display "
            "aliasing, print interference, and frequency discrimination. Distinct "
            "from vh_bars, halftone angles, intensity, and glyph farms."
        ),
...
def templates_mb(spec):
    if spec["pattern"] != "moire_beat2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    m0 = torch.full((8, 8), bg)
    m1 = torch.full((8, 8), bg)
    # period-2 vertical stripes
    m0[:, ::2] = fg
    # period-3 vertical stripes
    m1[:, ::3] = fg
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "moire_beat2 period-2 vs period-3 stripes: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_mb):
        centers = templates_mb(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["period-2 stripes", "period-3 stripes"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("moire_beat2 templates (frequency beat)")
            fig.tight_layout()
