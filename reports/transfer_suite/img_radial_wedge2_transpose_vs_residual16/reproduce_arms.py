#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel radial_wedge2."""
...
def make_task():
    ...
    task.update(
        name="img_radial_wedge2",
        pattern="radial_wedge2",
        modes=2,
        family="image_radial_wedge",
        importance_reason=(
            "Novel application: angular/radial wedge modes on 8x8 grayscale — upper-half pie "
            "disk vs left-half pie disk (same radius ~3.2). Product-adjacent to orientation / "
            "angular-structure discrimination. Distinct from mask_inpaint2, sparse_obs2, "
            "soft_ring2, intensity2, diag_ramp2, colorize_lr2, vh_bars2, digit/letter topology, "
            "and row/col fills."
        ),
...
def templates_radial_wedge(spec):
    if spec["pattern"] != "radial_wedge2":
        return _ORIG(spec)
    ...
    cy, cx = gy - 3.5, gx - 3.5
    r = torch.sqrt(cy**2 + cx**2)
    ang = torch.atan2(cy, cx)  # [-pi, pi]
    disk = r <= 3.2
    # mode0: upper half-disk (ang >= 0); mode1: left half-disk (|ang| >= pi/2)
    images[0, 0] = torch.where(disk & (ang >= 0), torch.tensor(0.9), torch.tensor(0.05))
    images[1, 0] = torch.where(disk & (ang.abs() >= 1.5708), torch.tensor(0.9), torch.tensor(0.05))
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "radial_wedge2 upper-half vs left-half pie: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_radial_wedge):
        centers = templates_radial_wedge(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["upper half-disk", "left half-disk"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("radial_wedge2 templates (angular wedges)")
            fig.tight_layout()
