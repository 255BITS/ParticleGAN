#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel swirl_cw2."""
...
def make_task():
    ...
    task.update(
        ...
        pattern="swirl_cw2",
        modes=2,
        family="image_swirl_cw",
        importance_reason=(
            "Novel application: clockwise vs counterclockwise spiral intensity on 8x8 "
            "grayscale — continuous field chirality / handedness. Product-adjacent to "
            "orientation discrimination. Distinct from L_chirality2 (discrete mirror), "
            "soft_ring2, intensity2, radial_wedge2, smile_frown2, dof_center_edge2, "
            "mask_inpaint2, sparse_obs2, colorize_lr2, and soft probes spokes_rings / "
            "unpaired_rot90 / edge_shade / blob_count / sparse_cols."
        ),
...
def templates_swirl(spec):
    if spec["pattern"] != "swirl_cw2":
        return _ORIG(spec)
    ...
    spiral_cw = 0.5 + 0.4 * torch.sin(ang * 2.0 + dist * 1.1)
    spiral_ccw = 0.5 + 0.4 * torch.sin(-ang * 2.0 + dist * 1.1)
    images[0, 0] = spiral_cw.clamp(0, 1)
    images[1, 0] = spiral_ccw.clamp(0, 1)
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "swirl_cw2 CW vs CCW spiral intensity: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_swirl):
        centers = templates_swirl(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["CW spiral", "CCW spiral"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("swirl_cw2 templates (field chirality)")
            fig.tight_layout()
