#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel stairs_asc_desc2."""
...
def make_task():
    ...
    task.update(
        name="img_stairs_asc_desc2",
        pattern="stairs_asc_desc2",
        modes=2,
        family="image_stairs_asc_desc",
        importance_reason=(
            "Novel application: ascending vs descending L->R staircase on 8x8 "
            "grayscale — discrete path / stair polarity discrimination. "
            "Product-adjacent to orientation under a shared GAN formulation. "
            "Distinct from dots_count23, barcode23, L_chirality2, ramp_lr_flip2, "
            "shear_lr2, and geometry GMM breaks."
        ),
...
def templates_stairs(spec):
    if spec["pattern"] != "stairs_asc_desc2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg, fg = 0.05, 0.9
    m0 = torch.full((8, 8), bg)
    m1 = torch.full((8, 8), bg)
    # ascending L->R staircase (thick steps)
    for s in range(4):
        m0[6 - s : 7 - s, 1 + 2 * s : 3 + 2 * s] = fg
    # descending L->R
    for s in range(4):
        m1[1 + s : 2 + s, 1 + 2 * s : 3 + 2 * s] = fg
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "stairs_asc_desc2 ascending vs descending: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_stairs):
        centers = templates_stairs(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["ascend L->R", "descend L->R"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("stairs_asc_desc2 templates (stair polarity)")
            fig.tight_layout()
