#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel L_chirality2."""
...
def make_task():
    ...
    task.update(
        name="img_L_chirality2",
        pattern="L_chirality2",
        modes=2,
        family="image_L_chirality",
        importance_reason=(
            "Novel application: L vs mirrored L on 8x8 grayscale — discrete letter "
            "chirality / corner-joint handedness. Product-adjacent to orientation "
            "discrimination on discrete shapes. Distinct from swirl_cw2 (continuous "
            "spiral field chirality), smile_frown2, radial_wedge2, T_junction2, "
            "dof_center_edge2, mask_inpaint2, soft_ring2, intensity2."
        ),
...
def templates_L(spec):
    if spec["pattern"] != "L_chirality2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg, fg = 0.05, 0.9
    m0 = torch.full((8, 8), bg)
    m1 = torch.full((8, 8), bg)
    # canonical L: left vertical stem + bottom foot
    m0[1:7, 1:3] = fg
    m0[5:7, 1:7] = fg
    # mirrored L: right vertical stem + bottom foot
    m1[1:7, 5:7] = fg
    m1[5:7, 1:7] = fg
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "L_chirality2 L vs mirrored L: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_L):
        centers = templates_L(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["L", "mirrored L"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("L_chirality2 templates (discrete letter chirality)")
            fig.tight_layout()
