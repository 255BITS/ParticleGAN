#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel T_junction2."""
...
def make_task():
    ...
    task.update(
        name="img_T_junction2",
        pattern="T_junction2",
        modes=2,
        family="image_T_junction",
        importance_reason=(
            "Novel application: T-junction orientation on 8x8 grayscale — T pointing down "
            "(top bar + vertical stem) vs T pointing right (left bar + horizontal stem). "
            "Product-adjacent to orientation / junction-structure discrimination. Distinct "
            "from radial_wedge2, vh_bars2, soft_ring2, intensity2, mask_inpaint2, sparse_obs2, "
            "diag_ramp2, colorize_lr2, fg_bg_invert2, yin_yang2, and digit/letter topology."
        ),
...
def templates_T_junction(spec):
    if spec["pattern"] != "T_junction2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    bg, fg = 0.05, 0.9
    # mode0: T pointing down — horizontal bar rows 1-2 cols 1-6; vertical stem rows 2-6 cols 3-4
    m0 = torch.full((8, 8), bg)
    m0[1:3, 1:7] = fg
    m0[2:7, 3:5] = fg
    # mode1: T pointing right — vertical bar rows 1-6 cols 1-2; horizontal stem rows 3-4 cols 2-6
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "T_junction2 down-vs-right T: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_T_junction):
        centers = templates_T_junction(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["T down", "T right"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("T_junction2 templates (orientation)")
            fig.tight_layout()
