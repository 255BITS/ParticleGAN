#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel soft_ring2."""
...
def make_task():
    ...
    task.update(
        ...
        pattern="soft_ring2",
        modes=2,
        family="image_soft_ring_topology",
        importance_reason=(
            "Novel application: soft-intensity annular ring (0.4) vs filled disk (0.85) "
            "with matched outer bbox. Topology + photometric fidelity; distinct from "
            "stock img_intensity2 (uniform central patches) and from binary ring/disk."
        ),
...
def templates_soft_ring(spec):
    if spec["pattern"] != "soft_ring2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    images[0, 0, 2:6, 2:6] = 0.4
    images[0, 0, 3:5, 3:5] = 0.0
    images[1, 0, 2:6, 2:6] = 0.85
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "soft_ring2 topology+photometric: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_soft_ring):
        # save template viz
        centers = templates_soft_ring(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["ring@0.4", "disk@0.85"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("soft_ring2 templates")
            fig.tight_layout()
