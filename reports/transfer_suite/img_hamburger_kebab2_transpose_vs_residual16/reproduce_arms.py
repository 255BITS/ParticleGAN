#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel hamburger_kebab2."""
...
def make_task():
    ...
    task.update(
        name="img_hamburger_kebab2",
        pattern="hamburger_kebab2",
        modes=2,
        family="image_hamburger_kebab_menu",
        importance_reason=(
            "Novel application: UI menu iconography on 8x8 grayscale — "
            "hamburger (three horizontal bars) vs kebab (three vertical dots). "
            "Product-adjacent to mobile/desktop overflow menus. Distinct from "
            "play/pause, traffic stack, sort chevrons, and glyph farms."
        ),
...
def templates_hk(spec):
    if spec["pattern"] != "hamburger_kebab2":
        return _ORIG(spec)
    ...
    images[0, 0, 4, 1:7] = fg
    images[0, 0, 6, 1:7] = fg
    images[1, 0, 1:3, 3:5] = fg
    images[1, 0, 3:5, 3:5] = fg
    images[1, 0, 5:7, 3:5] = fg
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "hamburger_kebab2 bars vs dots: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_hk):
        centers = templates_hk(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["hamburger bars", "kebab dots"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("hamburger_kebab2 templates (UI menus)")
            fig.tight_layout()
