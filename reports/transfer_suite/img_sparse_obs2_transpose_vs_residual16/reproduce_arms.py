#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel sparse_obs2."""
...
def make_task():
    ...
    task.update(
        name="img_sparse_obs2",
        pattern="sparse_obs2",
        modes=2,
        family="image_sparse_observation",
        importance_reason=(
            "Novel application: sparse-observation occupancy maps — corners (NW+SE) "
            "vs edges (NE+SW) 2x2 sensor blobs at 0.9. Multimodal sparse layout fidelity; "
            "distinct from soft_ring2/intensity2/vignette/gradient/stripes."
        ),
...
def templates_sparse_obs(spec):
    if spec["pattern"] != "sparse_obs2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    # mode0: NW + SE corner blobs
    images[0, 0, 1:3, 1:3] = 0.9
    images[0, 0, 5:7, 5:7] = 0.9
    # mode1: NE + SW corner blobs
    images[1, 0, 1:3, 5:7] = 0.9
    images[1, 0, 5:7, 1:3] = 0.9
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "sparse_obs2 corner-vs-edge occupancy: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_sparse_obs):
        centers = templates_sparse_obs(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["corners NW+SE", "edges NE+SW"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("sparse_obs2 templates")
            fig.tight_layout()
