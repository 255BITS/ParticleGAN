#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel letterbox_pillar2."""
...
def make_task():
    ...
    task.update(
        name="img_letterbox_pillar2",
        pattern="letterbox_pillar2",
        modes=2,
        family="image_cinematic_aspect_framing",
        importance_reason=(
            "Novel application: cinematic aspect framing on 8x8 grayscale — letterbox "
            "(horizontal bars top/bottom) vs pillarbox (vertical bars left/right) with "
            "matched bright content region. Product-adjacent to video/letterbox detection "
            "and aspect-ratio conditioning. Distinct from vh_bars glyph farm and geometry GMM breaks."
        ),
...
def templates_lp(spec):
    if spec["pattern"] != "letterbox_pillar2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    m0 = torch.full((8, 8), fg)
    m1 = torch.full((8, 8), fg)
    m0[0:2, :] = bg
    m0[6:8, :] = bg
    m1[:, 0:2] = bg
    m1[:, 6:8] = bg
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "letterbox_pillar2 cinematic aspect framing: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_lp):
        centers = templates_lp(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["letterbox (TB bars)", "pillarbox (LR bars)"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("letterbox_pillar2 templates (cinematic aspect framing)")
            fig.tight_layout()
