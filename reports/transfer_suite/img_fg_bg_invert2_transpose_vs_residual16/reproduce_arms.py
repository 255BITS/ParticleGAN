#!/usr/bin/env python3
"""Break: published transpose12 FAIL vs residual16 PASS on novel fg_bg_invert2."""
...
def make_task():
    ...
    task.update(
        name="img_fg_bg_invert2",
        pattern="fg_bg_invert2",
        modes=2,
        family="image_fg_bg_polarity",
        importance_reason=(
            "Novel application: figure-ground / photographic-invert unpaired polarity — identical "
            "8x8 central support with bright-on-dark vs dark-on-bright assignment. Product-adjacent "
            "to invert / style-polarity demos; distinct from intensity2 (same polarity, different "
            "level), colorize_lr2 (LR shade), vh_bars2, mask_inpaint2, soft_ring2, sparse_obs2, "
            "diag_ramp2, plus_vs_x2."
        ),
...
def templates_fg_bg(spec):
    if spec["pattern"] != "fg_bg_invert2":
        return _ORIG(spec)
    images = torch.zeros(spec["modes"], 1, 8, 8)
    # mode0: bright foreground blob on dark background
    images[0, 0, :, :] = 0.12
    images[0, 0, 2:6, 2:6] = 0.92
    # mode1: photographic invert — dark foreground on bright background (same support)
    images[1, 0, :, :] = 0.88
    images[1, 0, 2:6, 2:6] = 0.08
    return images
...
def main():
    ...
    summary = {
        "task": task["name"],
        "tip_sha": TIP,
        "principal": "fg_bg_invert2 figure-ground polarity: published transpose12 vs residual16",
        "published_winner": "image suite baseline transpose12 (RpGAN+b_cap+particles)",
        "arms": {},
    }
    index_arms = {}
    with patch.object(host, "templates", templates_fg_bg):
        centers = templates_fg_bg(task)
        try:
            import matplotlib
            matplotlib.use("Agg")
            import matplotlib.pyplot as plt
            fig, axes = plt.subplots(1, 2, figsize=(4, 2))
            for ax, img, title in zip(axes, centers, ["bright-on-dark", "dark-on-bright"]):
                ax.imshow(img[0].numpy(), cmap="gray", vmin=0, vmax=1, interpolation="nearest")
                ax.set_title(title)
                ax.axis("off")
            fig.suptitle("fg_bg_invert2 templates (figure-ground polarity)")
            fig.tight_layout()
