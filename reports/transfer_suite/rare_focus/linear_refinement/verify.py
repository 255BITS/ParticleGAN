...
for f in inv['files']:
 data=(ROOT/f['path']).read_bytes();assert sha(data)==f['sha256'] and len(data)==f['bytes']
...
rows=json.loads(gzip.decompress((ROOT/'index.json.gz').read_bytes()))['records'];seen=set();protocol=None;by={};ema_final=0
for phase in sorted({r['phase'] for r in rows}):
 p=json.loads(gzip.decompress((ROOT/phase/'protocol.json.gz').read_bytes()))
...
for row in rows:
 ...
 assert s==o|c['overrides']|{'research_discriminator':c['architecture']}
 assert c['overrides']==dict(d_hidden=c['architecture']['hidden'],d_layers=2)
...
assert len([r for r in rows if r['phase']=='screen'])==4
assert [r['candidate']['name'] for r in rows if r['phase']=='screen' and r['verdict']['passed']]==['linear_skip_d96_beta5']
assert len(rows)==9 and {n:sum(v) for n,v in by.items() if len(v)==6}=={'linear_skip_d96_beta5':3}
summary={'gzip_roundtrips':roundtrips,'verified':True,'episodes':len(rows),'live_observations':24*len(rows),'architecture_cards':len(by),'source_files':len(protocol['source_sha256']),
 'recorded_wall_seconds':sum(r['seconds'] for r in rows),'complete_profiles':{n:sum(v) for n,v in by.items() if len(v)==6},
 'ema_final_passes_separate':ema_final,'only_discriminator_architecture_changed':True,'source_commit_base':'981ccbcd6e7e77a1f41f8aac3cc42d1fa1ceab45'}
