...
import json
ROOT=Path('/tmp/pr36-valid-linear-final');rows=[]
...
(ROOT/'MATRIX.md').write_text('\n'.join(lines)+'\n');(ROOT/'index.json').write_text(json.dumps({'records':rows,'episodes':len(rows),'source_commit_base':'981ccbcd6e7e77a1f41f8aac3cc42d1fa1ceab45','research_module':'benchmarks/transfer_suite/linear_skip_refinement_research.py'},indent=2,sort_keys=True)+'\n')
for name,group in ordered:
