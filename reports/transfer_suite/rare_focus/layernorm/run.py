"""Rare-mode pointwise discriminator normalization; fixed formulation and recipe."""
...
CARDS=[dict(name=f'ln_{position}_{activation}_d{width}',features='axis',activation=activation,
            beta=5.,normalization='pointwise_layernorm',position=position,width=width)
       for position,activation in [('pre','softplus'),('post','softplus'),('pre','silu')] for width in (64,96)]


class NormalizedCritic(SmoothFourierCritic):
    def __init__(self,in_dim=2,hidden_dim=64,n_hidden=2,fourier=2,*,architecture):
        ...
        old=list(self.net);layers=[]
        for i in range(0,len(old)-1,2):
            linear,activation=old[i:i+2]
            norm=nn.LayerNorm(hidden_dim)
            layers.extend((linear,norm,activation) if architecture['position']=='pre' else (linear,activation,norm))
        layers.append(old[-1]);self.net=nn.Sequential(*layers)
...
def main():
    ...
    for card in CARDS:
        with torch.random.fork_rng():
            torch.manual_seed(0)
            d=NormalizedCritic(hidden_dim=card['width'],architecture=card)
    ...
    def run(card,name,phase):
        ...
        with patch.object(vector_tasks,'SimpleMLPDiscriminator',partial(NormalizedCritic,architecture=card)):
        ...
        lines=['# Pointwise LayerNorm discriminator search','',
