...
from benchmarks.transfer_suite.skip_critic_research import ARCHITECTURES,constructor

ROOT=Path('/tmp/pr36-valid-skip-d')
...
for card in cards:card['parameter_count']=checks[card['name']]['parameters']
output=ROOT/args.phase;output.mkdir(exist_ok=False);(output/'episodes').mkdir()
plan=dict(candidates=cards,tasks=tasks,formulation='original Rp logistic b_cap3/kappa1.25 prior_reg.05 no particle L2',recipe='original Adam(0,.99),G LR .001,D LR .0015,prior LR .01,cosine,1:1 updates',scope='research discriminator architecture only',seed=0, cross_rule='Only sustained rare winners may receive the other five data toys; best two at most by passing suffix then lower final normalized shortfall.')
