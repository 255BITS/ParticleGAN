"""One-shot round14 pipeline: completion-only drain, fixed selection, diagnostics."""
import json
...
def diagnostics(runs, destination):
    ...
    with (destination/'diagnostics.log').open('w') as log:
        for script, filename, extra in (
            ...
            ('diagnose_memory_future_rank.py', 'future_signal.json', []),
        ):
...
configs = root/'experiments/configs/memory_handoff/information_round14'
order = json.loads((report/'scouts.json').read_text())
...
if selected:
    followup = queue.with_name('information_round14_followup')
    ...
    folder = configs.with_name('information_round14_followup')
