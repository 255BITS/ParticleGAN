"""Apply the predeclared round14 gates, only after all selected scouts finish."""
import json
from pathlib import Path

queue = Path('runs/memory_path/information_round14')
report = Path('reports/memory-handoff/information_round14')
...
assert len(list((queue/'done').glob('*.json'))) == len(names)
...
result = dict(all_selected_completed=True, baseline=base['name'], criteria='plan.md, fixed before training',
