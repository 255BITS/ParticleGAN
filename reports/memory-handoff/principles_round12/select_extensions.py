"""Apply the predeclared round12 gates, only after all twelve scouts finish."""
import json
from pathlib import Path

queue = Path('runs/memory_path/principles_round12')
report = Path('reports/memory-handoff/principles_round12')
...
base = next(r for r in rows if r['name'] == 'proposal_mixed_pair25')
comparisons = []
for row in rows:
...
result = dict(all_twelve_completed=True, baseline=base['name'], criteria='plan.md, fixed before training',
