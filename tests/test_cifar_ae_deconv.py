"""Architecture, isolation, initialization and exact resume for plain deconv G."""
import copy
import hashlib
import json
import os
...
from experiments import train_cifar_ae_deconv as new
...
def test_generator_structure_output_and_latent_gradients():
    ...
    g = new.DeconvGenerator()
    assert sum(p.numel() for p in g.parameters()) == 298595
    ...
    assert not any(isinstance(m, (torch.nn.modules.batchnorm._NormBase,
                                  ...
                                  torch.nn.LocalResponseNorm)) for m in layers)
    ...
    assert [(m.in_channels, m.out_channels) for m in convs] == [(128, 64), (64, 32), (32, 3)]
...
def test_discriminator_encoder_and_global_rng_match_original():
    ...
    assert isinstance(g, new.DeconvGenerator)
