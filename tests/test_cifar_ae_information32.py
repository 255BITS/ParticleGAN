...
from experiments.probe_cifar_ae_information32 import (
...
def test_32_children_have_unique_streams_and_preserve_historical_first_16():
    from experiments.probe_cifar_ae_information32 import panel_child_seed
    used=set()
    for split in (1000000,2000000,3000000,4000000):
        for parent in range(1024):
            for child in range(32):
                seed=panel_child_seed(split+parent*16,child)
                assert seed not in used
                used.add(seed)
                if child<16:assert seed==split+parent*16+child
    assert len(used)==4*1024*32
