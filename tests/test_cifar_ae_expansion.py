...
from experiments import train_cifar_ae_expansion as new
...
def test_actual_parent_expansion_and_resume(tmp_path):
    ...
    assert torch.equal(ck['prior']['z'], original['prior']['z'].repeat_interleave(4,0))
    assert torch.equal(ck['ema_prior']['z'], original['ema_prior']['z'].repeat_interleave(4,0))
    for pid,state in original['optimizer_g']['state'].items():
        expanded = pid == original['optimizer_g']['param_groups'][2]['params'][0]
        for k,v in state.items():
            expected = v.repeat_interleave(4,0) if expanded and k != 'step' else v
    ...
    assert torch.equal(ia,ib//4) and torch.equal(a.get_state(),b.get_state())
    ...
    assert (prior.z.grad.reshape(1024,4,64).std(1).abs()>0).any()
    saved=copy.deepcopy(prior.state_dict())
    other=new.MoGParticlePrior(1024,64,sigma_rel=.025).cuda()
    new.activate_expansion(other,4,0,True);other.load_state_dict(saved)
...
@pytest.mark.skipif(os.environ.get('RUN_CUDA_IMAGE_TESTS') != '1', reason='real CIFAR CUDA test')
def test_expanded_full_resume_and_e_only_gradients(tmp_path, monkeypatch, deterministic_algorithms):
    ...
    cfg.update(expansion_factor=4,resume_checkpoint=str(path),resume_sha256=hashlib.sha256(path.read_bytes()).hexdigest())
    ...
    prior=new.MoGParticlePrior(16,8).cuda();new.activate_expansion(prior,4,123,True)
