...
from reports.toy100.exit_aware_step_clip import (
    exit_scales, exit_targets, realize_output_scales, support_fence,
)
...
def test_edge_step_away_from_reals_shrinks_and_stops_worsening():
    ...
    scales, fence = exit_scales(before, after, reals)
    assert float(torch.cdist(before, reals).min()) > float(fence)
    assert float(scales) == 0.0
    landed = exit_targets(before, after, reals, scales)
    d1 = torch.cdist(landed, reals).min()
    assert float(d1) <= float(fence) + 1e-4


def test_thin_margin_inside_the_fence_cannot_cross_it():
    reals = _reals()
    fence = float(support_fence(reals))
    before = torch.tensor([[fence * 0.5, 0.0]])
    after = torch.tensor([[fence + 0.3, 0.0]])
