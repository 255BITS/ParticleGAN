...
class ParticleFinetuneTests(unittest.TestCase):
    ...
    def test_cpu_smoke_writes_flushed_logs_and_replays(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("Deferred: no landing evaluation", summary["selection"])
            self.assertIn("landing selection is not part of this train step", text)
    ...
    def test_safe_fast_weight_zero_does_not_attach_and_positive_weight_trains(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("SELECTION shipped=1", text)
            self.assertIn("late-collapse selection runs after the last step", text)
            self.assertEqual(summary["selected_step"], 1)
            self.assertEqual(summary["adv_weight"], 1.)
