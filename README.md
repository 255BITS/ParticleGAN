...
(`python -u examples/slow_fast_paired_2d.py`) before a Lunar speed claim.
The [Lunar slow→fast path](docs/gym-slow-fast.md) collects successful `#18`
rollouts, pairs slow and fast landings, and finetunes with that same
paired-error RpGAN (`adv_weight=1`, not the safe-fast kinematic cost).
