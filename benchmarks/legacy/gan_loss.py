"""Pinned copy of the multi-mode GAN loss (benchmarks only; see benchmarks.legacy)."""
import torch
...
class GANLoss:
    ...
    def __init__(
        ...
        if self.mode not in ("vanilla", "rp", "ra"):
            raise ValueError(f"Unknown gan mode: {self.mode}")
        if not 0 <= self.label_smoothing <= 1 or not 0 <= self.label_flip_prob <= 1:
            raise ValueError("label_smoothing and label_flip_prob must be in [0, 1]")
