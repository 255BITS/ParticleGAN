...
from .shared_batch_feature_research import ARCHITECTURES, constructor, variant
...
def render(records, output):
    ...
    rows = ['# Shared cap6 minibatch-feature discriminator research', '',
...
def run(declaration, output):
    ...
    protocol.update(version='shared-batch-feature-v1', declaration=declaration, seed=0, jobs=jobs,
                    architectures=cards, recipe=recipe().to_dict(),
                    selection='D-only architecture trials. All 24 checks, final 5 live PASS; EMA separate. Stop after first sustained live PASS.')
