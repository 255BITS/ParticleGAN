...
Prints a single JSON object on stdout. Training chatter stays in ``--log``.

    python -u -m benchmarks.toy100.det_init_screen --gate ring --init hid_q_r2 --log results/det-init-f/logs/hid_q_r2-ring-s0.log --output results/det-init-f/runs/hid_q_r2/s0/ring
"""
...
def command(gate: str, init: str | None, output: Path, *, init_only: bool = False) -> list[str]:
    ...
    if init_only:
        if gate not in ("ring", "unequal", "stripes", "blobs"):
            raise ValueError(f"{gate} has no init-only capture")
...
def _param_hash(payload: dict) -> str | None:
    import hashlib
    optimizers = (payload.get("proof") or {}).get("initial_optimizers")
    if not optimizers:
        return None
    digest = hashlib.sha256()
    for group in optimizers:
        for item in group:
            digest.update(str(item.get("shape")).encode())
            digest.update(str(item.get("sha256")).encode())
    return digest.hexdigest()


def summary(gate: str, output: Path, returncode: int) -> dict:
    ...
    if gate in ("grid100", "rotated100"):
        ...
        train_summary = output / gate / "summary.json"
        if train_summary.exists():
            row["train_status"] = json.loads(train_summary.read_text()).get("status")
    ...
    row["torch"] = payload.get("torch")
    row["param_sha256"] = _param_hash(payload)
    if gate == "hold":
        ...
        keep = ("modes", "hq", "min_mass_ratio", "sw1_normalized", "mass_tv",
                "component_min_eigen_ratio", "component_covariance_error", "mean_rmse")
        ...
        if verdict.get("metrics"):
            row["metrics"] = {item["metric"]: [item["value"], item["status"]] for item in verdict["metrics"]}
    return row
...
def main() -> None:
    ...
    cmd = command(args.gate, args.init, args.output, init_only=args.init_only)
    if args.seed_offset:
    ...
    row = summary(args.gate, args.output, done.returncode)
    row.update(init=args.init or "k3p", seed_offset=args.seed_offset, log=str(args.log),
