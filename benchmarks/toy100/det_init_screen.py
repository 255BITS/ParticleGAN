...
Prints a single JSON object on stdout. Training chatter stays in ``--log``.
    ...
    python -u -m benchmarks.toy100.det_init_screen --gate ring --init hq_pb_pq --output /tmp/out --log /tmp/out.log
"""
...
def _summary(gate: str, output: Path, returncode: int) -> dict:
    ...
    initial = output / "initial-values.pt"
    if initial.exists():
        row["initial_sha256"] = hashlib.sha256(initial.read_bytes()).hexdigest()
    return row
...
def main() -> None:
    ...
    if args.init_only and args.gate not in ("grid100", "rotated100"):
        cmd.append("--init-only")
    if args.seed_offset or args.init_only:
    ...
    row.update(init=args.init or "baseline", seed_offset=args.seed_offset, log=str(args.log))
