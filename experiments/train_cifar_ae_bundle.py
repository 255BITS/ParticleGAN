#!/usr/bin/env python
"""Isolated quality scout for batched D + bcap-logit reuse + foreach EMA.

Snapshot of train_cifar_particle_ae.py at 492f665, kept separate while the
production trainer runs. Baseline and bundle differ only in implementation.
"""
import argparse
import copy
import hashlib
...
DEFAULTS = {
    ...
    'encoder_backbone': 'scratch', 'keep_checkpoints': False, 'reg_every': 4, 'speed_bundle': False,
}
...
def validate(cfg):
    ...
    if type(cfg['speed_bundle']) is not bool:
        raise ValueError('speed_bundle must be boolean')
...
def discriminator_loss(d, real, fake, adversarial, penalty, step, bundle):
    if not bundle:
        dp = penalty(d, real, fake, step)
        return adversarial.d_loss(d(real), d(fake)) + dp, dp
    if step % penalty.lazy_k == 0:
        joined = torch.cat([real.detach(), fake.detach()]).requires_grad_(True)
        real_logits, fake_logits = d(joined).chunk(2)
        grads = torch.autograd.grad(real_logits.sum() + fake_logits.sum(), joined,
                                    create_graph=True)[0].chunk(2)
        norms = [(grad.square().flatten(1).sum(1) + 1e-12).sqrt() for grad in grads]
        dp = penalty.coeff * penalty.lazy_k / 2 * sum(
            (norm - penalty.kappa).relu().square().mean() for norm in norms)
    else:
        real_logits, fake_logits = d(torch.cat([real, fake])).chunk(2)
        dp = real.new_zeros(())
    return adversarial.d_loss(real_logits, fake_logits) + dp, dp


@torch.no_grad()
def update_ema_bundle(targets, sources, decay):
    torch._foreach_lerp_([p for m in targets for p in m.parameters()],
                        [p for m in sources for p in m.parameters()], 1 - decay)
...
def train(cfg):
    ...
    ema_targets, ema_sources = [eg, ee, ep], [g, e, prior]
    og = torch.optim.Adam([
    ...
    metadata['speed_bundle'] = cfg['speed_bundle']
    ...
    print(f"START arm={cfg['arm']} encoder={cfg['encoder_backbone']} bundle={cfg['speed_bundle']} reg_every={cfg['reg_every']} steps={cfg['steps']} sigma={float(prior.sigma):.6f} init={initial_hash}", flush=True)
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as log:
        for step in range(1, cfg['steps'] + 1):
            ...
            dl, dp = discriminator_loss(d, real, fake, adversarial, penalty, step, cfg['speed_bundle'])
            dl.backward()
            od.step()
            ...
            og.step()
            if cfg['speed_bundle']:
                update_ema_bundle(ema_targets, ema_sources, cfg['ema'])
            else:
