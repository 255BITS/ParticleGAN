#!/usr/bin/env python
"""Wider deconvolutional CIFAR generator with hidden GroupNorm.

Standalone trainer preserves historical source certificates. The new generator
has one latent projection, three transposed convolutions, and hidden GroupNorm.
An explicit fixed_sigma overrides count-dependent calibration before hashing.
"""
import argparse
import copy
import hashlib
...
DEFAULTS = {
    ...
    'd_updates': 1, 'generator_arch': 'deconv_wide_norm', 'recon_grad': 'all',
...
def validate(cfg):
    ...
    if cfg['generator_arch'] not in ('cnn', 'transgan', 'deconv', 'deconv_wide_norm'):
        raise ValueError('generator_arch must be cnn, transgan, deconv or deconv_wide_norm')
    ...
    if cfg['generator_arch'] in ('transgan', 'deconv', 'deconv_wide_norm') and (cfg['g_width'] != 0 or cfg['g_depth'] != 1):
