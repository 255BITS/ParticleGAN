#!/usr/bin/env python
"""Full-state learning-rate forks with observational generator diagnostics.

Copied from the immutable particle scaling trainer; update ordering is preserved.
"""
import argparse
import copy
import hashlib
...
def validate(cfg):
    if type(cfg['generator_diagnostics']) is not bool:
        raise ValueError('generator_diagnostics must be bool')
...
def load_resume(cfg):
    ...
    allowed = {'generator_diagnostics', 'expansion_factor', 'initial_eval_samples', 'steps', 'out_dir', 'log_interval', 'eval_interval', 'eval_samples', 'final_samples',
...
@torch.no_grad()
def generator_update_metrics(module, before):
    """Actual single-Adam-step movement; each tensor retains its own scale."""
    names, values = [], []
    for name, p in module.named_parameters():
        old = before[name]
        weight = old.square().mean().sqrt()
        update = (p - old).square().mean().sqrt()
        grad = p.grad.square().mean().sqrt() if p.grad is not None else p.new_zeros(())
        names.append(name)
        values.append(torch.stack((weight, update, update/weight.clamp_min(1e-12), grad)))
    return {name: dict(zip(('weight_rms', 'update_rms', 'relative_update', 'gradient_rms'), row))
            for name, row in zip(names, torch.stack(values).cpu().tolist())}


class GeneratorDiagnostics:
    """Fixed parent-EMA latent inputs isolate generator movement from prior movement."""
    @torch.no_grad()
    def __init__(self, g, eg, prior, seed):
        stream = rng(seed + 182003)
        means = prior.means()
        ids = torch.randint(len(means), (256,), device=means.device, generator=stream)
        self.z = means[ids] + prior.sigma * torch.randn(256, means.shape[1], device=means.device, generator=stream)
        self.parent = {}
        self.previous = {}
        self.initial = self.observe(g, eg)

    @torch.no_grad()
    def observe(self, g, eg):
        cpu_rng, cuda_rng = torch.get_rng_state(), torch.cuda.get_rng_state_all()
        result = {}
        for label, model in [('live', g), ('ema', eg)]:
            captured, handles = {}, []
            def block_hook(name):
                def hook(module, inputs, output):
                    captured[name] = {'activation_rms': float(output.square().mean().sqrt())}
                return hook
            def cond_hook(name):
                def hook(module, inputs, output):
                    scale, shift = output.chunk(2, dim=1)
                    captured[name] = {'scale_rms': float(scale.square().mean().sqrt()),
                                      'gain_rms': float((1 + scale).square().mean().sqrt()),
                                      'gain_abs_max': float((1 + scale).abs().max()),
                                      'shift_rms': float(shift.square().mean().sqrt())}
                return hook
            for name, module in model.named_modules():
                if isinstance(module, ResBlock):
                    handles.append(module.register_forward_hook(block_hook(name)))
                    if module.affine:
                        handles.append(module.cond.register_forward_hook(cond_hook(name + '.cond')))
            try:
                output = model(self.z).detach()
            finally:
                for handle in handles:
                    handle.remove()
            if label not in self.parent:
                self.parent[label] = output.clone()
                self.previous[label] = output.clone()
            result[label] = {'layers': captured,
                             'output_rms': float(output.square().mean().sqrt()),
                             'saturated_fraction': float((output.abs() > .99).float().mean()),
                             'parent_drift_rms': float((output-self.parent[label]).square().mean().sqrt()),
                             'previous_probe_drift_rms': float((output-self.previous[label]).square().mean().sqrt())}
            self.previous[label] = output.clone()
...
def train(cfg):
    ...
    diagnostics = GeneratorDiagnostics(g, eg, ep, cfg['seed']) if cfg['generator_diagnostics'] else None
    if diagnostics is not None:
        write_json(out / 'generator_diagnostics_initial.json', diagnostics.initial)
    train_seconds = 0.
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as log:
        for step in range(start_step + 1, cfg['steps'] + 1):
            ...
            if step == start_step + 1 or step % cfg['log_interval'] == 0 or step == cfg['steps']:
                ...
                if diagnostics is not None:
                    generator_before = {name: p.detach().clone() for name, p in g.named_parameters()}
            og.step()
            if step == start_step + 1 or step % cfg['log_interval'] == 0 or step == cfg['steps']:
                prior_update_rms = float((prior.z.detach() - prior_before).square().mean().sqrt())
                if diagnostics is not None:
                    generator_updates = generator_update_metrics(g, generator_before)
            ...
            if report or evaluate:
                ...
                if diagnostics is not None:
                    row['generator_updates'] = generator_updates
                ...
                if evaluate:
                    ...
                    if diagnostics is not None:
                        row['generator_diagnostics'] = diagnostics.observe(g, eg)
