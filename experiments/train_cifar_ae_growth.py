#!/usr/bin/env python
"""Function-preserving AE-GAN capacity growth from full-state checkpoints.
...
def load_resume(cfg):
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('recon_weight', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'grow_g', 'grow_d_heads')
                    if old[k] != cfg[k]}}
