#!/usr/bin/env python
"""Checkpoint joint continuations isolating the generator learning rate.
...
def load_resume(cfg):
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('recon_weight', 'g_lr_scale', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_grad', 'reg_every', 'penalty_coeff', 'd_warmstart', 'd_warmstart_sha256')
                    if old[k] != cfg[k]}}
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as log:
        for step in range(start_step + 1, cfg['steps'] + 1):
            ...
            if report or evaluate:
                ...
                row = {'step': step, 'learning_rates': {'G': og.param_groups[0]['lr'], 'E': og.param_groups[1]['lr'], 'prior': og.param_groups[2]['lr'], 'D': od.param_groups[0]['lr']}, 'lr_scale': lr_scale, 'd_updates': cfg['d_updates'], 'd_loss': float(dl.detach()), 'g_loss': float(gl.detach()),
