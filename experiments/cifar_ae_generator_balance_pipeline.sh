...
set -euo pipefail
cd "$(dirname "$0")/.."
TRACK="${1:-generator_balance}"
GPUS="${2:-0,1}"
case "$TRACK" in generator_balance|generator_balance_smoke) ;; *) exit 2 ;; esac
...
.venv/bin/python -u experiments/follow_grid.py --root "$RUN_ROOT" --log "$RUN_ROOT/PIPELINE.log" -- \
  ...
  --python .venv/bin/python --trainer experiments/train_cifar_ae_balance.py

.venv/bin/python experiments/analyze_cifar_ae_balance.py --track "$TRACK"
