#!/usr/bin/env python
"""Fresh paired control evaluation of the single state encoder experiment."""
import argparse
import json
import logging
...
from lib.gym_control_evaluation import evaluate_controller, engine_regimes, paired_comparison, selection_key
from experiments.evaluate_gym_control import write_json, expert_action_diagnostics, verify_control_checkpoint

REPORTS = ROOT/'reports/gym/lunar_lander_state_control'
EPISODES = ROOT/'results/gym/lunar_lander/data/episodes.json'
REFERENCE = ROOT/'results/gym/lunar_lander_control/imitation/best.pt'


def check_disjoint(episodes, prior_protocols, validation, test):
    used = {int(e['seed']) for e in episodes}
    for protocol in prior_protocols:
        used.update(protocol.get('validation_seeds', []))
        used.update(protocol.get('test_seeds', []))
    if len(set(validation)) != len(validation) or len(set(test)) != len(test):
        raise ValueError('Duplicate evaluation reset seeds')
    if set(validation) & set(test) or used & set(validation+test):
        raise ValueError('Fresh evaluation reset seeds overlap previous data or evaluations')
    return sorted(used)
...
def freeze_protocol(path, episodes_path, reference):
    ...
    validation, test = list(range(591000,591020)), list(range(691000,691050))
    used = check_disjoint(json.loads(episodes_path.read_text()), previous, validation, test)
    source_names = ['experiments/evaluate_gym_state_control.py', 'experiments/evaluate_gym_control.py',
    ...
    configs = sorted((ROOT/'configs/gym/lunar_lander_state_control').glob('*.yaml'))
...
def verify_checkpoint(bundle, checkpoint):
    ...
    for key in ('preprocessing_checkpoint', 'episodes'):
        if sha256(ROOT/p[key]['path']) != p[key]['sha256']:
            raise RuntimeError(f'Training artifact changed: {key}')
    for filename,digest in [('source.zip',p['source_archive_sha256']), ('expert_records.npz',p['expert_data']['npz_sha256'])]:
...
def state_metrics(raw, target, scaler):
    normalized = scaler.state(target)
    return dict(standardized_continuous_mse=float((raw[:,:6]-normalized[:,:6]).square().mean()),
        physical_continuous_mse=float((scaler.inverse_state(raw)[:,:6]-target[:,:6]).square().mean()),
        contact_bce=float(F.binary_cross_entropy_with_logits(raw[:,6:], target[:,6:])),
        contact_brier=float((raw[:,6:].sigmoid()-target[:,6:]).square().mean()),
        contact_accuracy=float(((raw[:,6:] >= 0) == target[:,6:].bool()).float().mean()))


@torch.no_grad()
def auxiliary_metrics(bundle, records):
    from lib.gym_state_control import predict_state_control
    outputs=[]
    for start in range(0,len(records['states']),1024):
        raw,_ = predict_state_control(bundle, records['states'][start:start+1024], records['terrain'][start:start+1024])
        outputs.append(raw)
    raw=torch.cat(outputs)
    scaler=bundle['scaler']
    states,actions,following=[torch.as_tensor(records[k],dtype=torch.float32,device=bundle['device'])
        for k in ('states','actions','next_states')]
    predicted_actions=scaler.inverse_action(raw[:,8:10])
    agreement=engine_regimes(predicted_actions.cpu().numpy()) == engine_regimes(records['actions'])
    return dict(records=len(states), state=state_metrics(raw[:,:8],states,scaler),
        next_state=state_metrics(raw[:,10:],following,scaler),
        persistence_next_state=dict(standardized_continuous_mse=float((scaler.state(states)[:,:6]-scaler.state(following)[:,:6]).square().mean()),
            physical_continuous_mse=float((states[:,:6]-following[:,:6]).square().mean()),
            contact_brier=float((states[:,6:]-following[:,6:]).square().mean()),
            contact_accuracy=float((states[:,6:]==following[:,6:]).float().mean())),
        standardized_action_mse=float((raw[:,8:10]-scaler.action(actions)).square().mean()),
        physical_action_mse=float((predicted_actions-actions).square().mean()),
        main_regime_agreement=float(agreement[:,0].mean()), lateral_regime_agreement=float(agreement[:,1].mean()),
        joint_regime_agreement=float(agreement.all(1).mean()))


def rollout_auxiliary(bundle, row):
    with np.load(row['traces']) as trace:
        terrain={e['seed']:e['terrain'] for e in row['episodes']}
        records={key:trace[key] for key in ('states','actions','next_states')}
        records['terrain']=np.asarray([terrain[int(s)] for s in trace['seeds']],np.float32)
        metrics=auxiliary_metrics(bundle,records)
        metrics.update(traces_sha256=row['traces_sha256'], simulator_calls=0,
            scope='Predictions at learner-visited measured states versus actual successors under learner actions; transition-weighted',
            action_target='Recorded learner command: numerical replay consistency, not expert agreement')
        return metrics
...
def score(arm, checkpoint, split, name, out, protocol, device):
    ...
    if arm in ('probes','auxiliary'):
        if bundle['config']['steps'] != protocol['training_updates'] or bundle['config']['batch_size'] != protocol['training_batch_size']:
            raise RuntimeError('Training budget differs from frozen protocol')
    ...
    if arm in ('probes','auxiliary'):
        ...
        row['timing_scope']='E_control + G2 + MoG routing/offset diagnostics'
    else:
...
def refresh_leaderboard(out):
    ...
    lines=['# State-only Lunar Lander control','',
    ...
    lines += ['', 'Full action traces, paired comparisons, inference/training costs, engine usage, expert reconstruction and learner-successor metrics are in `leaderboard.json`. G3 has no alternative-action input; its consistency with G2 is measured on actual learner rollouts.','']
    (out/'README.md').write_text('\n'.join(lines))
    eligible=[s for s in selections if s['arm']!='expert']
    if eligible:
        winner=max(eligible,key=lambda s:selection_key(s['validation']))
        name=lambda arm: 'state_'+arm if arm in ('probes','auxiliary') else arm
...
def main():
    ...
    parser.add_argument('--log',type=Path,default=ROOT/'results/gym/lunar_lander_state_control/live.log')
    ...
    if args.baseline:
        ...
        for arm,checkpoint in [('expert',None),('imitation',args.reference)]:
