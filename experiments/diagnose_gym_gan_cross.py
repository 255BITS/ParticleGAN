#!/usr/bin/env python
"""Compare both selected GAN models on both frozen learner trace datasets."""
import argparse
import json
import logging
...
from experiments.evaluate_gym_gan_control import (REPORTS, auxiliary_metrics,
...
def diagnose(reports, output):
    ...
    for arm in ('joint','marginals'):
        ...
        models[arm]=load_gan_control_checkpoint(checkpoint,'cpu')
    ...
    value=dict(protocol_sha256=sha256(reports/'protocol.json'),provenance=provenance,
        sources={str(p.relative_to(ROOT)):sha256(p) for p in [Path(__file__),ROOT/'experiments/evaluate_gym_gan_control.py',ROOT/'lib/gym_state_control.py']},
