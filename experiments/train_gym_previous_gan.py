#!/usr/bin/env python
"""Train a previous-action three-generator GAN from scratch with expert action L2."""
import argparse
import copy
import hashlib
import json
...
DEFAULTS = dict(arm='previous_marginals', steps=2500, batch_size=256,
    ...
    out_dir='results/gym/lunar_lander_previous_gan/previous_marginals',
    live_log='results/gym/lunar_lander_previous_gan/live.log')


def validate(cfg):
    if set(cfg) != set(DEFAULTS) or cfg['arm'] != 'previous_marginals':
        raise ValueError('Unexpected previous-action GAN configuration')
    ...
    for key in ('marginal_weight', 'adversarial_weight', 'action_weight', 'lambda_state',
                'lambda_next', 'continuous_weight', 'contact_weight'):
...
def source_paths():
    names = ['experiments/train_gym_previous_gan.py', 'lib/gym_previous_gan.py',
    ...
    return ([ROOT / n for n in names] + sorted((ROOT / 'particlegan').glob('*.py')) +
            sorted((ROOT / 'configs/gym/lunar_lander_previous_gan').glob('*.yaml')))
...
def train(cfg):
    ...
    bundle = build_models(cfg, scaler, device)
    ...
    provenance = dict(sources=sources, source_archive_sha256=sha256(out / 'source.zip'),
        ...
        initial_parameters=hashes(bundle), initialization='All G/E/prior/D from scratch; no checkpoint or pretrained scaler',
    ...
    with (out / 'log.txt').open('w', buffering=1) as logfile, live.open('a', buffering=1) as livefile, \
        ...
        log('Scratch E(st, at-1, terrain) -> z; G1 -> st; G2 -> at; G3 -> st+1; GAN + action L2 throughout')
        sync()
        ...
        for step in range(1, cfg['steps'] + 1):
            ...
            task, terms = task_loss(decoded, real, cfg)
            ...
            if step in checkpoints:
                ...
                saved = dict(format='gym_previous_gan_v1', gan_steps=step, step=step,
