#!/usr/bin/env python
"""Plot the paired sparse-action control experiment and its validation curves."""
import argparse
import json
...
def plot(report_dir):
    ...
    order = [name for name in ("probes", "auxiliary", "full_label", "expert") if name in rows]
    labels = {"probes": "5 labeled episodes · detached probes", "auxiliary": "5 labeled episodes · joint auxiliary",
              "full_label": "47 labeled episodes · probe reference", "expert": "Heuristic expert"}
    colors = {"probes": "#478bc9", "auxiliary": "#1b9e77", "full_label": "#926ac1", "expert": "#bc8d32"}
    ...
    persistence = rows["probes"]["expert_diagnostics"]["persistence_next_state"]["standardized_continuous_mse"]
    axes[1].axhline(persistence, color="#333333", linestyle="--", label="Persistence on same records")
    for ax in axes:
        ax.set_xticks([0, 1], ["Detached probes", "Joint auxiliary"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-dir", default="reports/gym/lunar_lander_sparse_action")
