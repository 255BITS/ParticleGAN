...
BASE = ROOT / 'experiments/train_cifar_ae_scaling32.py'
BASE_SHA = 'bb0fa92a7233d2a240090b2366810c9f059d8cf08f5cf6a42a76272d78966b82'
assert hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA
from experiments import train_cifar_ae_scaling32 as base
...
def panel_child_seed(seed, child):
    # Preserve seeds of historical children0..15. New children use a separate
    # band so child16 cannot alias the next original parent's child0.
    return seed + child % 16 + (child // 16) * 100000
...
def generate_features(g, model, means, sigma, n, seed, batch_size):
    ...
    for i, mean in enumerate(means):
        stream = base.rng(panel_child_seed(seed, i))
...
def train(cfg):
    ...
    assert factor in (1, 4, 8, 16, 32) and cfg['parents'] <= parent['num_particles']
    ...
    for split, count, seed in [('train', cfg['train_draws'], 1000000), ('validation', cfg['val_draws'], 2000000),
        ...
        # First16 child streams match historical probes; further children get an independent seed band.
