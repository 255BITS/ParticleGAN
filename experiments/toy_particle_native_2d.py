...
module trained, no paired L2. `fixed` is the recipe now used by
gym_particle_finetune: Rp logistic on the live pair (record, z), sample-point
b_cap on that pair, and only E_control plus the action head trained.
...
def pretrain(log):
    recipe = get_recipe("mog", z_dim=Z, num_particles=K, total_steps=FINE, batch_size=B)
...
def run_arm(name, kind, recipe, init, log):
    ...
    for step in range(1, FINE + 1):
        ...
        if kind == "current":
            ...
            # Same pairing as lib.gym_particle_finetune.particle_game: prior fake vs live real pair.
