#!/usr/bin/env python
"""CIFAR AE-GAN generator-family and reconstruction-routing scouts.

Standalone entry point preserves historical source certificates. The transformer
is a smaller, deterministic TransGAN-style G; D remains our frozen ResNet18
feature critic. This is not a reproduction of the complete TransGAN recipe.
Architecture reference: VITA-Group/TransGAN, commit
6b85440ca56716fd7a60bac964466cc0296ce663, models_search/ViT_custom_rp.py
and exps/cifar_train.py (https://github.com/VITA-Group/TransGAN).
Reimplemented from the architecture: 8/16/32 grids, pixel shuffle, absolute and
relative positions, pre-normalized attention/MLPs. Differences from the official
CIFAR script include width512 instead of1024, latent64 instead of256, normalized
low-gain tanh RGB (the original RGB head is unbounded),
our D/objectives/particles/optimizer/EMA/data protocol, and PyTorch SDPA. No latent
normalization or stochastic G noise is added, preserving particle geometry.
"""
import argparse
import copy
import hashlib
...
def load_resume(cfg):
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('recon_weight', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_grad')
                    if old[k] != cfg[k]}}
