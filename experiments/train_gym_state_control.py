#!/usr/bin/env python
"""Matched scratch state-only control: detached probes versus auxiliary gradients."""
import argparse
import copy
import hashlib
import json
...
DEFAULTS = dict(arm="probes", steps=2500, batch_size=256, checkpoints=[250, 1000, 2500],
    ...
    preprocessing_checkpoint="results/gym/lunar_lander/adversarial/best.pt",
    episodes="results/gym/lunar_lander/data/episodes.json",
    out_dir="results/gym/lunar_lander_state_control/probes",
    live_log="results/gym/lunar_lander_state_control/live.log")
...
def capture_provenance(out, cfg, records, bundle, saved):
    paths = [Path(__file__), ROOT / "lib/gym_state_control.py", ROOT / "lib/gym_control.py",
    ...
    paths += sorted((ROOT / "configs/gym/lunar_lander_state_control").glob("*.yaml"))
    ...
    return dict(sources=sources, source_archive_sha256=sha256(out / "source.zip"),
        preprocessing_checkpoint=dict(path=cfg["preprocessing_checkpoint"], sha256=sha256(cfg["preprocessing_checkpoint"])),
        preprocessing_provenance=saved.get("provenance", {}),
        ...
        initial_parameters=parameter_hashes(bundle),
        normalization="Only scaler loaded from preprocessing checkpoint; original training split statistics",
        initialization="All E/G1/G2/G3/prior parameters freshly initialized; no pretrained model weights",
        loss_reductions="action: mean squared standardized error over batch x 2; each state: mean squared standardized error over batch x 6 + mean BCE logits over batch x 2; sum three weighted heads + default MoG prior regularizer")
...
def train(cfg):
    ...
    saved = torch.load(cfg["preprocessing_checkpoint"], map_location="cpu", weights_only=False)
    expected = saved.get("provenance", {}).get("dataset", {}).get("episodes.json")
    if expected is not None and sha256(cfg["episodes"]) != expected:
        raise ValueError("Expert episode source differs from preprocessing provenance")
    scaler = GymTransitionScaler(**saved["scaler"])
    bundle = build_state_models(cfg, scaler, device)
    records = build_expert_records(cfg["episodes"])
    # Previous actions are deliberately absent from this round's training records.
    records.pop("previous_actions")
    np.savez_compressed(out / "expert_records.npz", **records)
    provenance = capture_provenance(out, cfg, records, bundle, saved)
    values = {k: torch.as_tensor(records[k], device=device) for k in ("states", "actions", "next_states", "terrain")}
    ...
    def save(step):
        checkpoint = dict(format="gym_state_control_v1", config=cfg, recipe=recipe.to_dict(),
            ...
            data_draw_sha256=draws_digest.hexdigest(),
    ...
    with (out / "log.txt").open("w", buffering=1) as logfile, live.open("a", buffering=1) as livefile, \
        ...
        def log(message):
            ...
            logfile.write(text + "\n")
            livefile.write(text + "\n")
        log(f"START arm={cfg['arm']} steps={cfg['steps']} expert_records={len(records['states'])} device={device}")
        ...
        summary = dict(config=cfg, recipe=recipe.to_dict(), provenance=provenance,
            ...
            unique_training_records=len(records["states"]), real_draws=cfg["steps"] * cfg["batch_size"],
            generator_optimizer_record_draws=cfg["steps"] * cfg["batch_size"],
            discriminator_optimizer_record_draws=0, data_draw_sha256=draws_digest.hexdigest(), simulator_calls=0,
