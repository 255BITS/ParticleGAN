#!/usr/bin/env python
"""Grow SAGAN G and D from one to two spatial attention blocks at a checkpoint.

Standalone trainer preserves historical source certificates. Only the added
blocks use zero output initialization; existing G/D attention remains unchanged.
"""
import argparse
import copy
import hashlib
...
from lib.image_ddgan import ResBlock, ImageDiscriminator
...
def descendant_panel(g, prior, cfg, out, step, evaluator):
    ...
    children = max(4, factor)
    means = means.expand(-1, children, -1) if factor == 1 else means
    ...
    shaped = pixels.reshape(panel_count, children, 4, 3, 32, 32)
    save_image(shaped[:8].flatten(0, 2).float()/255, out/f'siblings_{step:06d}.png', nrow=children * 4)
    latent = means - means.mean(1, keepdim=True)
    result = {'parent_count': panel_count, 'children': children, 'draws_per_child': 4,
    ...
    def variation(x):
        x = x.double().reshape(panel_count, children, 4, -1)
...
DEFAULTS = {
    ...
    'z_dim': 64, 'num_particles': 1024, 'width': 32, 'sigma_rel': .025, 'fixed_sigma': 0.,
    ...
    'd_updates': 1, 'generator_arch': 'sagan_gd', 'recon_grad': 'all',
    ...
    'g_attention_depth': 2, 'd_attention_depth': 2,
}
...
def validate(cfg):
    for key in ('g_attention_depth', 'd_attention_depth'):
        if type(cfg[key]) is not int or cfg[key] not in (1, 2):
            raise ValueError(f'{key} must be 1 or 2')
    if cfg['generator_arch'] != 'sagan_gd':
        raise ValueError('attention growth requires sagan_gd')
    if type(cfg['expansion_factor']) is not int or cfg['expansion_factor'] not in (1, 4, 8, 16):
        raise ValueError('expansion_factor must be 1, 4, 8 or 16')
    ...
    for key in ('recon_weight', 'penalty_coeff', 'fixed_sigma'):
        if not math.isfinite(cfg[key]) or cfg[key] < 0:
    ...
    if cfg['generator_arch'] not in ('cnn', 'transgan', 'deconv', 'deconv_wide_norm', 'sagan_gd'):
        raise ValueError('generator_arch must be cnn, transgan, deconv, deconv_wide_norm or sagan_gd')
    ...
    if cfg['generator_arch'] in ('transgan', 'deconv', 'deconv_wide_norm', 'sagan_gd') and (cfg['g_width'] != 0 or cfg['g_depth'] != 1):
        raise ValueError('CNN g_width/g_depth overrides do not apply to this architecture')
    if not 0 <= cfg['ema'] < 1:
...
class DeconvGenerator(torch.nn.Module):
    """Linear 4x4 seed followed by three stride-two deconvolutions; no norms."""
    def __init__(self, z_dim=64):
        super().__init__()
        self.input = torch.nn.Linear(z_dim, 128 * 4 * 4)
    ...
    def forward(self, z):
        h = self.input(z).relu().reshape(-1, 128, 4, 4)
        return self.output(h)


class DeconvWideNormGenerator(torch.nn.Module):
    """256-channel 4x4 projection and 128/64-channel hidden deconvolutions."""
    def __init__(self, z_dim=64):
        super().__init__()
        self.input = torch.nn.Linear(z_dim, 256 * 4 * 4)
        self.input_norm = torch.nn.GroupNorm(8, 256)
        self.output = torch.nn.Sequential(
    ...
    def forward(self, z):
        h = self.input(z).reshape(-1, 256, 4, 4)
        return self.output(self.input_norm(h).relu())


class SAGANAttention(torch.nn.Module):
    """Single-head nonlocal attention with unscaled dots and an active residual.

    Manual matrix products keep input-gradient double backward available on CUDA.
    Bias-free projections avoid redundant key biases that cancel in softmax.
    """
    def __init__(self, channels):
        super().__init__()
        self.query = torch.nn.Conv2d(channels, max(1, channels // 8), 1, bias=False)
        self.key = torch.nn.Conv2d(channels, max(1, channels // 8), 1, bias=False)
        self.value = torch.nn.Conv2d(channels, max(1, channels // 2), 1, bias=False)
        self.project = torch.nn.Conv2d(max(1, channels // 2), channels, 1, bias=False)
...
class SAGANGenerator(DeconvWideNormGenerator):
    """Keep baseline layer names; insert attention after the 16x16 GN/ReLU."""
    def forward(self, z):
        ...
        for index, layer in enumerate(self.output):
            ...
            if index == 5:
                h = self.attention(h)
                if hasattr(self, 'attention_extra'):
                    h = self.attention_extra(h)
        return h


class SAGANPixelDiscriminator(ImageDiscriminator):
    """Existing pixel critic with attention after its first block and pooling."""
...
def attention_metadata(channels):
    return {'kind': 'SAGAN-style single-head spatial nonlocal attention',
            'resolution': 16, 'channels': channels,
            'query_key_channels': max(1, channels // 8),
            'value_channels': max(1, channels // 2),
            'dot_product_scaling': None, 'projection_bias': False,
            'residual_coefficient': 1., 'learnable_gate': False,
            'phase_in': False, 'initialization': 'PyTorch nonzero Conv2d defaults',
            'construction_seed': 124003, 'implementation': 'manual bmm and softmax',
            'spectral_normalization': False}


@torch.no_grad()
def configure_sigma(prior, cfg):
    """Zero means calibrated sigma; positive values request a fixed override."""
    calibrated = float(prior.sigma)
    if cfg['fixed_sigma'] > 0:
        prior.sigma.fill_(cfg['fixed_sigma'])
    prior._noise_enabled = bool(prior.sigma > 0)
    return {'calibrated_sigma': calibrated, 'fixed_sigma_requested': cfg['fixed_sigma'],
            'actual_sigma': float(prior.sigma),
            'mode': 'fixed_override' if cfg['fixed_sigma'] > 0 else 'calibrated'}


def generator_metadata(cfg):
    if cfg['generator_arch'] == 'sagan_gd':
        result = generator_metadata({**cfg, 'generator_arch': 'deconv_wide_norm'})
        result.update(architecture='sagan_gd', attention=attention_metadata(64),
                      implementation='SAGAN-style G/D attention adaptation of AE-GAN; not full SAGAN')
        result.update(attention_depth=cfg['g_attention_depth'], extra_attention_initialization='zero output projection; no ramp')
        return result
    if cfg['generator_arch'] == 'deconv_wide_norm':
        return {'architecture': 'deconv_wide_norm', 'latent_dim': cfg['z_dim'],
                'projection_shape': [256, 4, 4], 'deconv_channels': [128, 64, 3],
                'kernel_size': 4, 'stride': 2, 'padding': 1,
                'hidden_activation': 'ReLU', 'output_activation': 'Tanh',
                'normalization': 'GroupNorm', 'normalization_groups': 8,
                'normalized_channels': [256, 128, 64], 'output_normalization': None,
                'latent_conditioning': 'input_projection_only',
                'initialization': 'PyTorch Linear, ConvTranspose2d and GroupNorm defaults',
                'implementation': 'Wider deconvolutional G with hidden GroupNorm and existing AE-GAN recipe'}
    if cfg['generator_arch'] == 'deconv':
        return {'architecture': 'deconv', 'latent_dim': cfg['z_dim'],
                'projection_shape': [128, 4, 4], 'deconv_channels': [64, 32, 3],
                'kernel_size': 4, 'stride': 2, 'padding': 1,
                'hidden_activation': 'ReLU', 'output_activation': 'Tanh',
                'normalization': None, 'latent_conditioning': 'input_projection_only',
                'initialization': 'PyTorch Linear and ConvTranspose2d defaults',
                'implementation': 'Plain deconvolutional G with existing AE-GAN recipe'}
    if cfg['generator_arch'] == 'cnn':
        return {'architecture': 'cnn', 'cnn_width': cfg['g_width'] or cfg['width'],
                'cnn_depth': cfg['g_depth'], 'normalization': 'GroupNorm',
                'implementation': 'Residual convolutional G with latent conditioning'}
    return {'architecture': 'transgan', 'transformer_dim': cfg['transgan_dim'],
            'transformer_depths': cfg['transgan_depths'], 'transformer_heads': cfg['transgan_heads'],
            'transformer_mlp_ratio': cfg['transgan_mlp_ratio'],
...
def build_models(cfg):
    ...
    if cfg['generator_arch'] in ('deconv_wide_norm', 'sagan_gd'):
        with torch.random.fork_rng(devices=[]):
            g = DeconvWideNormGenerator(cfg['z_dim'])
    elif cfg['generator_arch'] == 'deconv':
        with torch.random.fork_rng(devices=[]):
            g = DeconvGenerator(cfg['z_dim'])
    elif cfg['generator_arch'] == 'transgan':
        with torch.random.fork_rng(devices=[]):
    ...
    d, e = DirectDiscriminator(cfg['width']), build_encoder(cfg)
    if cfg['generator_arch'] == 'sagan_gd':
        # All historical tensors and the shared stream stay bit-identical.
        # Seed only CPU RNG: models are constructed on CPU before .cuda().
        with torch.random.fork_rng(devices=[]):
            torch.random.default_generator.manual_seed(124003)
            g.__class__ = SAGANGenerator
            g.attention = SAGANAttention(64)
            d.critic.pixel.__class__ = SAGANPixelDiscriminator
            d.critic.pixel.attention = SAGANAttention(2 * cfg['width'])
    if cfg['g_attention_depth'] == 2:
        with torch.random.fork_rng(devices=[]):
            torch.random.default_generator.manual_seed(124004)
            g.attention_extra = SAGANAttention(64)
            torch.nn.init.zeros_(g.attention_extra.project.weight)
    if cfg['d_attention_depth'] == 2:
        with torch.random.fork_rng(devices=[]):
            torch.random.default_generator.manual_seed(124005)
            d.critic.pixel.attention_extra = SAGANAttention(2 * cfg['width'])
            torch.nn.init.zeros_(d.critic.pixel.attention_extra.project.weight)
    return g, d, e


def grow_checkpoint(ck, model, optimizer, side='G'):
    """Add identity block weights and migrate Adam IDs without resetting old state."""
    assert side in ('G', 'D')
    prefix = 'attention_extra.' if side == 'G' else 'critic.pixel.attention_extra.'
    names = [name for name, p in model.named_parameters() if p.requires_grad]
    old_names = [name for name in names if not name.startswith(prefix)]
    added = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()
             if k.startswith(prefix)}
    assert len(added) == 4 and added[prefix + 'project.weight'].count_nonzero() == 0
    for key in (('G', 'ema_G') if side == 'G' else ('D',)):
        assert not any(k.startswith(prefix) for k in ck[key])
        ck[key].update({k: v.clone() for k, v in added.items()})
    optimizer_key = 'optimizer_g' if side == 'G' else 'optimizer_d'
    old = ck[optimizer_key]
    target = optimizer.state_dict()
    assert len(old['param_groups']) == len(target['param_groups']) == (3 if side == 'G' else 1)
    migrated = {'state': {}, 'param_groups': []}
    for i, (before, after) in enumerate(zip(old['param_groups'], target['param_groups'])):
        if i == 0:
            assert len(before['params']) == len(old_names)
            assert len(after['params']) == len(names)
            ids = dict(zip(names, after['params']))
            pairs = zip(before['params'], (ids[name] for name in old_names))
        else:
            assert len(before['params']) == len(after['params'])
            pairs = zip(before['params'], after['params'])
        for old_id, new_id in pairs:
            if old_id in old['state']:
                migrated['state'][new_id] = old['state'][old_id]
        migrated['param_groups'].append({**before, 'params': after['params']})
    assert len(migrated['state']) == len(old['state'])
    ck[optimizer_key] = migrated
    return {'added_parameters': list(added), 'added_parameter_count': sum(v.numel() for v in added.values()),
            'existing_adam_states_preserved': len(old['state']), 'new_adam_states': 'fresh',
            'initial_function': 'identity via zero output projection', 'existing_parameters': 'unchanged'}
...
def load_resume(cfg):
    ...
    allowed = {'g_attention_depth', 'd_attention_depth', 'expansion_factor', 'initial_eval_samples', 'steps', 'out_dir', 'log_interval', 'eval_interval', 'eval_samples', 'final_samples',
    ...
    for key in ('g_attention_depth', 'd_attention_depth'):
        old[key] = ck['config'].get(key, 1)
        if not old[key] <= cfg[key] <= 2:
            raise ValueError('cannot remove existing attention blocks')
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('g_attention_depth', 'd_attention_depth', 'expansion_factor', 'recon_weight', 'g_lr_scale', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_grad', 'reg_every', 'penalty_coeff', 'd_warmstart', 'd_warmstart_sha256')
                    if old[k] != cfg[k]}}
...
def train(cfg):
    ...
    sigma_metadata = configure_sigma(prior, cfg)
    ...
    if checkpoint is not None:
        grow_g = checkpoint['config'].get('g_attention_depth', 1) < cfg['g_attention_depth']
        grow_d = checkpoint['config'].get('d_attention_depth', 1) < cfg['d_attention_depth']
        growing = grow_g or grow_d
        expected_initial_hash = initial_hash
        if growing:
            base_g, base_d = copy.deepcopy(g), copy.deepcopy(d)
            if grow_g:
                del base_g.attention_extra
            if grow_d:
                del base_d.critic.pixel.attention_extra
            expected_initial_hash = state_hash([base_g, base_d, e, prior])
            del base_g, base_d
        if checkpoint['initialization_sha256'] != expected_initial_hash:
            raise ValueError('checkpoint initialization differs from this model/configuration')
        if growing:
            resume_info['attention_growth'] = {}
            if grow_g:
                resume_info['attention_growth']['G'] = grow_checkpoint(checkpoint, g, og, 'G')
            if grow_d:
                resume_info['attention_growth']['D'] = grow_checkpoint(checkpoint, d, od, 'D')
    ...
    metadata = {'initialization_sha256': initial_hash, 'sigma': float(prior.sigma),
                'sigma_configuration': sigma_metadata,
    ...
    metadata['generator'] = generator_metadata(cfg)
    if cfg['generator_arch'] == 'sagan_gd':
        metadata['discriminator_attention'] = {
            **attention_metadata(2 * cfg['width']),
            'depth': cfg['d_attention_depth'],
            'extra_attention_initialization': 'zero output projection; no ramp',
            'placement': 'trainable pixel branch, after first residual block and pooling',
            'pretrained_branch': 'unchanged frozen ResNet18 features',
            'score_mixing': 'unchanged pixel plus pretrained feature heads'}
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as log:
        for step in range(start_step + 1, cfg['steps'] + 1):
            ...
            if step == start_step + 1 or step % cfg['log_interval'] == 0 or step == cfg['steps']:
                ...
                if cfg['generator_arch'] == 'sagan_gd':
                    for name, module in [('G_attention', g.attention),
                                         ('D_attention', d.critic.pixel.attention)]:
                        grad_norms[name] = float(sum((p.grad.detach().square().sum()
                            for p in module.parameters() if p.grad is not None),
                            gl.new_zeros(())).sqrt())
                if hasattr(g, 'attention_extra'):
                    grad_norms['G_attention_extra'] = float(sum((p.grad.detach().square().sum()
                        for p in g.attention_extra.parameters() if p.grad is not None), gl.new_zeros(())).sqrt())
                if hasattr(d.critic.pixel, 'attention_extra'):
                    grad_norms['D_attention_extra'] = float(sum((p.grad.detach().square().sum()
                        for p in d.critic.pixel.attention_extra.parameters() if p.grad is not None), gl.new_zeros(())).sqrt())
