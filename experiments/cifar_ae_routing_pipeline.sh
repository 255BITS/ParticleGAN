...
set -euo pipefail
cd "$(dirname "$0")/.."
TRACK="${1:?Specify routing_scout or routing_200k}"
GPUS="${2:-0,1}"
case "$TRACK" in routing_scout|routing_200k) ;; *) exit 2 ;; esac
...
.venv/bin/python -u experiments/follow_grid.py --root "$RUN_ROOT" --log "$RUN_ROOT/PIPELINE.log" -- \
  ...
  --trainer experiments/train_cifar_ae_routing.py || status=$?
.venv/bin/python -u experiments/analyze_cifar_ae_plateau.py --config_manifest "$MANIFEST" \
  --report "$REPORT" --trainer experiments/train_cifar_ae_routing.py >> "$RUN_ROOT/PIPELINE.log" 2>&1 || status=$?
