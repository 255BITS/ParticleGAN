#!/usr/bin/env python
"""Continue the certified wider GroupNorm deconv checkpoint to 80k on GPU 0."""
import hashlib
import json
...
TRACK = 'deconv_wide_norm_16k_80k'
...
def prepare():
    parent_report = ROOT / 'reports/cifar-particle-ae/deconv_wide_norm_16k_scout'
    ...
    cfg = {**parent['config'], 'steps': 80000, 'eval_interval': 5000,
           'eval_samples': 50000, 'final_samples': 50000, 'initial_eval_samples': 0,
           'max_train_seconds': 10800., 'keep_checkpoints': True,
...
def run():
    ...
    assert r['start_step'] == 40000 and r['final']['step'] == 80000
    ...
    assert [x['step'] for x in curve] == list(range(45000, 80001, 5000))
    ...
    for name, step, fid in [('final', 80000, r['final']['fid']),
    ...
    lines = ['# Wider GroupNorm deconv 16k, continuation to 80k', '',
             '| Step | FID50k |', '|---|---:|']
