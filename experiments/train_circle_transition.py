...
DEFAULTS = dict(
    ...
    rho_low=0.8, rho_high=1.2, on_circle_rate=0.5, rho_curriculum=False,
...
def validate(cfg):
    ...
    if type(cfg["rho_curriculum"]) is not bool:
        raise ValueError("rho_curriculum must be a bool")
...
def train(cfg):
    ...
    with (out / "log.txt").open("w", buffering=1) as logfile, live.open("a", buffering=1) as livefile, \
        ...
        for step in range(1, cfg["finetune_steps"] + 1):
            ...
            train_rho = (0.5, 1.5) if step <= curriculum_until else (cfg["rho_low"], cfg["rho_high"])
        ...
        for module in (bundle["G"], bundle["E"], bundle["E_control"], bundle["D"], bundle["prior"]):
            module.eval()
            module.requires_grad_(False)
        if cfg["save_checkpoint"]:
            torch.save(dict(format="circle_transition_v1", config=cfg, protocol=protocol(),
