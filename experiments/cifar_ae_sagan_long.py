#!/usr/bin/env python
"""Continue the certified SAGAN G/D 40k checkpoint to 200k on GPU 1."""
import argparse
import hashlib
import json
...
def prepare():
    folder = ROOT / 'reports/cifar-particle-ae/sagan_gd_16k_scout'
    ...
    assert ck['step'] == 40000 and audit['interventions'] == {}
...
def run():
    ...
    pipeline.write(REPORT / 'STATUS.json', {'status': 'running', 'gpu': 1,
                                           'start_step': 40000, 'target_step': 200000})
    ...
    assert [x['step'] for x in curve] == list(range(45000, 200001, 5000))
    ...
    lines = ['# SAGAN G/D 16k, unchanged continuation to 200k', '',
             '| Step | FID50k |', '|---|---:|']
    ...
    lines += ['', f"Best sampled: {best['generation']['fid']:.4f} at {best['step']}; "
              ...
              'These are contextual references, not matched 200k architecture comparisons.']
    ...
    (REPORT / 'FINDINGS.md').write_text('# Duration result\n\n' + verdict +
        f" Endpoint minus180k FID: {late:+.4f}. Endpoint minus40k: "
    ...
    pipeline.write(REPORT / 'STATUS.json', {'status': 'complete', 'final_step': 200000,
...
if __name__ == '__main__':
    ...
    try:
        if args.preflight:
            prepare()
            print('PREFLIGHT certified: unchanged 40k full-state checkpoint -> 200k', flush=True)
        else:
            run()
