#!/usr/bin/env python
"""Plain deconvolutional CIFAR generator with the existing particle AE-GAN recipe.

Standalone trainer preserves historical source certificates. The deconv generator
has one latent projection, three transposed convolutions, and no normalization.
...
DEFAULTS = {
    ...
    'd_updates': 1, 'generator_arch': 'deconv', 'recon_grad': 'all',
...
def validate(cfg):
    ...
    if cfg['generator_arch'] not in ('cnn', 'transgan', 'deconv'):
        raise ValueError('generator_arch must be cnn, transgan or deconv')
    ...
    if cfg['generator_arch'] in ('transgan', 'deconv') and (cfg['g_width'] != 0 or cfg['g_depth'] != 1):
