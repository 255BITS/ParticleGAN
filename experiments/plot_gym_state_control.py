#!/usr/bin/env python
"""Plot the paired state-only control experiment and its validation curves."""
import argparse
import json
...
def plot(report_dir):
    ...
    order = [name for name in ("probes", "auxiliary", "imitation", "expert") if name in rows]
    labels = {"probes": "State-only + detached probes", "auxiliary": "State-only + joint auxiliary",
              "imitation": "Pretrained imitation reference", "expert": "Heuristic expert"}
    colors = {"probes": "#478bc9", "auxiliary": "#1b9e77", "imitation": "#926ac1", "expert": "#bc8d32"}
    ...
    fig.suptitle("Does auxiliary learning help land? · 50 paired held-out worlds\nCheckpoints selected on separate validation episodes", fontsize=13)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-dir", default="reports/gym/lunar_lander_state_control")
