...
def configs(track, smoke):
    ...
    paths=[]
    for name,factor in [('split_8192',8),('split_16384',16)]:
...
def analyze(track):
    ...
    if not track.endswith('smoke'):
        previous = json.loads((ROOT/'reports/cifar-particle-ae/particle_expansion_scout/results.json').read_text())
        comparison = sorted(previous + rows, key=lambda r:r['config']['expansion_factor'])
        (report/'scaling_curve.json').write_text(json.dumps(comparison,indent=2)+'\n')
        lines += ['', '## Existing matched-parent benchmarks', '', '| Centers | FID15k | FID20k | Train minutes |', '|---|---:|---:|---:|']
        for r in comparison:
            lines.append(f"| {1024*r['config']['expansion_factor']} | {r['curve'][0]['generation']['fid']:.4f} | {r['final']['fid']:.4f} | {r['train_seconds']/60:.2f} |")
        winner=min(rows,key=lambda r:r['final']['fid'])
        baseline=next(r for r in previous if r['config']['expansion_factor']==4)
        sustained=all(a['generation']['fid']<b['generation']['fid'] for a,b in zip(winner['curve'],baseline['curve']))
        finding=(f"{winner['name']} beats the previous 4096 arm at both sampled steps." if sustained else 'No new arm selected by final FID beats the previous 4096 arm at both sampled steps; inspect trajectories.')
        recommendation=('Continue the best larger-count arm alongside the existing 4096 trajectory to compare longer-term behavior.' if sustained else 'Do not assume monotonic scaling; examine per-center exposure, differentiation and adaptation before choosing the next count or duration.')
        (report/'FINDINGS.md').write_text('# Particle-count scaling\n\n'+finding+'\n\n'+recommendation+'\n\nSame original 10k checkpoint and evaluation protocol; previous 1024/4096 benchmarks reused, not rerun. Production CUDA nondeterminism is not estimated by this design. Review scaling_curve.json and sibling diagnostics; no automatic long promotion.\n')
...
if __name__=='__main__':
    ...
    track='particle_scaling_smoke' if a.smoke else 'particle_scaling_scout'
