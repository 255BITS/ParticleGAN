...
set -euo pipefail
cd "$(dirname "$0")/.."
TRACK="${1:-transgan_scout}"
GPUS="${2:-0,1}"
case "$TRACK" in transgan_scout|transgan_smoke) ;; *) exit 2 ;; esac
...
.venv/bin/python -u experiments/follow_grid.py --root "$RUN_ROOT" --log "$RUN_ROOT/PIPELINE.log" -- \
  ...
  --trainer experiments/train_cifar_ae_transgan.py || status=$?
if [[ "$TRACK" == transgan_scout ]]; then
  .venv/bin/python -u experiments/analyze_cifar_ae_transgan.py --config_manifest "$MANIFEST" \
