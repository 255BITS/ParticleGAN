...
set -euo pipefail
cd "$(dirname "$0")/.."
TRACK="${1:-discriminator_joint}"
GPUS="${2:-0,1}"
case "$TRACK" in discriminator_joint|discriminator_joint_smoke) ;; *) exit 2 ;; esac
...
.venv/bin/python -u experiments/follow_grid.py --root "$RUN_ROOT" --log "$RUN_ROOT/PIPELINE.log" -- \
  ...
  --python .venv/bin/python --trainer experiments/train_cifar_ae_discriminator.py
