#!/usr/bin/env python
"""Instrument eight original joint updates without changing their update order.
...
def run(parent,out):
    ...
    def step(optimizer,*args,**kwargs):
        ...
        measure('after_D' if len(optimizer.param_groups)==1 else 'after_G_prior')
        return value
    ...
    result={'parent':str(parent),'parent_sha256':diag.digest(parent),'script_sha256':diag.digest(__file__),
            ...
            'note':'Live weights, production precision. Same held-out real images and same prior ID/noise draws at each phase. Probe preserves weights, flags and global RNG. Trace has no comparable FID or training-speed measurement.'}
