...
def main():
    ...
    g,d=ImageGenerator(cfg).cuda().eval(),ImageDiscriminator(cfg).cuda().eval()
    ...
    result={'step':ck['step'],'config':cfg,'prior_std':float(prior.table.detach().std(0).mean()),'timesteps':[]}
