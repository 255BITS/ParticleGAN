#!/usr/bin/env python
"""AE-GAN reconstruction gradient routing, with audited continuation interventions.
...
DEFAULTS = {
    ...
    'd_updates': 1, 'recon_prior_grad': False, 'recon_generator_grad': True,
}
...
def validate(cfg):
    ...
    for key in ('recon_prior_grad', 'recon_generator_grad'):
...
def load_resume(cfg):
    ...
    allowed = {'steps', 'out_dir', 'log_interval', 'eval_interval', 'eval_samples', 'final_samples',
               ...
               'recon_prior_grad', 'recon_generator_grad'}
    old = {**DEFAULTS, **ck['config']}
    # Historical trainers sent reconstruction gradients to G and the prior.
    for key in ('recon_prior_grad', 'recon_generator_grad'):
        old[key] = ck['config'].get(key, True)
    def immutable(c):
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('recon_weight', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_prior_grad', 'recon_generator_grad')
                    if old[k] != cfg[k]}}
...
def reconstruction_loss(g, e, prior, real, cfg):
    ...
    if not cfg['recon_generator_grad']:
