#!/usr/bin/env python
"""Checkpoint joint continuations testing D regularization or D-only warmup.

Standalone copy preserves historical source certificates and the original joint
...
def load_resume(cfg):
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('recon_weight', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_grad', 'reg_every', 'penalty_coeff', 'd_warmstart', 'd_warmstart_sha256')
                    if old[k] != cfg[k]}}
