...
def initialize(pg, models, depth, steps):
    ...
    # Develop's current defaults differ from the measured release recipe.
    # Restore the complete resolved recipe rather than selecting moving defaults.
    from particlegan import Recipe
    fields = json.loads(Path(__file__).with_name('recipe.json').read_text())
    recipe = Recipe(**fields).replace(total_steps=steps)
...
def main():
    ...
    parser.add_argument('--particlegan-root', type=Path, default=Path(__file__).resolve().parents[2])
    ...
    paths = [source_root / 'examples/100gaussians.py', source_root / 'lib/toy_models.py',
             *sorted((source_root / 'particlegan').glob('*.py')), Path(__file__).resolve(),
             Path(__file__).with_name('recipe.json')]
