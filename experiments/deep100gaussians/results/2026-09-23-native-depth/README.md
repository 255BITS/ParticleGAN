...
Measured runner commit: `f9eb52ff` in HyperGAN, before relocation to ParticleGAN.
The exact measured runner is preserved in `runner-at-measurement.py` as a source
snapshot; use the current runner above for new experiments. ParticleGAN checkout:
...
Moving this research did not rerun the 7k experiments. The current adapter pins
their complete recipe explicitly because ParticleGAN develop now has different
defaults. A relocation test verifies the recipe and all three initial model
identities against this archive. Historical paths in provenance identify the
original execution; they have intentionally not been rewritten.
...
Regenerate the comparison with `experiments/deep100gaussians/summarize.py REPORT_ROOT`.
