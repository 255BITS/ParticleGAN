...
Research handoff: [HANDOFF.md](HANDOFF.md). The objective is a training method
robust across generator depth and form, not a one-off fix for this particular MLP.
...
The current checkout is selected automatically. `recipe.json` pins the complete
recipe used in the recorded study: ParticleGAN develop now has different
`gan_v3` defaults. This experiment deliberately does not inherit those changes.
...
loop with the pinned recipe across four updates including its annealing transition. They also verify
...
The relocation also checks all three initial model hashes and the full recipe
against the archived experiment.
