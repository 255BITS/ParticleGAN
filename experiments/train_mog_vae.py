...
DEFAULTS = {
    ...
    'obs_sigma': .1, 'temperature': .0025, 'seed': 24002,
    'steps': 6000, 'batch_size': 256, 'width': 128, 'num_particles': 400,
    'sigma_rel': .025, 'lr': .0006, 'd_lr': .0009, 'prior_lr': .006,
    'spread_weight': 1., 'log_interval': 250, 'eval_interval': 2000,
    'eval_samples': 20000, 'final_samples': 100000, 'recon_samples': 8192,
    'max_train_seconds': 300., 'out_dir': 'runs/mog_vae/default',
}
...
def sampled_codes(e, x, prior, cfg, generator, draws):
    means = prior.means()
    logq, u, log_s = e(x, means, cfg['temperature'])
    ids = torch.multinomial(logq.exp(), draws, replacement=True, generator=generator)
    noise = torch.randn((len(x), draws, 2), device=x.device, generator=generator)
    local = cfg['posterior'] == 'local'
    offsets = u[:, None] + log_s.exp()[:, None] * noise if local else noise
    return means[ids] + prior.sigma * offsets, ids, logq, u, log_s
...
def evaluate(g, e, prior, cfg, out, step):
    ...
    for chunk in x.split(512):
        if cfg['posterior'] == 'ae':
            ...
            kc, kl = posterior_kl(logq, u, log_s, cfg['posterior'] == 'local')
...
def validate(cfg):
    if set(cfg) != set(DEFAULTS) or cfg['posterior'] not in ('none', 'ae', 'categorical', 'local'):
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as history:
        for step in range(1, cfg['steps'] + 1):
            ...
            if cfg['posterior'] == 'ae':
                ...
                reconstruction_loss = two_draw_loss(cost, logq.gather(1, ids))
                rec = cost.mean()
                categorical, local = posterior_kl(logq, u, log_s, cfg['posterior'] == 'local')
                kc, kl = categorical.mean(), local.mean()
            loss = cfg['gan_weight'] * gl + reconstruction_loss + cfg['obs_sigma']**2 * cfg['kl_weight'] * (kc + kl) + cfg['spread_weight'] * spread(prior.z)
