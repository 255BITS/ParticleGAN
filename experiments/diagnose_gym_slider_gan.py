#!/usr/bin/env python
"""Compare slider-error and L2 GANs on identical saved inputs; no rollouts."""
import inspect
...
from experiments.evaluate_gym_slider_gan import REPORTS, verify_protocol, diagnostics
...
from lib.gym_slider_gan import load_checkpoint
...
def main():
    ...
    checkpoint = ROOT/'results/gym/lunar_lander_slider_gan/sliders_all/best.pt'
    board = json.loads((REPORTS/'leaderboard.json').read_text())
    l2_checkpoint = Path(next(r for r in board['gan_rows'] if r['arm']=='previous_l2')['test']['checkpoint'])
    new = load_checkpoint(checkpoint)
    l2 = load_l2(l2_checkpoint)
    for arm, path in (('sliders_all', checkpoint), ('previous_l2', l2_checkpoint)):
        row = next(r for r in board['gan_rows'] if r['arm'] == arm)
        if sha256(path) != row['test']['checkpoint_sha256']:
            raise RuntimeError('Diagnostic checkpoint differs from evaluated model')
    bundles = dict(sliders_all={**new, 'E_control': new['E']}, previous_l2={**l2, 'E_control': l2['E']})
    output = dict(kind='Posthoc fixed-input diagnostics; no training or selection',
        ...
        checkpoints=dict(sliders_all=sha256(checkpoint), previous_l2=sha256(l2_checkpoint)),
        ...
        expert={}, fixed_traces={}, predictions=dict(sliders_all=diagnostics(new), previous_l2=diagnostics(l2)))
    ...
    try:
        for source, filename in [('sliders_all','sliders_selected_test.json'), ('previous_l2','l2_selected_test.json')]:
