#!/usr/bin/env python
"""GAN-only paired control leaderboard: joint versus joint plus marginal critics."""
import argparse
import hashlib
import json
import logging
...
from experiments.evaluate_gym_control import write_json, verify_control_checkpoint, expert_action_diagnostics
from experiments.evaluate_gym_state_control import auxiliary_metrics, rollout_auxiliary, check_disjoint

REPORTS = ROOT/'reports/gym/lunar_lander_gan_control'
EPISODES = ROOT/'results/gym/lunar_lander/data/episodes.json'
REFERENCE = ROOT/'results/gym/lunar_lander_control/joint/best.pt'
ALLOWED_ARMS = ('joint','marginals','legacy_joint')
...
def freeze_protocol(path, episodes_path, reference):
    ...
    validation, test = list(range(991000,991020)), list(range(1091000,1091050))
    used = check_disjoint(json.loads(episodes_path.read_text()), previous, validation, test)
    source_names = ['experiments/evaluate_gym_gan_control.py', 'experiments/evaluate_gym_sparse_action.py',
        'experiments/train_gym_gan_control.py','lib/gym_gan_control.py', 'experiments/evaluate_gym_state_control.py',
    ...
    configs = sorted((ROOT/'configs/gym/lunar_lander_gan_control').glob('*.yaml'))
    ...
    from experiments.train_gym_gan_control import DEFAULTS
    make_controller('legacy_joint',reference,'cpu')
    value = dict(version=1, supervision=dict(selection=selection,
        ...
        training_config={k:DEFAULTS[k] for k in ('seed','num_particles','z_dim','width','encoder_width','context_dim',
            'lambda_state','lambda_next','continuous_weight','contact_weight','adversarial_weight','marginal_weight',
            'd_width','marginal_width','labeled_episode_count')},
        eligible_arms=list(ALLOWED_ARMS), eligibility='New candidates must carry trained discriminators and GAN updates at every step from initialization; legacy reference is the previously verified joint GAN only',
        legacy_reference='Pretrained joint GAN fine-tuned with all 47 expert episodes; different history and labels, not a matched control',
...
def verify_legacy_gan(bundle, checkpoint):
    if bundle['config'].get('arm')!='joint' or bundle.get('D') is None:
        raise ValueError('Legacy reference must be the adversarial joint controller, never imitation')
    if bundle.get('world_config',{}).get('arm')!='adversarial':
        raise ValueError('Legacy reference lacks adversarial world-model pretraining')
    if not {'experiments/train_gym_control.py','experiments/train_gym_transition.py'} <= set(bundle['provenance']['sources']):
        raise ValueError('Legacy GAN training source provenance missing')
    known_selection=json.loads((ROOT/'reports/gym/lunar_lander_control/selections/joint.json').read_text())
    if sha256(checkpoint)!=known_selection['validation']['checkpoint_sha256']:
        raise ValueError('Legacy reference is not the known validation-selected joint GAN')
    verify_control_checkpoint(bundle,checkpoint)
    parameters=list(bundle['D'].parameters())
    if not parameters or not all(torch.isfinite(p).all() for p in parameters):
        raise RuntimeError('Legacy discriminator weights missing or nonfinite')


def verify_gan_training(bundle):
    cfg,p,summary=bundle['config'],bundle['provenance'],bundle.get('training_summary',{})
    if cfg.get('adversarial_weight',0)<=0 or (cfg.get('arm')=='marginals' and cfg.get('marginal_weight',0)<=0):
        raise ValueError('Generator adversarial loss must remain active')
    if cfg.get('arm') not in ('joint','marginals') or bundle.get('gan_training') is not True:
        raise ValueError('Candidate did not train as an eligible GAN')
    if bundle.get('gan_steps')!=bundle['step'] or bundle['step']<=0:
        raise ValueError('GAN updates must cover every training step')
    if p.get('gan_training') is not True or summary.get('gan_training') is not True or summary.get('gan_steps')!=cfg['steps']:
        raise ValueError('Completed training provenance does not establish GAN throughout')
    if 'experiments/train_gym_gan_control.py' not in p['sources'] or 'lib/gym_gan_control.py' not in p['sources']:
        raise ValueError('Candidate lacks the verified GAN implementation source')
    d=bundle.get('D')
    expected={'joint','action','state'} if cfg['arm']=='marginals' else {'joint'}
    if d is None or set(d.critics)!=expected:
        raise ValueError('Required trained discriminator weights are missing')
    from lib.gym_state_control import parameter_hashes
    for name,critic in d.critics.items():
        parameters=list(critic.parameters())
        if not parameters or not all(torch.isfinite(v).all() for v in parameters):
            raise ValueError('Discriminator contains missing or nonfinite parameters')
        initial=p['initial_parameters'].get('D.'+name)
        current=parameter_hashes(dict(G=critic,E=critic,prior=critic))['G']
        if not initial or current==initial:
            raise ValueError('Discriminator has not changed from initialization')


def gan_eligibility(bundle, arm, checkpoint):
    if arm=='legacy_joint':
        return dict(eligible=True,kind='known pretrained joint GAN',gan_steps=bundle['step'],
            matched_to_new_arms=False,labeled_episodes=47,checkpoint_sha256=sha256(checkpoint),
            evidence='Original joint training source/archive/checkpoint provenance and discriminator weights verified')
    return dict(eligible=True,kind='GAN from first update',gan_steps=bundle['gan_steps'],
        matched_to_new_arms=True,labeled_episodes=len(bundle['provenance']['labeled_data']['episode_ids']),
        discriminator_roles=list(bundle['D'].critics),checkpoint_sha256=sha256(checkpoint),
        evidence='Known GAN source/archive, every-step GAN count, finite discriminator weights changed from initialization')


def verify_checkpoint(bundle, checkpoint):
    verify_gan_training(bundle)
...
def make_controller(arm, checkpoint, device):
    if arm not in ALLOWED_ARMS:
        raise ValueError('GAN-only leaderboard excludes non-GAN controllers')
    if arm=='legacy_joint':
        ...
        verify_legacy_gan(bundle,checkpoint)
    ...
    from lib.gym_state_control import control_action_details
    bundle=load_gan_control_checkpoint(checkpoint,device)
    verify_checkpoint(bundle,checkpoint)
    if bundle['config']['arm']!=arm:
        raise ValueError('Requested GAN arm differs from checkpoint')
    return lambda env,s,a,c:control_action_details(bundle,s,c),bundle


def score(arm, checkpoint, split, name, out, protocol, device):
    if arm not in ALLOWED_ARMS: raise ValueError('GAN-only evaluation rejects this arm')
    destination=out/'evaluations'/f'{name}_{split}.json'
    digest=sha256(checkpoint) if checkpoint else None
    if destination.exists():
        make_controller(arm,checkpoint,device)
        old=json.loads(destination.read_text())
        if not old.get('gan_eligibility',{}).get('eligible'): raise RuntimeError('Cached score is not GAN eligible')
    ...
    if arm in ('joint','marginals'):
        ...
        row['expert_diagnostics']=expert_action_diagnostics(bundle,protocol['source_episodes'],split)
        row['timing_scope']='Legacy E_control(state,previous action,terrain) + G2 + routing diagnostics'
    row['gan_eligibility']=gan_eligibility(bundle,arm,checkpoint)
    row['labeled_episodes']=47 if arm=='legacy_joint' else len(bundle['provenance']['labeled_data']['episode_ids'])
...
def refresh_leaderboard(out):
    ...
    if any(s['arm'] not in ALLOWED_ARMS for s in selections):
        raise RuntimeError('GAN-only leaderboard contains an ineligible selection')
    verified={}
    def validate_artifact(arm,checkpoint):
        digest=sha256(checkpoint)
        if (arm,digest) not in verified:
            _,bundle=make_controller(arm,checkpoint,'cpu')
            verified[arm,digest]=gan_eligibility(bundle,arm,checkpoint)
        return digest
    rows=[]
    for selection in selections:
        digest=validate_artifact(selection['arm'],selection['selected_checkpoint'])
        if selection['validation']['checkpoint_sha256']!=digest:
            raise RuntimeError('Playable GAN selection differs from validated checkpoint')
        for role in ('selected','final'):
            if selection.get(role+'_test'):
                row=json.loads(Path(selection[role+'_test']).read_text())
                if row['arm'] not in ALLOWED_ARMS or not row.get('gan_eligibility',{}).get('eligible'):
                    raise RuntimeError('GAN-only leaderboard refuses an ineligible score')
                digest=validate_artifact(row['arm'],row['checkpoint'])
                if row['checkpoint_sha256']!=digest:
                    raise RuntimeError('GAN leaderboard checkpoint changed')
                row['gan_eligibility']=verified[row['arm'],digest]
    ...
    lines=['# Lunar Lander GAN-only control leaderboard','',
    ...
    if eligible:
        ...
        name=lambda arm: 'gan_'+arm if arm in ('joint','marginals') else 'joint'
        controllers={name(s['arm']):dict(checkpoint=s.get('selected_checkpoint'),validation=s['validation']['summary'],step=s['validation'].get('step',0),gan_eligibility=s['validation']['gan_eligibility']) for s in selections}
        write_json(out/'controllers.json',dict(default_controller=name(winner['arm']),controllers=controllers,
            selection='Fresh paired validation landing rate, then mean return; eligible GAN controllers only',protocol_sha256=sha256(out/'protocol.json')))
    return rows
...
def main():
    ...
    parser.add_argument('--log',type=Path,default=ROOT/'results/gym/lunar_lander_gan_control/live.log')
    ...
    if args.baseline:
        ...
        for arm,checkpoint in [('legacy_joint',args.reference)]:
    ...
    if args.arm:
        from lib.gym_gan_control import load_gan_control_checkpoint
        final_bundle=load_gan_control_checkpoint(args.final,args.device)
        ...
        for checkpoint in args.checkpoint:
            candidate=load_gan_control_checkpoint(checkpoint,args.device)
