#!/usr/bin/env python
"""Fresh paired comparison of slider-error and L2 supervision in the previous-action GAN."""
import argparse
import hashlib
import json
import logging
...
from experiments.train_gym_slider_gan import DEFAULTS, source_paths
...
from lib.gym_slider_gan import load_checkpoint, hashes
from lib.gym_previous_gan import control_action_details, decode
from experiments.evaluate_gym_previous_gan import (verify_candidate as verify_l2, verify_protocol as verify_l2_protocol)
from lib.gym_previous_gan import load_checkpoint as load_l2

REPORTS = ROOT / 'reports/gym/lunar_lander_slider_gan'
...
L2_RUN = ROOT/'results/gym/lunar_lander_previous_gan/previous_marginals'
GAN_ARMS = ('sliders_all', 'previous_l2', 'joint')
...
def freeze():
    ...
    validation, test = list(range(1391000, 1391020)), list(range(1491000, 1491050))
    ...
    sources = source_paths() + [Path(__file__), ROOT/'lib/gym_control_evaluation.py', ROOT/'lib/gym_data.py',
        ...
        ROOT/'experiments/evaluate_gym_state_control.py', ROOT/'lib/gym_gan_control.py', ROOT/'experiments/evaluate_gym_previous_gan.py']
    ...
    value = dict(version=1, validation_seeds=validation, test_seeds=test,
        ...
        l2_checkpoints={str(L2_RUN/f'checkpoint_{step}.pt'):sha256(L2_RUN/f'checkpoint_{step}.pt') for step in (250,1000,2500)},
        ...
        interpretation='Sliders versus L2 shares model/data/budget/initialization/record streams; changes paired supervision and adds an error critic; both reselected on the same fresh validation worlds',
        ...
        training='Scratch G1/G2/G3/E/prior/D/R; joint + shared state + action critics retained; all MSE/BCE replaced with paired-error GAN; default MoG prior regularizer retained',
...
def verify_protocol():
    ...
    artifacts = {**value['sources'], **value['prior_protocols'],
        ...
        **{v['path']: v['sha256'] for v in value['references'].values()}, **value['l2_checkpoints']}
...
def verify_candidate(bundle, checkpoint, protocol):
    ...
    baseline = json.loads((L2_RUN/'summary.json').read_text())
    for key, digest in baseline['provenance']['initial_parameters'].items():
        if provenance['initial_parameters'][key] != digest:
            raise ValueError(f'Initial model mismatch against L2: {key}')
    if summary['data_draw_sha256'] != baseline['data_draw_sha256']:
        raise ValueError('Real training minibatch streams differ from L2')
    current = hashes(bundle)
    for name in ('G', 'E', 'prior', 'D.joint', 'D.action', 'D.state', 'R'):
        if current[name] == provenance['initial_parameters'][name]:
            raise ValueError(f'Untrained component: {name}')
    if any(not torch.isfinite(p).all() for k in ('G', 'E', 'prior', 'D', 'R') for p in bundle[k].parameters()):
...
def score(arm, checkpoint, split, name, protocol):
    ...
    elif arm == 'previous_l2':
        bundle = load_l2(checkpoint)
        verify_l2(bundle, checkpoint, verify_l2_protocol())
        action = lambda env,s,a,c: control_action_details(bundle,s,a,c)
...
def evaluate(run_dir):
    ...
    vals = [score('sliders_all', run_dir/f'checkpoint_{step}.pt', 'validation', f'sliders_{step}', protocol)
            for step in protocol['candidate_updates']]
    winner = max(vals, key=selection_key)
    shutil.copyfile(winner['checkpoint'], run_dir/'best.pt')
    selected = score('sliders_all', run_dir/'best.pt', 'test', 'sliders_selected', protocol)
    final = selected if sha256(run_dir/'final.pt') == selected['checkpoint_sha256'] else score(
        'sliders_all', run_dir/'final.pt', 'test', 'sliders_final', protocol)
    rows = [dict(arm='sliders_all', validation=winner, test=selected)]
    l2_vals = [score('previous_l2', L2_RUN/f'checkpoint_{step}.pt', 'validation', f'l2_{step}', protocol)
               for step in protocol['candidate_updates']]
    l2_winner = max(l2_vals, key=selection_key)
    rows.append(dict(arm='previous_l2', validation=l2_winner,
        test=score('previous_l2', Path(l2_winner['checkpoint']), 'test', 'l2_selected', protocol)))
    reference = None
    ...
    for name in ('summary.json', 'provenance.json', 'source.zip', 'config.yaml', 'normalization.json',
                 'recipe.json', 'environment.json', 'metrics.jsonl', 'error_normalization.json'):
        shutil.copyfile(run_dir/name, train_archive/name)
    lines = ['# Slider-error versus L2 supervision', '',
