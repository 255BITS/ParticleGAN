#!/usr/bin/env python
"""Checkpoint interventions: freeze particle centers or reduce fixed noise.

Checkpoint particle-count scaling to 8192 and 16384 centers.
...
# Pin the read-only geometry helper as part of this trainer's source archive.
OVERLAP_DEPENDENCIES = {
    'experiments/probe_cifar_ae_overlap.py': 'b5c666754f58c53ebcbdb82d1b640e1a413422a30ccd72c95ecf05cc4e6c355b',
...
for dependency, expected_sha in OVERLAP_DEPENDENCIES.items():
    assert hashlib.sha256((ROOT/dependency).read_bytes()).hexdigest() == expected_sha
...
DEFAULTS = {
    ...
    'freeze_prior': False, 'noise_scale': 1.,
...
def validate(cfg):
    ...
    if type(cfg['freeze_prior']) is not bool:
        raise ValueError('freeze_prior must be boolean')
    for key in ('noise_scale', 'g_lr_scale', 'lr_scale', 'lr_final_ratio'):
...
def load_resume(cfg):
    ...
    allowed = {'freeze_prior', 'noise_scale', 'expansion_factor', 'initial_eval_samples', 'steps', 'out_dir', 'log_interval', 'eval_interval', 'eval_samples', 'final_samples',
    ...
    return ck, {'path': str(path), 'sha256': digest, 'step': ck['step'],
                ...
                'interventions': {k: {'before': old[k], 'after': cfg[k]} for k in
                    ('freeze_prior', 'noise_scale', 'expansion_factor', 'recon_weight', 'g_lr_scale', 'lr_scale', 'lr_final_ratio', 'lr_decay_start', 'lr_decay_end', 'd_updates', 'recon_grad', 'reg_every', 'penalty_coeff', 'd_warmstart', 'd_warmstart_sha256')
                    if old[k] != cfg[k]}}
...
@torch.no_grad()
def geometry_with_preserved_precision(prior, cfg):
    matmul, cudnn = torch.backends.cuda.matmul.allow_tf32, torch.backends.cudnn.allow_tf32
    try:
        return latent_geometry(prior.means(), prior.sigma, cfg['expansion_factor'], 32768)
    finally:
...
def train(cfg):
    ...
    provenance['sources'].update(OVERLAP_DEPENDENCIES)
    ...
    if checkpoint is not None:
        ...
        # Sigma factors are absolute relative to the original calibrated scale.
        old_noise_scale = checkpoint['config'].get('noise_scale', 1.)
        for module in (prior, ep):
            module.sigma.mul_(cfg['noise_scale'] / old_noise_scale)
        initial_sigma = prior.sigma.clone()
        start_step, previous_train_seconds = checkpoint['step'], checkpoint['train_seconds']
        resume_info['intervened_state_sha256'] = state_hash([g, d, e, prior, eg, ee, ep])
        write_json(out / 'resume.json', resume_info)
        print(f"RESUME step={start_step} parent_sha256={resume_info['sha256']}", flush=True)
        del checkpoint
    if resume_info is None:
        for module in (prior, ep):
            module.sigma.mul_(cfg['noise_scale'])
        initial_sigma = prior.sigma.clone()
    # Keep both live and EMA centers fixed at their respective restored values.
    # Preserving EMA avoids an immediate evaluation distribution jump.
    frozen_prior_z = prior.z.detach().clone()
    frozen_ema_prior_z = ep.z.detach().clone()
    if cfg['freeze_prior']:
    ...
    metadata['prior_intervention'] = {'freeze_prior': cfg['freeze_prior'], 'noise_scale': cfg['noise_scale'],
                                      'ema_centers_frozen_separately': cfg['freeze_prior']}
    write_json(out/'initial_geometry.json', geometry_with_preserved_precision(ep, cfg))
    ...
    with (out / 'metrics.jsonl').open('w', buffering=1) as log:
        for step in range(start_step + 1, cfg['steps'] + 1):
            ...
            for target, source in ((eg, g), (ee, e), (ep, prior)):
                if target is not ep or not cfg['freeze_prior']:
            ...
            if report or evaluate:
                ...
                if evaluate:
                    ...
                    row['latent_geometry'] = geometry_with_preserved_precision(ep, cfg)
    ...
    prior_frozen_unchanged = None
    if cfg['freeze_prior']:
        prior_frozen_unchanged = (torch.equal(prior.z, frozen_prior_z)
                                  and torch.equal(ep.z, frozen_ema_prior_z))
        assert prior_frozen_unchanged, 'frozen live/EMA centers moved'
    summary = {'prior_frozen_unchanged': prior_frozen_unchanged, 'config': cfg, 'final': {'step': step, **row['generation'], 'reconstruction': row['reconstruction']},
